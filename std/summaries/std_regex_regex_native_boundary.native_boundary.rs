...
pub const SOURCE_MODULE: &str = "std.regex.Regex";
pub const NATIVE_MODULE: &str = "std_regex_regex_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ...
    ("named_capture", "std.regex.regex.named_capture", 3),
    ("replace", "std.regex.regex.replace", 3),
    ("split", "std.regex.regex.split", 2),
    ("escape", "std.regex.regex.escape", 1),
];
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        ...
                        "std.regex.regex.named_capture" => native_unimplemented_operation(operation),
                        "std.regex.regex.replace" => native_unimplemented_operation(operation),
                        "std.regex.regex.split" => native_unimplemented_operation(operation),
                        "std.regex.regex.escape" => native_unimplemented_operation(operation),

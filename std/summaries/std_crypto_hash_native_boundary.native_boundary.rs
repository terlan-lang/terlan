...
pub const SOURCE_MODULE: &str = "std.crypto.Hash";
pub const NATIVE_MODULE: &str = "std_crypto_hash_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ...
    ("sha256_nul_separated", "std.crypto.hash.sha256_nul_separated", 1),
];
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        ...
                        "std.crypto.hash.sha256_nul_separated" => native_unimplemented_operation(operation),

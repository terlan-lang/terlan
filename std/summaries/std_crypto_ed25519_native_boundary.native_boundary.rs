...
pub const SOURCE_MODULE: &str = "std.crypto.Ed25519";
pub const NATIVE_MODULE: &str = "std_crypto_ed25519_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("verify", "std.crypto.ed25519.verify", 3),
];
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.crypto.ed25519.verify" => native_unimplemented_operation(operation),

...
pub const SOURCE_MODULE: &str = "std.system.Platform";
pub const NATIVE_MODULE: &str = "std_system_platform_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("current", "std.system.platform.current", 0),
];
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.system.platform.current" => native_unimplemented_operation(operation),

...
pub const SOURCE_MODULE: &str = "std.io.Path";
pub const NATIVE_MODULE: &str = "std_io_path_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("from_string", "std.io.path.from_string", 1),
    ("to_string", "std.io.path.to_string", 1),
    ("join", "std.io.path.join", 2),
    ("file_name", "std.io.path.file_name", 1),
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.io.path.from_string" => native_unimplemented_operation(operation),
                        "std.io.path.to_string" => native_unimplemented_operation(operation),
                        "std.io.path.join" => native_unimplemented_operation(operation),
                        "std.io.path.file_name" => native_unimplemented_operation(operation),

...
pub const SOURCE_MODULE: &str = "std.http.WebSocket";
pub const NATIVE_MODULE: &str = "std_http_web_socket_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("text", "std.http.websocket.text", 1),
    ("ping", "std.http.websocket.ping", 1),
    ("pong", "std.http.websocket.pong", 1),
    ("close", "std.http.websocket.close", 0),
    ("endpoint", "std.http.websocket.endpoint", 2),
];
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.http.websocket.text" => native_unimplemented_operation(operation),
                        "std.http.websocket.ping" => native_unimplemented_operation(operation),
                        "std.http.websocket.pong" => native_unimplemented_operation(operation),
                        "std.http.websocket.close" => native_unimplemented_operation(operation),
                        "std.http.websocket.endpoint" => native_unimplemented_operation(operation),

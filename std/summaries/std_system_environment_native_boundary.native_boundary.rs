...
pub const SOURCE_MODULE: &str = "std.system.Environment";
pub const NATIVE_MODULE: &str = "std_system_environment_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("contains", "std.system.environment.contains", 1),
    ("current_directory", "std.system.environment.current_directory", 0),
];
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.system.environment.contains" => native_unimplemented_operation(operation),
                        "std.system.environment.current_directory" => native_unimplemented_operation(operation),

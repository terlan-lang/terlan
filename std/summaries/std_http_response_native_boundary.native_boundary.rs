...
pub const SOURCE_MODULE: &str = "std.http.Response";
pub const NATIVE_MODULE: &str = "std_http_response_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ...
    ("stream", "std.http.response.stream", 5),
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        ...
                        "std.http.response.stream" => native_unimplemented_operation(operation),

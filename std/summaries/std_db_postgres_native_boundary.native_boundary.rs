...
pub const SOURCE_MODULE: &str = "std.db.Postgres";
pub const NATIVE_MODULE: &str = "std_db_postgres_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("connect", "std.db.postgres.connect", 1),
    ("query", "std.db.postgres.query", 3),
    ("query_one", "std.db.postgres.query_one", 3),
    ("execute", "std.db.postgres.execute", 3),
    ("transaction", "std.db.postgres.transaction", 2),
    ("string", "std.db.postgres.string", 2),
    ("int", "std.db.postgres.int", 2),
    ("bool", "std.db.postgres.bool", 2),
    ("json", "std.db.postgres.json", 2),
];
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.db.postgres.connect" => native_unimplemented_operation(operation),
                        "std.db.postgres.query" => native_unimplemented_operation(operation),
                        "std.db.postgres.query_one" => native_unimplemented_operation(operation),
                        "std.db.postgres.execute" => native_unimplemented_operation(operation),
                        "std.db.postgres.transaction" => native_unimplemented_operation(operation),
                        "std.db.postgres.string" => native_unimplemented_operation(operation),
                        "std.db.postgres.int" => native_unimplemented_operation(operation),
                        "std.db.postgres.bool" => native_unimplemented_operation(operation),
                        "std.db.postgres.json" => native_unimplemented_operation(operation),

...
pub const SOURCE_MODULE: &str = "std.system.Arguments";
pub const NATIVE_MODULE: &str = "std_system_arguments_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("count", "std.system.arguments.count", 0),
];
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.system.arguments.count" => native_unimplemented_operation(operation),

...
pub const SOURCE_MODULE: &str = "std.data.Toml";
pub const NATIVE_MODULE: &str = "std_data_toml_native_boundary";
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.data.toml.parse" => native_unimplemented_operation(operation),

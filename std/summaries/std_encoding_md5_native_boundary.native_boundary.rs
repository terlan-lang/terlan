...
pub const SOURCE_MODULE: &str = "std.encoding.Md5";
pub const NATIVE_MODULE: &str = "std_encoding_md5_native_boundary";
...
pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("digest", "std.encoding.md5.digest", 1),
];
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.encoding.md5.digest" => native_unimplemented_operation(operation),

...
pub const SOURCE_MODULE: &str = "std.package.Registry";
pub const NATIVE_MODULE: &str = "std_package_registry_native_boundary";
pub const SCHEDULER: &str = "normal";

pub const FUNCTIONS: &[(&str, usize)] = &[
    ("parse_publish_request", 1),
];

pub const OPERATIONS: &[(&str, &str, usize)] = &[
    ("parse_publish_request", "std.package.registry.parse_publish_request", 1),
];
        ...
        match command {
            ...
            NativeBoundaryCommand::Call { request_id, operation, args, reply } => {
                let result = match validate_args(&resources, &args) {
                    Ok(()) => match operation {
                        "std.package.registry.parse_publish_request" => native_unimplemented_operation(operation),

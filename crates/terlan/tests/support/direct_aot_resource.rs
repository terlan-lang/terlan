...
pub(super) fn assert_native_resource_transitions(
    image_path: &Path,
    ...
    let mut worker = Command::new(env!("CARGO_BIN_EXE_terlan-native-worker"))
        ...
        .stdout(Stdio::piped())
        .spawn()
        .expect("start native Resource worker");
    ...
    for (request_id, export) in [(1, direct_export), (2, call_export)] {
        ...
        assert_eq!(kind, 8, "unexpected Resource reply payload: {transition:?}");
        assert_resource_frame(&transition);
        write_control_frame(
            worker.stdin.as_mut().unwrap(),
            9,
            &resume_payload(request_id, transition_continuation(&transition), &[9, 41]),
        );
    ...
    assert!(worker.wait().expect("wait for Resource worker").success());
}

pub(super) fn assert_vm_resource_transition(image_path: &Path) {
    let run = |entry: &str| {
        Command::new(env!("CARGO_BIN_EXE_terlan-vm"))
            ...
            .output()
            .expect("run native Resource Terlan consumer")
    };
    let valid = run("resource_then_true");
    assert!(
        valid.status.success(),
        "terlan-vm could not service native Resource transition:\n{}",
        String::from_utf8_lossy(&valid.stderr)
    );
    let invalid = run("resource_zero_invalid");
    assert!(!invalid.status.success());
    assert!(String::from_utf8_lossy(&invalid.stderr).contains("Resource kind tag must be positive"));
}

fn assert_resource_frame(frame: &[u8]) {
    assert_eq!(u16::from_le_bytes(frame[24..26].try_into().unwrap()), 8);

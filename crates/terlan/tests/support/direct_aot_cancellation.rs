...
pub(super) fn assert_native_cancellation_transitions(
    image_path: &Path,
    ...
    let mut worker = Command::new(env!("CARGO_BIN_EXE_terlan-native-worker"))
        ...
        .stdout(Stdio::piped())
        .spawn()
        .expect("start native Cancellation worker");
    ...
    for (request_id, export) in [(1, direct_export), (2, call_export)] {
        ...
        assert_eq!(kind, 8, "unexpected Cancellation reply: {transition:?}");
        assert_cancellation_frame(&transition);
        write_control_frame(
            worker.stdin.as_mut().unwrap(),
            9,
    ...
    assert!(worker
        .wait()
        .expect("wait for Cancellation worker")
        .success());
}

fn assert_cancellation_frame(frame: &[u8]) {
    assert_eq!(u16::from_le_bytes(frame[24..26].try_into().unwrap()), 9);

...
pub(super) fn assert_native_link_transitions(
    image_path: &Path,
    ...
    let mut worker = Command::new(env!("CARGO_BIN_EXE_terlan-native-worker"))
        ...
        .stdout(Stdio::piped())
        .spawn()
        .expect("start native Link worker");
    ...
    for (request_id, export) in [(1, direct_export), (2, call_export)] {
        ...
        assert_eq!(kind, 8, "unexpected Link reply payload: {transition:?}");
        assert_link_frame(&transition);
        write_control_frame(
            worker.stdin.as_mut().unwrap(),
            9,
    ...
    assert!(worker.wait().expect("wait for Link worker").success());
}

fn assert_link_frame(frame: &[u8]) {
    assert_eq!(u16::from_le_bytes(frame[24..26].try_into().unwrap()), 6);

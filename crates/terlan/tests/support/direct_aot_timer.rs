...
pub(super) fn assert_native_timer_transitions(
    image_path: &Path,
    ...
    let mut worker = Command::new(env!("CARGO_BIN_EXE_terlan-native-worker"))
        ...
        .stdout(Stdio::piped())
        .spawn()
        .expect("start native Timer worker");
    ...
    for (request_id, export) in [(1, direct_export), (2, call_export)] {
        ...
        assert_eq!(kind, 8, "unexpected Timer reply payload: {transition:?}");
        assert_timer_frame(&transition);
        write_control_frame(
            worker.stdin.as_mut().unwrap(),
            9,
    ...
    assert!(worker.wait().expect("wait for Timer worker").success());
}

pub(super) fn assert_vm_timer_transition(image_path: &Path) {
    let run = |entry: &str| {
        Command::new(env!("CARGO_BIN_EXE_terlan-vm"))
            ...
            .output()
            .expect("run native Timer Terlan consumer")
    };
    let valid = run("timer_then_true");
    assert!(
        valid.status.success(),
        "terlan-vm could not service native Timer transition:\n{}",
        String::from_utf8_lossy(&valid.stderr)
    );
    for entry in ["timer_zero", "timer_negative"] {
        let invalid = run(entry);
        assert!(!invalid.status.success(), "{entry} unexpectedly succeeded");
        assert!(String::from_utf8_lossy(&invalid.stderr).contains("Timer delay must be positive"));
    }
}

fn assert_timer_frame(frame: &[u8]) {
    assert_eq!(u16::from_le_bytes(frame[24..26].try_into().unwrap()), 5);
    assert_eq!(u16::from_le_bytes(frame[26..28].try_into().unwrap()), 1);
    assert_eq!(u16::from_le_bytes(frame[28..30].try_into().unwrap()), 1);
    assert_eq!(i64::from_le_bytes(frame[30..38].try_into().unwrap()), 3);

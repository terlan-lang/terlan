...
pub(super) fn assert_native_monitor_transitions(
    image_path: &Path,
    ...
    let mut worker = Command::new(env!("CARGO_BIN_EXE_terlan-native-worker"))
        ...
        .stdout(Stdio::piped())
        .spawn()
        .expect("start native Monitor worker");
    ...
    for (request_id, export) in [(1, direct_export), (2, call_export)] {
        write_control_frame(
            ...
            3,
            &call_payload(request_id, export, &[2, 41]),
        );
        let (kind, transition) = read_control_frame(worker.stdout.as_mut().unwrap());
        assert_eq!(kind, 8, "unexpected Monitor reply payload: {transition:?}");
        assert_monitor_frame(&transition);
        write_control_frame(
            worker.stdin.as_mut().unwrap(),
            9,
            &resume_payload(request_id, transition_continuation(&transition), &[7, 41]),
        );
    ...
    assert!(worker.wait().expect("wait for Monitor worker").success());
}

fn assert_monitor_frame(frame: &[u8]) {
    assert_eq!(u16::from_le_bytes(frame[24..26].try_into().unwrap()), 7);
    assert_eq!(u16::from_le_bytes(frame[26..28].try_into().unwrap()), 1);
    assert_eq!(u16::from_le_bytes(frame[28..30].try_into().unwrap()), 1);
    assert_eq!(i64::from_le_bytes(frame[30..38].try_into().unwrap()), 2);

...
pub(super) fn assert_native_scheduling_transitions(
    image_path: &Path,
    ...
    let mut worker = Command::new(env!("CARGO_BIN_EXE_terlan-native-worker"))
        ...
        .stdout(Stdio::piped())
        .spawn()
        .expect("start native Scheduling worker");
    ...
    for (request_id, export) in [(1, direct_export), (2, call_export)] {
        ...
        assert_eq!(kind, 8, "unexpected Scheduling reply: {transition:?}");
        assert_scheduling_frame(&transition);
        write_control_frame(
            worker.stdin.as_mut().unwrap(),
            9,
    ...
    assert!(worker.wait().expect("wait for Scheduling worker").success());
}

pub(super) fn assert_vm_scheduling_transition(image_path: &Path) {
    let run = |entry: &str| {
        Command::new(env!("CARGO_BIN_EXE_terlan-vm"))
            ...
            .output()
            .expect("run native Scheduling Terlan consumer")
    };
    for entry in [
...
fn assert_scheduling_frame(frame: &[u8]) {
    assert_eq!(u16::from_le_bytes(frame[24..26].try_into().unwrap()), 11);

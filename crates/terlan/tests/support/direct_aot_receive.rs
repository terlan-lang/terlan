...
pub(super) fn assert_native_receive_transitions(
    image_path: &Path,
    ...
    let mut worker = Command::new(env!("CARGO_BIN_EXE_terlan-native-worker"))
        ...
        .stdout(Stdio::piped())
        .spawn()
        .expect("start native Receive worker");
    ...
    for (request_id, export) in [(1, direct_export), (2, call_export)] {
        ...
        assert_eq!(kind, 8);
        assert_receive_frame(&transition);
        write_control_frame(
            worker.stdin.as_mut().unwrap(),
            9,
            &resume_payload(request_id, transition_continuation(&transition), &[73, 41]),
        );
        let (success_kind, success) = read_control_frame(worker.stdout.as_mut().unwrap());
        assert_eq!(success_kind, 4);
        assert_eq!(frame_value(&success), 114);
    }
    ...
    assert!(worker.wait().expect("wait for Receive worker").success());
}

fn assert_receive_frame(frame: &[u8]) {
    assert_eq!(u16::from_le_bytes(frame[24..26].try_into().unwrap()), 3);
    assert_eq!(u16::from_le_bytes(frame[26..28].try_into().unwrap()), 0);
    assert_eq!(u16::from_le_bytes(frame[28..30].try_into().unwrap()), 1);
    assert_eq!(i64::from_le_bytes(frame[30..38].try_into().unwrap()), 41);
}

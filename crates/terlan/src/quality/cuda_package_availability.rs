...
///
/// Output:
/// - Success only when CUDA appears available and the future CUDA package check
///   has a real implementation.
/// - Stable failure on CPU-only machines or before the package exists.
///
/// Transformation:
/// - Reuses the default availability probe and refuses to report a fake CUDA
///   package pass until a real external package smoke is wired.
...
impl CudaPackageAvailabilitySummary {
    fn direct_cuda_reason_codes(&self) -> Vec<&'static str> {
        ...
        if !self.toolkit_available {
            missing.push("cuda-toolkit-unavailable");
        }
...
fn validate_cuda_package_execution_readiness(
    summary: &CudaPackageAvailabilitySummary,
) -> QualityResult<()> {
    let reason_codes = summary.direct_cuda_reason_codes();
    if !reason_codes.is_empty() {
        return Err(format!(
            "error[cuda_package_unavailable]: direct CUDA package execution skipped: {}; run `make cuda-package-availability-check` for capability details",
            reason_codes.join(",")
        ));
    }
    Err(
        "error[cuda_package_not_implemented]: CUDA capabilities are available, but the external CUDA package smoke is not implemented"
            .to_string(),
    )
}
...
fn write_status_report(root: &Path, summary: &CudaPackageAvailabilitySummary) -> QualityResult<()> {
    ...
    let report = json!({
        ...
        "direct_cuda": {
            "ready": summary.status == CudaAvailabilityStatus::Available,
...
mod tests {
    ...
    fn probe_keeps_driver_device_toolkit_and_libtorch_independent() {
        ...
        assert_eq!(
            driver_only.summary().direct_cuda_reason_codes(),
            ["cuda-toolkit-unavailable"]
        );
    ...
    fn package_execution_diagnostic_uses_shared_sorted_reason_codes() {
        ...
        assert_eq!(
            validate_cuda_package_execution_readiness(&unavailable)
                .expect_err("missing capabilities must skip"),
            "error[cuda_package_unavailable]: direct CUDA package execution skipped: cuda-device-unavailable,cuda-driver-unavailable,cuda-toolkit-unavailable; run `make cuda-package-availability-check` for capability details"
        );
        ...
        assert_eq!(
            validate_cuda_package_execution_readiness(&available)
                .expect_err("unimplemented package smoke must fail closed"),
            "error[cuda_package_not_implemented]: CUDA capabilities are available, but the external CUDA package smoke is not implemented"
        );
    }
    ...
    fn availability_report_is_deterministic_and_keeps_cuda_lanes_separate() {
        ...
        let report: serde_json::Value =
            serde_json::from_slice(&first).expect("parse status report");
        assert_eq!(report["gate_result"], "passed");
        assert_eq!(report["direct_cuda"]["execution_disposition"], "skip");
        assert_eq!(
            report["direct_cuda"]["reason_codes"],
            json!(["cuda-toolkit-unavailable"])
        );
    ...
    fn availability_report_sorts_reason_codes() {
        ...
        let report: serde_json::Value = serde_json::from_slice(
            &fs::read(root.path().join(STATUS_REPORT)).expect("read report"),
        )
        .expect("parse report");
        assert_eq!(
            report["direct_cuda"]["reason_codes"],

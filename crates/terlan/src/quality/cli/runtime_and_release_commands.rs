...
pub(super) fn run_runtime_and_release_command(
    command: Option<&str>,
    ...
    let result = match command {
        ...
        Some("vm-otp-abstractions-terlan-stdlib") => {
            match run_vm_otp_abstractions_terlan_stdlib(Path::new(".")) {
                Ok(summary) => {
                    println!(
                        "[vm-otp-abstractions-terlan-stdlib] {} behavior modules checked; {} policy docs checked; {} framework intrinsics found; {} direct runtime magic keys found.",
        ...
        Some("aot-developer-hot-reload") => match run_aot_developer_hot_reload(Path::new(".")) {
            Ok(summary) => {
                println!(
                        "[aot-developer-hot-reload] {} contracts and {} adversarial cases checked; report written to {}.",

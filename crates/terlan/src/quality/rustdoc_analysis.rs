...
use super::has_inline_test_marker;
use super::rust_quality::{QualityResult, RustFile, RustItem};
...
pub(super) fn normalized_signature(line: &str) -> String {
...
pub(super) fn line_has_rustdoc(lines: &[String], item_index: usize) -> bool {
...
/// Returns whether an internal declaration is explicitly in Rustdoc policy.
///
/// A `// rustdoc-contract: ...` comment is an auditable opt-in for subsystem
/// contracts, safety-sensitive boundaries, and non-obvious algorithms that
/// are not exported as public Rust APIs. The marker must be adjacent to the
/// declaration, apart from attributes.
pub(super) fn line_has_rustdoc_contract_marker(lines: &[String], item_index: usize) -> bool {
    let mut index = item_index;
    ...
    while index > 0 {
        ...
        if previous.starts_with("//") && !previous.starts_with("///") {
            if previous.contains("rustdoc-contract:") {
                return true;
            }
            index -= 1;
            continue;
...
/// Returns whether a Rust source path is test implementation rather than an
/// API or subsystem implementation.
pub(super) fn is_rustdoc_test_path(path: &Path) -> bool {
    if path.components().any(|component| {
        component.as_os_str().to_str().is_some_and(|name| {
            name == "tests" || name.ends_with("_test") || name.ends_with("_tests")
        })
    }) {
        return true;
    }
...
pub(super) fn escaped_string_state(line: &str, active: bool) -> (bool, bool) {
...
pub(super) fn undocumented_items(items: &[RustItem]) -> BTreeMap<String, RustItem> {
    items
        .iter()
        .filter(|item| item.requires_documentation() && !item.documented)
...
pub(super) fn read_rustdoc_baseline(root: &Path) -> QualityResult<(BTreeSet<String>, Vec<String>)> {
...
pub(super) fn check_rustdoc_baseline(
...
pub(super) fn rustdoc_baseline_path(root: &Path) -> PathBuf {
    root.join("tools")
        .join("quality")
...
pub(super) fn read_size_baseline(
    root: &Path,
) -> QualityResult<(BTreeMap<PathBuf, usize>, Vec<String>)> {
    let path = root
        .join("tools")
        .join("quality")
...
pub(super) fn read_inline_test_baseline(
    root: &Path,
) -> QualityResult<(BTreeSet<PathBuf>, Vec<String>)> {
    let path = root
        .join("tools")
        .join("quality")
...
pub(super) fn files_with_inline_tests(
    root: &Path,
    files: &[RustFile],
) -> QualityResult<BTreeSet<PathBuf>> {
    let mut paths = BTreeSet::new();
    for file in files {
        let logical_path = logical_rust_source_path(root, &file.path);
        if is_rustdoc_test_path(&logical_path) {
            continue;
        }
...
pub(super) fn logical_rust_source_path(root: &Path, path: &Path) -> PathBuf {
...
pub(super) fn check_file_sizes(
    files: &[RustFile],
    baseline: &BTreeMap<PathBuf, usize>,
) -> Vec<String> {
    let mut diagnostics = Vec::new();
    let current = files
        .iter()
...
///   out over time.
pub(super) fn check_inline_tests(
...
pub(crate) fn render_failure(name: &str, diagnostics: &[String]) -> String {

...
use crate::terlan_quality::{
    run_achamp_adversarial_coverage, run_binary_descriptor_contract, run_cli_exact_selectors,
    run_compiler_incremental_cache, run_core_typing_spec, run_cuda_package_availability,
    run_cuda_package_check, run_dev_fast_feedback_profile, run_device_target_planner,
    run_dormant_runtime_code, run_editor_code_action_auto_import_report,
    ...
    run_lean_proof_regression, run_lean_proof_runtime, run_lean_proof_track, run_mobile_boundary,
    run_module_readmes, run_native_binding_generator_contract, run_native_boundary_security,
    run_native_boundary_terminology, run_no_default_tokio_runtime, run_no_implicit_otp_runtime,
    run_no_terlan_vm_erts_rust_dependency, run_operator_coverage_100, run_otp_reference_inventory,
    run_otp_runtime_exit, run_otp_test_pipeline_inventory, run_oxc_boundary,
    run_package_api_compatibility, run_package_build_artifact_isolation,
    ...
    run_std_test_honesty, run_terlan_lint_style_profile, run_terlan_polars_package,
    run_terlan_vm_external_repo_boundary, run_terlan_vm_internal_crate, run_test_hierarchy,
    run_typed_template_render_mode, run_vm_artifact_format, run_vm_db_migration_command,
    run_vm_deterministic_hashmap, run_vm_dev_dependency_orchestration, run_vm_diagnostics_quality,
...
include!("main_command_group_001.rs");
include!("main_command_group_002.rs");
include!("main_command_group_003.rs");
...
fn main() -> ExitCode {
    ...
    if let Some(result) = run_command_group_001(command, &mut args) {
        return result;
    }
    if let Some(result) = run_command_group_002(command, &mut args) {
        return result;
    }
    if let Some(result) = run_command_group_003(command, &mut args) {
        return result;
    }
    match command {
        Some(command) => {

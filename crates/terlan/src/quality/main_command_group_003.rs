fn run_command_group_003(
    ...
    let result = match command {
        ...
        Some("terlan-polars-package") => match run_terlan_polars_package(Path::new(".")) {
            Ok(summary) => {
                println!(
                    "[terlan-polars-package] {} operations and {} package tests covered by the boundary gate.",
                    summary.checked_operation_count, summary.checked_test_count
                );
                ExitCode::SUCCESS
            }
            Err(message) => failure(message),

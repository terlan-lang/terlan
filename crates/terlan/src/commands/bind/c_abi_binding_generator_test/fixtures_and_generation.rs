...
pub(super) fn contains_ignoring_whitespace(source: &str, expected: &str) -> bool {
...
pub(super) fn generated_files(root: &Path) -> Vec<PathBuf> {
...
#[test]
pub(super) fn structured_c_metadata_generates_real_ffi_package() {
    ...
    assert!(package.contains("helper_env = \"TERLAN_C_ABI_FIXTURE_NATIVE_BOUNDARY_HELPER_PATH\""));
...
pub(super) fn immutable_only_packages_do_not_emit_unused_mutable_accessors() {
    let manifest = write_fixture_variant("immutable_only_accessors", |metadata| {
        metadata["modules"][0]["functions"]
            .as_array_mut()
            .expect("functions")
            .retain(|function| function["role"] != "mutable_method");
    });
    let out_dir = temp_dir("immutable_only_accessors_output");

    generate_c_abi_bindings(&manifest, &out_dir).expect("generate immutable-only C ABI package");
    let helper = fs::read_to_string(out_dir.join("native/rust/src/bin/native_boundary_helper.rs"))
        .expect("helper");

    assert!(helper.contains("fn live_nativeboundary(&self"));
    assert!(!helper.contains("fn live_nativeboundary_mut("));
...
#[test]
pub(super) fn supplemental_private_c_headers_are_copied_and_compiled() {
...
#[test]
pub(super) fn generated_package_identity_is_independent_from_native_crate_identity() {
...
#[test]
pub(super) fn generated_package_identity_rejects_non_exact_versions() {
...
#[test]
pub(super) fn c_abi_generation_rejects_disabled_validation_obligations() {
...
#[test]
pub(super) fn c_abi_generation_rejects_placeholder_native_symbols() {
    let manifest = write_fixture_variant("placeholder_native_symbol", |metadata| {
        symbol_mut(metadata, "function.native_boundary_create")["c_name"] =
            Value::String("placeholder_native_symbol".to_string());
    });
    let error = generate_c_abi_bindings(&manifest, &temp_dir("placeholder_native_symbol_output"))
        .expect_err("placeholder native symbol must fail");
    assert!(error.contains("error[native_bindgen.placeholder_symbol]"));
    fs::remove_dir_all(manifest.parent().expect("variant root")).expect("remove variant");
}

#[test]
pub(super) fn generated_scalar_smoke_can_defer_package_specific_operation_domains() {
...
pub(super) fn dispatcher_metadata_generates_owned_optional_and_integer_list_stack_values() {
    ...
    assert!(contains_ignoring_whitespace(
        &adapter,
        "dispatcher_optional_dimensions.write_stable_ivalue(dispatcher_list_dimensions.into_stable_ivalue())"
    ));
    assert!(contains_ignoring_whitespace(
        &adapter,
...
#[test]
pub(super) fn dispatcher_metadata_generates_owned_fixed_string_stack_values() {

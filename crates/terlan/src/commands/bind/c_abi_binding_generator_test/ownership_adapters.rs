...
#[test]
pub(super) fn c_abi_wrapper_generates_optional_owned_resource_results() {
    let manifest = write_fixture_variant("optional_resource_output", |metadata| {
        metadata["c_metadata"]["symbols"]
            .as_array_mut()
            .expect("symbols")
            .push(serde_json::json!({
                "id": "function.native_boundary_maybe_clone",
                "c_name": "terlan_c_native_boundary_maybe_clone",
                "kind": "function",
                "status": "bind",
                "returns": "int32_t",
        ...
        metadata["modules"][0]["functions"]
            ...
            .push(serde_json::json!({
                "name": "maybe_clone",
                "operation": "c_abi_fixture.native_boundary.maybe_clone",
                "c_symbol": "function.native_boundary_maybe_clone",
                "role": "immutable_method",
                "args": [{"name": "boundary", "ty": "NativeBoundary"}],
                "returns": "Option[NativeBoundary]",
                "blocking": "fast",
                "resource": "opaque_handle",
                "documentation": "Optionally returns an independently owned native boundary.",
                "generated_smoke": "package_owned"
            }));
    });
    let out_dir = temp_dir("optional_resource_output_generated");

    generate_c_abi_bindings(&manifest, &out_dir).expect("generate optional resource wrapper");
    let module =
        fs::read_to_string(out_dir.join("src/c_abi_fixture/NativeBoundary.terl")).expect("module");
    assert!(module.contains("maybe_clone(boundary: NativeBoundary): Option[NativeBoundary]"));
    let adapter = fs::read_to_string(out_dir.join("native/rust/src/lib.rs")).expect("adapter");
    assert!(
        adapter.contains("pub fn maybe_clone(&self) -> Result<Option<NativeBoundary>, CAbiError>")
    );
    assert!(adapter.contains("Ok(NonNull::new(raw).map(|raw| Self { raw }))"));
    let helper = fs::read_to_string(out_dir.join("native/rust/src/bin/native_boundary_helper.rs"))
        .expect("helper");
    assert!(helper.contains("Ok(Some(value))"));
    assert!(helper.contains("ok_some_handle"));
    assert!(helper.contains("Ok(None) => \"ok_none\""));
...
#[test]
pub(super) fn c_abi_wrapper_generates_owned_resource_list_results() {
    let manifest = write_fixture_variant("resource_list_output", |metadata| {
        let symbols = metadata["c_metadata"]["symbols"]
            .as_array_mut()
            .expect("symbols");
        symbols.push(serde_json::json!({
            "id": "function.delete_owned_handle_array",
            "c_name": "terlan_c_delete_owned_handle_array",
            "kind": "function",
            "status": "bind",
            "returns": "void",
        ...
        symbols.push(serde_json::json!({
            "id": "function.native_boundary_clones",
            "c_name": "terlan_c_native_boundary_clones",
            "kind": "function",
            "status": "bind",
            "returns": "int32_t",
            "error_model": "status_code",
            ...
            "parameters": [
                {"name": "boundary", "c_type": "const TerlanCNativeBoundary *", "direction": "input", "ownership": "borrowed_call"},
                {
                    ...
                    "owned_handle_array": {
                        ...
                        "destructor_symbol": "function.delete_owned_handle_array",
        ...
        metadata["modules"][0]["functions"]
            ...
            .push(serde_json::json!({
                "name": "clones",
                "operation": "c_abi_fixture.native_boundary.clones",
                "c_symbol": "function.native_boundary_clones",
                "role": "immutable_method",
                "args": [{"name": "boundary", "ty": "NativeBoundary"}],
                "returns": "List[NativeBoundary]",
                "blocking": "fast",
                "resource": "opaque_handle",
                "documentation": "Returns independently owned native boundaries.",
                "generated_smoke": "package_owned"
            }));
    });
    let out_dir = temp_dir("resource_list_output_generated");

    generate_c_abi_bindings(&manifest, &out_dir).expect("generate resource-list wrapper");
    let adapter = fs::read_to_string(out_dir.join("native/rust/src/lib.rs")).expect("adapter");
    assert!(adapter.contains("pub fn clones(&self) -> Result<Vec<NativeBoundary>, CAbiError>"));
    assert!(adapter.contains("ffi::terlan_c_delete_owned_handle_array(out_out_boundaries)"));
    assert!(adapter.contains("ffi::terlan_c_native_boundary_destroy(value as _)"));
    assert!(adapter.contains("raw_values.iter().any(|value| *value == 0)"));
    assert!(adapter.contains("raw: unsafe { NonNull::new_unchecked(value as _) }"));
    let helper = fs::read_to_string(out_dir.join("native/rust/src/bin/native_boundary_helper.rs"))
        .expect("helper");
    assert!(helper.contains("Ok(values) => values"));
    assert!(helper.contains("ok_handles"));

    let valid: Value =
        serde_json::from_str(&fs::read_to_string(&manifest).expect("read resource-list manifest"))
            .expect("parse resource-list manifest");
    let mut malformed_length = valid.clone();
    symbol_mut(&mut malformed_length, "function.native_boundary_clones")["parameters"][2]
        ["c_type"] = Value::String("int64_t *".to_string());
    fs::write(
        &manifest,
        serde_json::to_string_pretty(&malformed_length)
            .expect("render malformed resource-list length"),
    )
    .expect("write malformed resource-list length");
    let error = generate_c_abi_bindings(
        &manifest,
        &temp_dir("malformed_resource_list_length_generated"),
    )
    .expect_err("owned handle arrays require a size_t output length");
    assert!(error.contains("error[native_bindgen.c_owned_handle_array_contract]"));

    let mut mismatched_element = valid;
    mismatched_element["modules"][0]["functions"]
        .as_array_mut()
        .expect("functions")
        .last_mut()
        .expect("resource-list function")["returns"] =
        Value::String("List[OtherBoundary]".to_string());
    fs::write(
        &manifest,
        serde_json::to_string_pretty(&mismatched_element)
            .expect("render mismatched resource-list element"),
    )
    .expect("write mismatched resource-list element");
    let error = generate_c_abi_bindings(
        &manifest,
        &temp_dir("mismatched_resource_list_element_generated"),
    )
    .expect_err("owned handle array element metadata must match the Terlan list type");
    assert!(error.contains("error[native_bindgen.c_owned_handle_array_contract]"));
...
#[test]
pub(super) fn c_abi_wrapper_copies_owned_utf8_outputs_and_calls_the_named_destructor() {
...
#[test]
pub(super) fn c_abi_wrapper_copies_owned_int_arrays_and_calls_the_named_destructor() {
...
#[test]
pub(super) fn c_abi_wrapper_copies_owned_float_arrays_and_calls_the_named_destructor() {
...
#[test]
pub(super) fn c_abi_wrapper_validates_and_copies_owned_bool_arrays() {
...
#[test]
pub(super) fn c_abi_wrapper_copies_owned_utf8_string_arrays_and_calls_the_named_destructor() {
...
#[test]
pub(super) fn c_abi_wrapper_supports_borrowed_int_input_arrays_with_linked_lengths() {
...
#[test]
pub(super) fn c_abi_wrapper_supports_borrowed_opaque_resource_input_arrays() {
...
#[test]
pub(super) fn c_abi_resource_input_arrays_require_known_matching_resource_types() {
...
#[test]
pub(super) fn c_abi_wrapper_supports_borrowed_float_input_arrays_with_linked_lengths() {
...
#[test]
pub(super) fn c_abi_wrapper_supports_borrowed_byte_inputs_in_the_native_worker() {
    let manifest = write_fixture_variant("byte_input_arrays", |metadata| {
        metadata["c_metadata"]["symbols"]
            .as_array_mut()
            .expect("symbols")
            .push(serde_json::json!({
                "id": "function.native_boundary_from_bytes",
                "c_name": "terlan_c_native_boundary_from_bytes",
                "kind": "function",
                "status": "bind",
                "returns": "int32_t",
                "error_model": "status_code",
                "success_code": 0,
                "parameters": [
                    {"name": "bytes", "c_type": "const uint8_t *", "direction": "input", "ownership": "borrowed_call", "input_array": {"length_parameter": "bytes_length", "bytes": true}},
                    {"name": "bytes_length", "c_type": "int64_t", "direction": "input", "ownership": "value"},
                    {"name": "out_boundary", "c_type": "TerlanCNativeBoundary **", "direction": "output", "ownership": "transfer_full"}
                ]
            }));
        metadata["modules"][0]["functions"]
            ...
            .push(serde_json::json!({
                "name": "from_bytes",
                "operation": "c_abi_fixture.native_boundary.from_bytes",
                "c_symbol": "function.native_boundary_from_bytes",
                "role": "constructor",
                "args": [{"name": "bytes", "ty": "Bytes"}],
                "returns": "NativeBoundary",
                "blocking": "fast",
                "resource": "opaque_handle",
                "documentation": "Copies bytes into a boundary."
            }));
    });
    let out_dir = temp_dir("byte_input_arrays_output");

    generate_c_abi_bindings(&manifest, &out_dir).expect("generate byte input C wrapper");
    let adapter = fs::read_to_string(out_dir.join("native/rust/src/lib.rs")).expect("adapter");
    assert!(adapter.contains("pub fn from_bytes(bytes: &[u8]) -> Result<Self, CAbiError>"));
    assert!(adapter.contains("let bytes_length = i64::try_from(bytes.len())"));
    assert!(adapter.contains("bytes.as_ptr(), bytes_length, &mut raw"));
    let helper = fs::read_to_string(out_dir.join("native/rust/src/bin/native_boundary_helper.rs"))
        .expect("helper");
    assert!(helper.contains("const MAX_ADAPTER_TRANSFER_BYTES: usize = 16777216"));
    assert!(helper.contains("Arg::Bytes(bytes)"));
    assert!(helper.contains("bytes.as_slice()"));
    assert!(helper.contains("strip_prefix(\"x:\")"));
    assert!(helper.contains("copied byte argument exceeds the native adapter transfer bound"));
    let source = fs::read_to_string(out_dir.join("src/c_abi_fixture/NativeBoundary.terl"))
        .expect("Terlan source");
    assert!(source.contains("pub from_bytes(bytes: Bytes): NativeBoundary"));
...
#[test]
pub(super) fn c_abi_wrapper_copies_bool_input_arrays_into_explicit_bytes() {
...
#[test]
pub(super) fn c_abi_bool_input_arrays_require_bool_terlan_arguments() {
...
pub(super) fn compile_and_exercise_generated_c_adapter(
    out_dir: &Path,
    target_dir: &Path,
) -> PathBuf {
    ...
    assert!(
        ...
        "foreign worker handle was not rejected:\n{helper_stdout}"
    );
    helper
}

#[test]
pub(super) fn generated_c_adapter_compiles_and_enforces_public_protocol() {
...
#[test]
pub(super) fn generated_c_ffi_compiles_links_owns_and_executes_from_terlan() {
    let _guard = crate::commands::bind::native_helper_env_lock()
        .lock()
        .expect("native helper env lock");
...
#[test]
pub(super) fn skipped_c_symbols_are_sorted_and_cover_required_rejection_families() {

...
pub(super) fn render_consumer_test(manifest: &CAbiBindingManifest) -> Result<String, String> {
...
pub(super) fn first_type(manifest: &CAbiBindingManifest) -> Result<&CAbiBindingType, String> {
    manifest
        .modules
        .iter()
...
pub(super) fn binding_types(
    manifest: &CAbiBindingManifest,
) -> Vec<(&CAbiBindingModule, &CAbiBindingType)> {
    manifest
        .modules
        .iter()
...
/// Resolves `Option[OpaqueType]` without treating other generic types as resources.
pub(super) fn optional_binding_type<'a>(
    manifest: &'a CAbiBindingManifest,
    name: &str,
) -> Option<(&'a CAbiBindingModule, &'a CAbiBindingType)> {
    let inner = name.strip_prefix("Option[")?.strip_suffix(']')?;
    binding_type(manifest, inner)
}

/// Resolves `List[OpaqueType]` without treating primitive lists as resources.
pub(super) fn list_binding_type<'a>(
    manifest: &'a CAbiBindingManifest,
    name: &str,
) -> Option<(&'a CAbiBindingModule, &'a CAbiBindingType)> {
...
pub(super) fn dispose_for_type<'a>(
    manifest: &'a CAbiBindingManifest,
    type_name: &str,
...
pub(super) fn function_owner_type<'a>(
    manifest: &'a CAbiBindingManifest,
...
pub(super) fn role_name(role: CAbiFunctionRole) -> &'static str {
    match role {
...
pub(super) fn blocking_policy_name(policy: &CAbiBlockingPolicy) -> &'static str {
    match policy {
...
pub(super) fn resource_policy_name(policy: &CAbiResourcePolicy) -> &'static str {
    match policy {
...
pub(super) fn c_pointer_base(c_type: &str) -> &str {
    c_type
        .trim()
...
pub(super) fn validate_relative_metadata_path(value: &str) -> Result<(), String> {
...
pub(super) fn is_environment_variable(value: &str) -> bool {
    !value.is_empty()
...
pub(super) fn is_link_library_name(value: &str) -> bool {
    !value.is_empty()
        && value
            .bytes()
...
pub(super) fn is_version_requirement(value: &str) -> bool {
    !value.is_empty()
...
pub(super) fn is_pinned_cargo_version(value: &str) -> bool {
...
pub(super) fn consumer_test_path(namespace: &str) -> PathBuf {

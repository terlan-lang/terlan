...
pub(super) struct SafeWrapperRendering<'a> {
    pub(super) manifest: &'a CAbiBindingManifest,
    pub(super) symbols: &'a BTreeMap<&'a str, &'a CSymbol>,
    pub(super) aliases: &'a BTreeMap<String, String>,
}

pub(super) struct SafeWrapperTarget<'a> {
    pub(super) function: &'a CAbiBindingFunction,
    pub(super) symbol: &'a CSymbol,
    pub(super) record: Option<&'a CSymbol>,
    pub(super) ty: Option<&'a CAbiBindingType>,
    pub(super) inside_impl: bool,
}

pub(super) fn render_safe_wrapper(
    rendering: SafeWrapperRendering<'_>,
    target: SafeWrapperTarget<'_>,
) -> Result<String, String> {
    let SafeWrapperRendering {
        manifest,
        symbols,
        aliases,
    } = rendering;
    let SafeWrapperTarget {
        function,
        symbol,
        record,
        ty,
    ...
    let owner_name = ty.map(|ty| ty.name.as_str());
    ...
    for argument in &function.args {
        if owner_name == Some(argument.ty.as_str())
            && inside_impl
            && !receiver_skipped
            && matches!(
                function.role,
    ...
    let return_ty = match function.returns.as_str() {
        value if owner_name == Some(value) && inside_impl => "Self".to_string(),
        value if binding_type(manifest, value).is_some() => value.to_string(),
        value if optional_binding_type(manifest, value).is_some() => {
            let (_, inner) =
                optional_binding_type(manifest, value).expect("matched optional resource return");
        ...
        "List[Int]" => "Vec<i64>".to_string(),
        "List[Float]" => "Vec<f64>".to_string(),
        "List[Bool]" => "Vec<bool>".to_string(),
    ...
    if let Some(dispatcher) = &function.dispatcher {
        let record = record.ok_or_else(|| {
            format!(
                "error[native_bindgen.dispatcher_resource_required]: `{}` requires an opaque resource",
                function.name
            )
        })?;
    ...
    let mut owned_handle_array_output: Option<(String, String, String, String, String)> = None;
    let mut handle_output: Option<(&CAbiBindingType, bool)> = None;
    ...
    let owned_string_array_counts = symbol
        ...
        .collect::<BTreeMap<_, _>>();
    let owned_handle_array_lengths = symbol
        .parameters
        .iter()
        .filter_map(|parameter| {
            parameter
    ...
    for parameter in &symbol.parameters {
        ...
        if let Some(array_name) = owned_handle_array_lengths.get(parameter.name.as_str()) {
            call_args.push(format!("&mut out_{}_length", array_name));
            continue;
        }
        ...
        if let Some(array) = &parameter.owned_handle_array {
            let (_, output_ty) = binding_type(manifest, &array.element_type).ok_or_else(|| {
                format!(
                    "error[native_bindgen.c_owned_handle_array_contract]: unknown opaque element `{}`",
                    array.element_type
                )
            })?;
            let output_record = symbols
                .get(output_ty.c_symbol.as_str())
                .copied()
                .ok_or_else(|| format!("unknown C record `{}`", output_ty.c_symbol))?;
            let element_destructor = output_record
                .destructor_symbol
                .as_deref()
                .and_then(|symbol| symbols.get(symbol).copied())
                .ok_or_else(|| format!(
                    "error[native_bindgen.c_owned_handle_array_contract]: `{}` has no element destructor",
                    output_ty.name
                ))?;
            let array_destructor = symbols
                .get(array.destructor_symbol.as_str())
                .expect("validated owned-handle-array destructor");
            rendered.push_str(&format!(
                "{indent}    let mut out_{}: *mut u64 = std::ptr::null_mut();\n{indent}    let mut out_{}_length: usize = 0;\n",
                parameter.name, parameter.name
            ));
            call_args.push(format!("&mut out_{}", parameter.name));
            owned_handle_array_output = Some((
                format!("out_{}", parameter.name),
                format!("out_{}_length", parameter.name),
                array_destructor.c_name.clone(),
                element_destructor.c_name.clone(),
        ...
        match (parameter.direction, parameter.ownership, is_handle) {
            ...
            (Some(CParameterDirection::Output), Some(CParameterOwnership::TransferFull), true) => {
                ...
                let optional = optional_binding_type(manifest, &function.returns)
                    .is_some_and(|(_, optional_ty)| optional_ty.name == output_ty.name);
                if function.returns != output_ty.name && !optional {
                    return Err(format!(
                ...
                handle_output = Some((output_ty, optional));
            }
            ...
            (Some(CParameterDirection::Input), Some(CParameterOwnership::Value), false) => {
                ...
                if source_rust_ty == Some(rust_ty.as_str()) {
                    call_args.push(parameter.name.clone());
                } else {
    ...
    let result = if let Some((output_ty, optional)) = handle_output {
        let constructor = if inside_impl && owner_name == Some(output_ty.name.as_str()) {
            "Self".to_string()
        } else {
            output_ty.name.clone()
        };
        if optional {
            format!("Ok(NonNull::new(raw).map(|raw| {constructor} {{ raw }}))")
        } else {
            format!(
    ...
    } else if let Some((pointer, length, array_destructor, element_destructor, output_ty)) =
        owned_handle_array_output
    {
        if !fallible {
            return Err(format!(
                "error[native_bindgen.c_owned_handle_array_contract]: `{}` must report status",
                function.name
            ));
        }
        format!(
            "let raw_values = if {length} == 0 {{\n{indent}        Ok(Vec::new())\n{indent}    }} else {{\n{indent}        NonNull::new({pointer}).map(|pointer| {{\n{indent}            // SAFETY: the producer returned `{length}` initialized transferred handle identities.\n{indent}            unsafe {{ std::slice::from_raw_parts(pointer.as_ptr(), {length}).to_vec() }}\n{indent}        }}).ok_or(CAbiError {{ operation: {:?}, status: -1 }})\n{indent}    }};\n{indent}    // SAFETY: only the temporary identity array is released; its resource elements were transferred.\n{indent}    unsafe {{ ffi::{array_destructor}({pointer}) }};\n{indent}    let raw_values = raw_values?;\n{indent}    if raw_values.iter().any(|value| *value == 0) {{\n{indent}        for value in raw_values.into_iter().filter(|value| *value != 0) {{\n{indent}            // SAFETY: nonzero identities still belong to this failed transfer.\n{indent}            let _ = unsafe {{ ffi::{element_destructor}(value as _) }};\n{indent}        }}\n{indent}        return Err(CAbiError {{ operation: {:?}, status: -1 }});\n{indent}    }}\n{indent}    Ok(raw_values.into_iter().map(|value| {output_ty} {{\n{indent}        // SAFETY: zero identities were rejected above.\n{indent}        raw: unsafe {{ NonNull::new_unchecked(value as _) }},\n{indent}    }}).collect())",
            function.operation, function.operation
        )
    ...
    } else if let Some((value, rust_ty)) = scalar_output {
        if fallible {
            ...
            } else if function.returns == "Bool" || rust_ty == "i64" {
                format!("Ok({value})")
            } else {
        ...
        let return_ty = rust_ffi_type(symbol.returns.as_deref().unwrap_or("void"), aliases)?;
        if return_ty == "i64" {
            "value".to_string()
        } else {
...
pub(super) fn render_dispatcher_wrapper_body(
...
pub(super) fn render_raw_ffi_function(
    symbol: &CSymbol,

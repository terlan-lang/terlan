...
pub(super) fn validate_c_type_references(
...
pub(super) fn validate_borrowed_arrays(
    symbols: &BTreeMap<&str, &CSymbol>,
    aliases: &BTreeMap<String, String>,
) -> Result<(), String> {
    let opaque_names = symbols
...
pub(super) fn validate_owned_strings(
    symbols: &BTreeMap<&str, &CSymbol>,
    aliases: &BTreeMap<String, String>,
) -> Result<(), String> {
    for symbol in symbols.values().filter(|symbol| {
...
pub(super) fn validate_owned_arrays(
    symbols: &BTreeMap<&str, &CSymbol>,
    aliases: &BTreeMap<String, String>,
) -> Result<(), String> {
    for symbol in symbols.values().filter(|symbol| {
...
pub(super) fn validate_owned_string_arrays(
    symbols: &BTreeMap<&str, &CSymbol>,
    aliases: &BTreeMap<String, String>,
) -> Result<(), String> {
    for symbol in symbols.values().filter(|symbol| {
...
pub(super) fn parameter_metadata_count(parameter: &CParameter) -> usize {
    usize::from(parameter.borrowed_array.is_some())
        ...
        + usize::from(parameter.owned_handle_array.is_some())
        + usize::from(parameter.fixed.is_some())
}

pub(super) fn validate_owned_handle_arrays(
    symbols: &BTreeMap<&str, &CSymbol>,
    aliases: &BTreeMap<String, String>,
) -> Result<(), String> {
    for symbol in symbols.values().filter(|symbol| {
        ...
        for parameter in &symbol.parameters {
            let Some(array) = &parameter.owned_handle_array else {
                continue;
            };
            if symbol.error_model != Some(CErrorModel::StatusCode)
                || parameter.direction != Some(CParameterDirection::Output)
                || parameter.ownership != Some(CParameterOwnership::TransferFull)
                || resolve_c_type(&parameter.c_type, aliases)? != "uint64_t **"
                || parameter_metadata_count(parameter) != 1
            {
                return Err(format!(
                    "error[native_bindgen.c_owned_handle_array_contract]: `{}` must be a status-controlled uint64_t ** owned-handle output",
                    parameter.name
                ));
            }
            let length = symbol
                ...
                .find(|candidate| candidate.name == array.length_parameter)
                .ok_or_else(|| format!(
                    "error[native_bindgen.c_owned_handle_array_contract]: `{}` names unknown length parameter `{}`",
                    parameter.name, array.length_parameter
                ))?;
            if length.direction != Some(CParameterDirection::Output)
                || length.ownership != Some(CParameterOwnership::BorrowedCall)
                || resolve_c_type(&length.c_type, aliases)? != "size_t *"
                || parameter_metadata_count(length) != 0
            {
                return Err(format!(
                    "error[native_bindgen.c_owned_handle_array_contract]: `{}` requires a distinct size_t output length parameter",
                    parameter.name
                ));
            }
            let destructor = symbols.get(array.destructor_symbol.as_str()).ok_or_else(|| {
                format!(
                    "error[native_bindgen.c_owned_handle_array_contract]: `{}` names unknown destructor `{}`",
                    parameter.name, array.destructor_symbol
                )
            })?;
            if destructor.status != CSymbolStatus::Bind
                ...
                || resolve_c_type(&destructor.parameters[0].c_type, aliases)? != "uint64_t *"
            {
                return Err(format!(
                    "error[native_bindgen.c_owned_handle_array_contract]: destructor `{}` must consume one owned uint64_t pointer",
                    array.destructor_symbol
                ));
            }
        }
...
pub(super) fn validate_c_symbol(
    symbol: &CSymbol,
    aliases: &BTreeMap<String, String>,
) -> Result<(), String> {
    ...
    if ["placeholder", "unimplemented", "todo_native"]
        .iter()
        .any(|marker| {
            symbol.id.to_ascii_lowercase().contains(marker)
                || symbol.c_name.to_ascii_lowercase().contains(marker)
        })
    {
        return Err(format!(
            "error[native_bindgen.placeholder_symbol]: structured C symbol `{}` uses placeholder identity `{}`",
            symbol.id, symbol.c_name
        ));
    }
    match symbol.status {
...
pub(super) fn validate_c_return_shape(
    symbol: &CSymbol,
    c_type: &str,
    aliases: &BTreeMap<String, String>,
) -> Result<(), String> {
...
pub(super) fn validate_c_parameter_shape(
    symbol: &CSymbol,
...
pub(super) fn validate_c_fixed_input(
    parameter: &CParameter,
    fixed: &CFixedInput,
    resolved: &str,
...
pub(super) fn validate_c_input_arrays(
    symbol: &CSymbol,
    aliases: &BTreeMap<String, String>,
) -> Result<(), String> {
    for parameter in &symbol.parameters {
...
pub(super) fn validate_owned_string_binding(
    function: &CAbiBindingFunction,
    symbol: &CSymbol,
) -> Result<(), String> {
    let outputs = symbol
...
pub(super) fn validate_owned_array_binding(
    function: &CAbiBindingFunction,
    symbol: &CSymbol,
) -> Result<(), String> {
    let outputs = symbol

...
#[test]
pub(super) fn structured_build_plan_renders_without_shell_commands() {
...
#[test]
pub(super) fn structured_build_plan_rejects_unsafe_or_ambiguous_tokens() {
    let cases = [
        (
            "build_parent",
...
#[test]
pub(super) fn package_null_failures_use_a_hidden_finite_native_status_probe() {
...
#[test]
pub(super) fn package_null_failure_policy_rejects_untrusted_or_ambiguous_classification() {
    let cases = [
        (
            "unknown_probe",
            serde_json::json!({
...
#[test]
pub(super) fn every_opaque_resource_requires_a_producer_and_independent_disposer() {
...
#[test]
pub(super) fn opaque_resource_accepts_a_reviewed_cross_module_producer() {
...
#[test]
pub(super) fn owned_value_projection_accepts_a_reviewed_cross_module_record() {
...
#[test]
pub(super) fn copied_value_records_require_complete_reviewed_field_projections() {
...
#[test]
pub(super) fn copied_record_inputs_require_complete_ordered_typed_parameter_mappings() {
...
#[test]
pub(super) fn generated_helpers_lower_only_reviewed_enum_argument_atoms() {
...
#[test]
pub(super) fn owned_double_vectors_map_to_copied_float_lists() {
...
#[test]
pub(super) fn borrowed_numeric_slices_map_to_copied_list_arguments() {
...
#[test]
pub(super) fn handle_result_policy_distinguishes_owned_and_nullable_producers() {
    let out_dir = temp_dir("explicit_handle_result_policy");
    generate_cpp_bindings(&fixture_manifest(), &out_dir)
        .expect("generate explicit owned and nullable handle policies");

    let metadata =
        fs::read_to_string(out_dir.join("native/terlan-native.toml")).expect("native metadata");
    assert!(metadata.contains("resource = \"owned_handle\""));
    assert!(metadata.contains("resource = \"nullable_handle\""));

    let docs = fs::read_to_string(out_dir.join("docs/cpp_fixture.NativeGauge.md"))
        .expect("nullable handle docs");
    assert!(docs.contains("Ownership: `nullable_handle`"));
    fs::remove_dir_all(out_dir).expect("remove generated package");
}

#[test]
pub(super) fn nullable_handle_requires_finite_failure_policy_and_owned_resource_result() {
    let missing_policy = write_fixture_variant("nullable_without_failure_policy", |manifest| {
        manifest
            .as_object_mut()
            .expect("manifest object")
            .remove("null_failure");
    });
    let error = generate_cpp_bindings(
        &missing_policy,
        &temp_dir("nullable_without_failure_policy_out"),
    )
    .expect_err("nullable handle without failure policy must fail");
    assert!(error.contains("error[cpp.nullable_handle.failure_policy]"));
    fs::remove_dir_all(missing_policy.parent().expect("variant root")).expect("remove variant");

    let scalar_result = write_fixture_variant("owned_scalar_result", |manifest| {
        function_mut(manifest, "value")["resource"] = Value::String("owned_handle".into());
    });
    let error = generate_cpp_bindings(&scalar_result, &temp_dir("owned_scalar_result_out"))
        .expect_err("owned handle classification on scalar result must fail");
    assert!(error.contains("error[cpp.ownership.handle_result]"));
    fs::remove_dir_all(scalar_result.parent().expect("variant root")).expect("remove variant");
}

#[test]
pub(super) fn generated_helpers_decode_numeric_lists_and_typed_empty_lists() {
...
#[test]
pub(super) fn copied_numeric_lists_reject_mismatched_cpp_slice_elements() {

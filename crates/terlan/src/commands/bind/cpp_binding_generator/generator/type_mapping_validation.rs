...
pub(super) fn validate_function_return_mapping(
    ...
    if matches!(
        ...
        owned_unique_ptr_name(returns).is_some()
            && find_resource_type_in_modules(modules, &function.returns).is_some()
    }) {
        return Err(format!(
            "error[cpp.ownership.handle_result]: function `{}` classifies `{canonical}` as `{}` but must return a package-owned std::unique_ptr resource",
            function.name,

...
fn render_native_boundary_metadata(manifest: &CAbiBindingManifest) -> Result<String, String> {
...
fn render_rust_adapter_cargo(manifest: &CAbiBindingManifest) -> String {
    ...
    if let Some(extension) = &manifest.package.rust_extension {
        for (name, version) in &extension.dependencies {
            cargo.push_str(&format!("{name} = {version:?}\n"));
        }
    }
    if !manifest.c_metadata.sources.is_empty() || uses_pkg_config {
        cargo.push_str("\n[build-dependencies]\n");
        if !manifest.c_metadata.sources.is_empty() {
            cargo.push_str(&format!("cc = \"={CC_VERSION}\"\n"));
        }
        if uses_pkg_config {
            cargo.push_str("pkg-config = \"=0.3.33\"\n");
        }
    }
...
fn render_c_build(metadata: &CMetadata) -> String {
...
fn is_cpp_adapter_source(source: &str) -> bool {
    Path::new(source)
        .extension()
...
fn render_rust_ffi_and_adapter(
    manifest: &CAbiBindingManifest,
    symbols: &BTreeMap<&str, &CSymbol>,
) -> Result<String, String> {
    let types = binding_types(manifest);
    let dispatcher_ty = first_type(manifest)?;
    let dispatcher_record = symbols
        .get(dispatcher_ty.c_symbol.as_str())
        .copied()
        .ok_or_else(|| format!("unknown C record `{}`", dispatcher_ty.c_symbol))?;

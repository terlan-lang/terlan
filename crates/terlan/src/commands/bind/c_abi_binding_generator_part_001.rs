...
pub(super) struct CAbiBindingGenerationSummary {
...
pub(super) fn generate_c_abi_bindings(
    manifest_path: &Path,
    out_dir: &Path,
    ...
    for module in &manifest.modules {
        let module_source = crate::terlan_syntax::format_source_module(&render_module_source(
            module,
        ))
        .map_err(|error| {
            format!(
                "failed to format generated Terlan module `{}`: {}",
    ...
    write_file(
        ...
        &render_rust_ffi_and_adapter(&manifest, &symbols)?,
    )?;
    write_file(
        &out_dir.join("native/rust/src/bin/native_boundary_helper.rs"),
        &render_native_helper(&manifest, &symbols)?,
    )?;
    write_file(

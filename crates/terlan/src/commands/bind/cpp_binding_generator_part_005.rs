...
fn render_consumer_test(module: &NativeBindingModule) -> Result<Option<String>, String> {
    ...
    let mut exception_checks = String::new();
    for function in module
        .functions
        .iter()
        .filter(|function| function.role == NativeFunctionRole::ExceptionMethod)
    {
        ...
        exception_steps.push_str(&format!(
            "    let contained_{} = {}(boundary);\n",
            function.name, function.name
        ));
        exception_checks.push_str(&format!(
            " and std.core.Result.is_ok(contained_{})",
            function.name
        ));
    }
...
fn role_name(role: NativeFunctionRole) -> &'static str {
    match role {
...
fn render_args(args: &[NativeBindingArg]) -> String {
    args.iter()
...
fn lower_type_name(value: &str) -> String {
...
fn blocking_policy_name(policy: &NativeBlockingPolicy) -> &'static str {
    match policy {
...
fn resource_policy_name(policy: &NativeResourcePolicy) -> &'static str {
    match policy {
...
fn validate_cpp_identifier_path(kind: &str, value: &str) -> Result<(), String> {
    if value.split("::").all(is_identifier_segment) {
        Ok(())
    } else {
...
fn consumer_test_path(module: &str) -> PathBuf {

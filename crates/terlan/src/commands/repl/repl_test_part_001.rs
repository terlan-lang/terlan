...
use super::event::render_repl_json_event;
use super::source::{
    expression_parse_error_blocks_declaration_fallback, parse_repl_declaration, repl_load_sources,
};
use super::{
    evaluate_repl_prompt_inputs, evaluate_repl_prompt_inputs_with_publication,
    interactive_repl_line_break, is_repl_debug_command, is_repl_help_args, parse_repl_command_args,
    parse_repl_value_binding, repl_debug_reserved_fields, repl_debug_reserved_message,
    repl_expression_module_source, repl_expression_with_bindings, repl_json_field,
    run_repl_expression_in_session_with_output, validate_repl_seed_target_evidence,
    ReplCompilerService, ReplValueBinding,
};
...
fn publish_repl_expression_generation(
...
#[test]
fn repl_rejects_local_constants_but_accepts_constant_imports() {
...
#[test]
fn interactive_repl_line_break_returns_to_column_zero() {
...
#[test]
fn repl_command_args_reject_runtime_selection() {
...
/// Verifies REPL debug mode is a reserved command-surface option.
///
/// Inputs:
...
#[test]
fn repl_command_args_accept_debug_mode() {
...
/// Verifies the interactive REPL debug command is reserved explicitly.
///
/// Inputs:
...
#[test]
fn repl_debug_reserved_json_event_is_stable() {
    let value: serde_json::Value = serde_json::from_str(&render_repl_json_event(
        "error",
        &repl_debug_reserved_fields(),
        repl_debug_reserved_message(),
    ))
    .expect("valid repl debug JSON event");

    assert_eq!(value["kind"], "error");
    assert_eq!(value["code"], "debugger_unimplemented");
    assert_eq!(value["command"], "debug");
    assert_eq!(value["implemented"], false);
    assert_eq!(
        value["text"],
        "terlc debug is reserved for VM-owned debugger execution"
    );
}
...
#[test]
fn repl_command_args_reject_beam_runtime() {
...
///   evaluation.
#[test]
fn repl_seed_target_inference_rejects_js_source_evidence() {
...
#[test]
fn repl_seed_target_inference_rejects_explicit_js_profile_for_vm_source() {
...
///   native image.
#[test]
fn repl_expression_runs_arithmetic_through_vm_runtime() {
...
#[test]
fn value_lifecycle_public_constants_execute_in_repl_without_runtime_symbols() {
...
#[test]
fn repl_expression_vm_runtime_does_not_emit_erlang_artifacts() {
...
#[test]
fn repl_publish_expression_generation_records_hot_reload_event() {
...
#[test]
fn value_lifecycle_constant_change_publishes_one_new_repl_generation() {
    let mut code_server = VmCodeServer::default();
...
#[test]
fn repl_prompt_inputs_publish_vm_generations_for_expressions() {
...
#[test]
fn repl_assignment_expression_error_blocks_declaration_fallback() {
...
#[test]
fn repl_string_capture_errors_block_declaration_fallback() {
    for message in [
...
/// Verifies the REPL executes the same typed route capture used by tests.
///
/// Inputs:
/// - A route-shaped case expression with a typed integer capture and guard.
///
/// Output:
/// - The captured integer is available to the matching branch and evaluates
///   to `43` after arithmetic.
///
/// Transformation:
/// - Exercises the complete non-interactive REPL command pipeline through
///   parsing, type checking, CoreIR lowering, and VM evaluation.
#[test]
fn repl_prompt_inputs_execute_typed_string_capture_pattern() {
    let outputs = evaluate_repl_prompt_inputs(
        &[concat!(
            "case \"GET /users/42\" { ",
            "\"GET /users/${id: Int}\" where id > 0 -> id + 1; ",
            "_ -> 0 }."
        )
        .to_string()],
        DiagnosticFormat::Text {
    ...
    .expect("evaluate typed string capture in repl");

    assert_eq!(outputs, vec![vec!["43".to_string()]]);
}
...
#[test]
fn repl_prompt_inputs_reject_adjacent_string_captures_without_declaration_noise() {

...
use crate::commands::debug::{RESERVED_CODE as DEBUG_RESERVED_CODE, RESERVED_MESSAGE};
...
use source::{
    expression_parse_error_blocks_declaration_fallback, load_repl_seed_declarations,
    parse_repl_declaration, parse_repl_declaration_and_log, repl_declarations_to_source,
};
...
struct ReplCommandArgs {
    ...
    /// Whether the reserved debugger entry point was requested.
    debug: bool,
}
...
///
/// Transformation:
/// - Accepts one optional seed path and the reserved debugger switch. Runtime
///   selection is intentionally absent because the REPL always executes an
///   admitted Terlan native image.
fn parse_repl_command_args(args: &[String]) -> Result<ReplCommandArgs, String> {
...
///
/// Transformation:
/// - Keeps the reserved debugger command explicit so future debugger arguments
///   do not accidentally parse as ordinary REPL source.
fn is_repl_debug_command(command: &str) -> bool {
    command == ":debug"
}

/// Returns the reserved REPL debugger message.
///
/// Inputs:
/// - None.
///
/// Output:
/// - Stable text diagnostic shared by `terlc repl --debug` and `:debug`.
///
/// Transformation:
/// - Reuses the command-line debugger diagnostic code so REPL and editor
///   integrations see one debugger-unavailable contract.
fn repl_debug_reserved_message() -> &'static str {
    RESERVED_MESSAGE
}

/// Returns structured fields for the reserved REPL debugger event.
///
/// Inputs:
/// - None.
///
/// Output:
/// - JSON event fields describing the reserved debugger state.
///
/// Transformation:
/// - Converts the debugger command reservation into the existing REPL event
///   schema without introducing a separate protocol.
fn repl_debug_reserved_fields() -> Vec<event::ReplJsonField> {
    vec![
        repl_json_field("code", DEBUG_RESERVED_CODE),
        repl_json_field("command", "debug"),
        repl_json_field("implemented", false),
    ]
}

/// Emits the reserved REPL debugger event.
///
/// Inputs:
/// - `diagnostic_format`: text or JSON output mode selected by global options.
///
/// Output:
/// - Writes a stable REPL event.
///
/// Transformation:
/// - Routes REPL debugger entry points through the same unsupported-debugger
///   diagnostic used by the public `terlc debug` command.
fn emit_repl_debug_reserved(diagnostic_format: DiagnosticFormat) {
    emit_repl_event(
        diagnostic_format,
        "error",
...
/// RAII guard that restores terminal raw-mode state after one prompt read.
///
/// Inputs:
/// - Created only after `enable_raw_mode` succeeds.
///
/// Output:
/// - Raw mode is disabled when the guard is dropped.
///
/// Transformation:
/// - Keeps terminal-mode cleanup local to the interactive line editor, even
///   when a prompt exits through Enter, EOF, interrupt, or an IO error.
struct RawModeGuard;
...
fn read_interactive_repl_line(
    ...
    enable_raw_mode().map_err(|error| format!("failed to enable REPL raw mode: {error}"))?;
...
fn interactive_repl_line_break() -> &'static str {
    "\r\n"
}
...
pub(crate) fn run(cmd: CliCommand, state: CliState) -> ExitCode {
    match cmd.args.as_slice() {
        ...
        args => {
            ...
            if parsed.debug {
                emit_repl_debug_reserved(state.diagnostic_format);

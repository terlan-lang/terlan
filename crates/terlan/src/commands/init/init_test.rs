...
///
/// Transformation:
/// - Delegates to the shared test filesystem helper with the init namespace.
fn temp_dir(name: &str) -> PathBuf {
    test_fs::temp_path("init", name)
}
...
fn write_project_web_profile_template_declaration_resolves() {
    ...
    let module = crate::terlan_syntax::parse_module_as_syntax_output(&http_source)
        .expect("parse http module");
    let collected =

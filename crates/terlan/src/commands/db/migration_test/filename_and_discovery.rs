...
use super::super::migration::{
    discover_migration_files, duplicate_migration_id_message, load_migration_file,
    load_migration_files, migration_engine_inputs, migration_status, parse_migration_file_name,
    pending_migration_engine_inputs, AppliedMigration, LoadedMigration, MigrationDiagnostic,
    MigrationFileName, MigrationStatusEntry, MigrationStatusState,
};
...
pub(super) fn migration_file_inventory(
    file_names: &[&str],
    ...
    for pair in parsed.windows(2) {
        if pair[0].version == pair[1].version {
            return Err(MigrationDiagnostic {
                line: 1,
                message: duplicate_migration_id_message(&pair[1].version),
            });
        }
    }
...
pub(super) fn pending_migration_status(loaded: &[LoadedMigration]) -> Vec<MigrationStatusEntry> {
    migration_status(loaded, &[])
}

/// Builds local migrations with a history gap before a later applied row.
pub(super) fn out_of_order_history_fixture(
    name: &str,
) -> (PathBuf, Vec<LoadedMigration>, Vec<AppliedMigration>) {
    let directory = temp_migration_dir(name);
    for (filename, sql) in [
        (
        ...
        write_file(&directory, filename, &format!("-- +terlan Up\n{sql}\n"));
    }
    ...
    let applied = [0, 2]
        .into_iter()
        .map(|index| AppliedMigration {
            version: loaded[index].file.parsed.version.clone(),
            name: loaded[index].file.parsed.name.clone(),
            checksum: loaded[index].checksum.clone(),
            applied_at: "2026-06-19T12:30:00.000000Z".to_string(),
        })
        .collect();
    (directory, loaded, applied)
}
...
#[test]
pub(super) fn parse_migration_file_name_accepts_timestamped_sql_name() {
    assert_eq!(
...
#[test]
pub(super) fn parse_migration_file_name_rejects_non_sql_extension() {
    assert_eq!(
...
#[test]
pub(super) fn parse_migration_file_name_rejects_missing_separator() {
    assert_eq!(
...
#[test]
pub(super) fn parse_migration_file_name_rejects_invalid_timestamp() {
    assert_eq!(
...
#[test]
pub(super) fn parse_migration_file_name_rejects_invalid_name() {
    assert_eq!(
...
///   execution exists.
#[test]
pub(super) fn migration_file_inventory_sorts_by_timestamp() {
...
#[test]
pub(super) fn migration_file_inventory_rejects_duplicate_timestamps() {
    assert_eq!(
        ...
        Err(MigrationDiagnostic {
            line: 1,
            message: "error[db.migration.duplicate_id]: Migration id `20260619123000` is used by multiple local files.".to_string(),
        })
    );
}
...
#[test]
pub(super) fn discover_migration_files_sorts_regular_files() {
...
///   execution.
#[test]
pub(super) fn discover_migration_files_ignores_subdirectories() {
...
///   ignored.
#[test]
pub(super) fn discover_migration_files_rejects_invalid_filename() {
...
#[test]
pub(super) fn discover_migration_files_rejects_duplicate_timestamps() {
    ...
    assert_eq!(
        diagnostic.message,
        "error[db.migration.duplicate_id]: Migration id `20260619123000` is used by multiple local files."
    );
...
///   execution.
#[test]
pub(super) fn load_migration_files_parses_discovered_sources() {
...
///   migration execution.
#[test]
pub(super) fn migration_engine_inputs_preserve_sections_and_metadata() {
...
#[test]
pub(super) fn load_migration_file_reports_parser_error_with_path() {
...
///   comparison exists.
#[test]
pub(super) fn pending_migration_status_marks_loaded_migrations_pending() {
...
///
/// Inputs:
/// - Temporary directory containing applied, pending, and checksum-mismatched local
///   migrations.
/// - In-memory applied history containing applied, missing, and checksum-mismatched
///   rows.
///
/// Output:
/// - Test passes when status rows are sorted by version and classified as
///   applied, pending, missing, or checksum-mismatched.
///
/// Transformation:
/// - Locks migration history comparison semantics before database history
///   loading is implemented.
#[test]
pub(super) fn migration_status_classifies_applied_pending_missing_and_checksum_mismatch() {
    ...
    let statuses = migration_status(
        ...
        &[
            ...
            AppliedMigration {
                ...
                applied_at: "2026-06-19T12:50:00.000000Z".to_string(),
            },
            AppliedMigration {
                version: "20260619130000".to_string(),
                name: "add_name".to_string(),
                checksum: "1".repeat(64),
                applied_at: "2026-06-19T13:00:00.000000Z".to_string(),
            },
        ],
    );
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("20260619130000", MigrationStatusState::ChecksumMismatch,),
        ]
    );
    assert_eq!(statuses[0].state.label(), "applied");
    assert_eq!(
        statuses[0].applied_at.as_deref(),
        Some("2026-06-19T12:30:00.000000Z")
    );
    assert_eq!(statuses[1].applied_at, None);
    assert_eq!(statuses[2].name, "drop_unused");
    assert_eq!(
        statuses[2].applied_at.as_deref(),
        Some("2026-06-19T12:50:00.000000Z")
    );
    assert_eq!(statuses[3].state.label(), "checksum-mismatch");
    assert_eq!(statuses[3].checksum, loaded[2].checksum);
    assert_eq!(
        statuses[3].applied_at.as_deref(),
        Some("2026-06-19T13:00:00.000000Z")
    );

    remove_dir(&directory);
}

/// Distinguishes applied-name drift from checksum drift.
#[test]
pub(super) fn migration_status_classifies_name_mismatch() {
    let directory = temp_migration_dir("classifies_name_mismatch");
    write_file(
        &directory,
        "20260619123000_create_users.sql",
        "-- +terlan Up\nCREATE TABLE users (id BIGSERIAL PRIMARY KEY);\n",
    ...
    assert_eq!(statuses[0].state, MigrationStatusState::NameMismatch);
    assert_eq!(statuses[0].state.label(), "name-mismatch");
    remove_dir(&directory);
}

/// Classifies a local gap before a later applied migration as out of order.
#[test]
pub(super) fn migration_status_classifies_gap_before_applied_history_as_out_of_order() {
    let (directory, loaded, applied) =
        out_of_order_history_fixture("classifies_out_of_order_status");

    let statuses = migration_status(&loaded, &applied);

    assert_eq!(statuses[0].state, MigrationStatusState::Applied);
    assert_eq!(statuses[1].state, MigrationStatusState::OutOfOrder);
    assert_eq!(statuses[1].state.label(), "out-of-order");
    assert_eq!(statuses[2].state, MigrationStatusState::Applied);
    remove_dir(&directory);
}
...
#[test]
pub(super) fn pending_migration_engine_inputs_selects_unapplied_migrations() {
...
/// Rejects applying a local migration after a later migration is recorded.
#[test]
pub(super) fn pending_migration_engine_inputs_rejects_out_of_order_history_gap() {
    let (directory, loaded, applied) =
        out_of_order_history_fixture("rejects_out_of_order_history_gap");

    let result = pending_migration_engine_inputs(&loaded, &applied);

    assert_eq!(
        result,
        Err(MigrationDiagnostic {
            line: 1,
            message: "error[db.migration.out_of_order]: Database history contains a later applied migration before local migration `20260619124500`.".to_string(),
        })
    );
    remove_dir(&directory);
}
...
#[test]
pub(super) fn pending_migration_engine_inputs_rejects_missing_history() {
    assert_eq!(
        ...
        Err(MigrationDiagnostic {
            line: 1,
            message: "error[db.migration.file_missing]: Database history migration `20260619123000` has no local migration file.".to_string(),
        })
    );
}

/// Rejects execution planning when an applied checksum differs locally.
///
/// Inputs:
/// - One local migration.
...
///
/// Output:
/// - Test passes when execution planning returns a checksum-mismatch
///   diagnostic.
///
/// Transformation:
...
#[test]
pub(super) fn pending_migration_engine_inputs_rejects_checksum_mismatch() {
    let directory = temp_migration_dir("rejects_checksum_mismatch");
    write_file(
        &directory,
        "20260619123000_create_users.sql",
        "-- +terlan Up\nCREATE TABLE users (id BIGSERIAL PRIMARY KEY);\n",
    ...
    assert_eq!(
        ...
        Err(MigrationDiagnostic {
            line: 1,
            message: "error[db.migration.checksum_mismatch]: Local migration `20260619123000` does not match its applied checksum.".to_string(),
        })
    );

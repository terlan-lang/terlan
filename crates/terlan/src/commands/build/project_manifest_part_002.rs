...
fn validate_script_path(script_path: &str, path: &Path, line_no: usize) -> Result<(), String> {
    ...
    if candidate.extension().and_then(|ext| ext.to_str()) != Some("terl") {
        return Err(format!(
            "{}:{}: [scripts] path `{script_path}` must point to a .terl file",
            path.display(),
            line_no
        ));
    }
...
fn parse_git_dependency_source(
    fields: &BTreeMap<String, ProjectManifestInlineValue>,
    path: &Path,
    line_no: usize,
) -> Result<ProjectDependencySource, String> {

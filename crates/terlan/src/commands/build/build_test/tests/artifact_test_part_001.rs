...
///
/// Output:
/// - Test passes when the checked compiler interface is emitted without Vm or
///   VM output; atom values remain outside the scalar native ABI.
///
/// Transformation:
/// - Runs normal VM build emission for an atom alias source so the current
///   artifact path keeps accepting language-level atom values.
#[test]
fn build_command_builds_atom_alias_for_vm() {
    ...
    assert!(!out_dir.join("vm/atom_manifest_single_file.tvm").exists());
    assert!(
...
#[test]
fn build_command_emits_terlan_vm_artifact_without_erlang_or_beam() {
    ...
    assert!(image.symbols().any(|symbol| {
        symbol.name().is_ok_and(|name| {
            name == "terlan_native_dispatch_v2" || name == "_terlan_native_dispatch_v2"
        })
    }));
...
#[test]
fn build_command_lowers_if_and_unary_expressions_into_native_ir() {
...
#[test]
fn value_lifecycle_constants_lower_into_native_aot_without_runtime_storage() {
...
#[test]
fn build_command_defaults_to_terlan_vm_artifact_without_erlang_or_beam() {
...
#[test]
fn build_command_defaults_project_directory_to_terlan_vm_artifacts() {

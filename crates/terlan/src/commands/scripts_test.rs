...
fn discover_project_scripts_lists_convention_scripts() {
    ...
    let script = root.join("scripts").join("SeedDatabase.terl");
    write_script(&script);
...
fn resolve_project_script_accepts_configured_alias() {
    ...
    let script = root.join("scripts").join("SmokeHttp.terl");
    write_script(&script);
    fs::write(
        root.join("terlan.toml"),
        "\
...
/// Verifies configured aliases fail fast when they are not runnable.
#[test]
fn discover_project_scripts_rejects_configured_script_without_main() {
    let root = shared_temp_dir("scripts_command", "rejects_configured_without_main");
...
",
    ...
    assert!(message.contains("must define `pub main`"), "{message}");
}
...
fn discover_project_scripts_rejects_configured_alias_shadowing_discovered_script() {
    ...
    write_script(&root.join("scripts").join("SeedDatabase.terl"));
    write_script(&root.join("scripts").join("Other.terl"));
    fs::write(
        root.join("terlan.toml"),
        "\
[package]
...
[scripts]
seed_database = \"scripts/Other.terl\"
",
    )
    .expect("write manifest");
    ...
    assert!(message.contains("Other.terl"), "{message}");
    assert!(message.contains("SeedDatabase.terl"), "{message}");
}

...
pub(super) const SAMPLE_ACCOUNT_CREDENTIALS_JSON: &str = r#"{"id":"id","key_pkcs8":"MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgJVWC_QzOTCS5vtsJp2IG-UDc8cdDfeoKtxSZxaznM-mhRANCAAQenCPoGgPFTdPJ7VLLKt56RxPlYT1wNXnHc54PEyBg3LxKaH0-sJkX0mL8LyPEdsfL_Oz4TxHkWLJGrXVtNhfH","directory":"https://acme-v02.api.letsencrypt.org/directory"}"#;
...
pub(super) fn write_self_signed_cert_pair(dir: &Path) {
...
pub(super) fn sample_account_credentials() -> instant_acme::AccountCredentials {
...
#[test]
pub(super) fn runtime_tls_config_returns_none_for_plain_http_package() {
...
#[test]
pub(super) fn runtime_tls_config_rejects_invalid_manual_tls_files() {
...
#[test]
pub(super) fn runtime_tls_config_accepts_manual_certificate_tls() {
...
key = "certs/dev-key.pem""#,
    ...
    let config = runtime_tls_config(&web_root)
        .expect("manual TLS config should load")
        .expect("manual TLS config");
    assert_eq!(
...
#[test]
pub(super) fn runtime_tls_config_accepts_internal_local_tls() {
...
///   the ACME network.
#[test]
pub(super) fn acme_runtime_plan_defaults_to_lets_encrypt_production() {
    ...
    assert_eq!(
        ...
        dir.join(".terlan/tls/acme/renewal.json")
    );
    assert_eq!(
...
#[test]
pub(super) fn acme_runtime_plan_preserves_zerossl_fallback_provider() {
...
///   `instant-acme`.
#[test]
pub(super) fn acme_domain_identifiers_preserve_dns_names() {
    let identifiers =
...
#[test]
pub(super) fn acme_contact_strings_wrap_optional_email() {
    assert_eq!(
...
#[test]
pub(super) fn pending_http01_challenges_select_pending_http_challenges() {
...
#[test]
pub(super) fn pending_http01_challenges_skip_valid_authorizations() {
    let authorization = serde_json::from_str::<instant_acme::Authorization>(
        r#"{
          "status": "valid",
          "identifier": {"type": "dns", "value": "www.example.org"},
...
///   does not serve.
#[test]
pub(super) fn pending_http01_challenges_reject_missing_http01() {
    let authorization = serde_json::from_str::<instant_acme::Authorization>(
        r#"{
          "status": "pending",
          "identifier": {"type": "dns", "value": "www.example.org"},
...
#[test]
pub(super) fn generate_acme_csr_returns_der_and_private_key_pem() {
...
///
/// Output:
/// - Test passes when the issuance preflight returns a stable provider
///   diagnostic.
///
/// Transformation:
/// - Exercises the production issuer preflight without contacting an ACME
///   server or requiring an async runtime, preserving the rule that unsupported
///   provider-specific account requirements must fail before partial cache
///   writes.
#[test]
pub(super) fn issue_acme_certificate_cache_rejects_zerossl_before_network() {
    ...
    let message = issue_acme_certificate_cache_preflight(&plan)
...
/// Verifies unsupported ACME providers fail before cached material is loaded.
///
/// Inputs:
/// - Auto TLS project metadata using ZeroSSL as the primary provider.
/// - A valid-looking local certificate cache.
///
/// Output:
/// - Stable `error[serve_tls]` diagnostic for unsupported external account
///   binding, even though the cache exists.
///
/// Transformation:
/// - Prevents unsupported provider configuration from being activated by
///   project-local cache state that was created by an earlier or external
///   process.
#[test]
pub(super) fn runtime_tls_config_rejects_zerossl_primary_before_cache_loading() {
    let dir = temp_dir("auto_runtime_zerossl");
    let web_root = dir.join("_build/web");
    write_valid_package(&web_root);
    write_project_manifest(
        &dir.join("terlan.toml"),
...
primary_provider = "zerossl""#,
    ...
    let message = match runtime_tls_config(&web_root) {
        Ok(_) => panic!("ZeroSSL primary should fail before cache loading"),
        Err(message) => message,
    };
...
#[test]
pub(super) fn acme_account_credentials_round_trip_through_cache() {
...
///   automatic renewal.
#[test]
pub(super) fn acme_account_credentials_cache_reports_invalid_json() {
...
///   probes.
#[test]
pub(super) fn acme_http01_challenge_cache_writes_valid_token() {
...
#[test]
pub(super) fn acme_http01_challenge_cache_rejects_invalid_token() {

...
#[test]
fn vm_stream_request_executes_managed_template_html_handler() {
...
#[test]
fn vm_stream_request_executes_external_template_render_plan() {
    clear_vm_handler_module_cache_for_test();
...
#[test]
fn vm_stream_request_executes_complete_typed_template_matrix() {
    clear_vm_handler_module_cache_for_test();
...
#[test]
fn vm_stream_session_state_and_lifecycle_are_vm_owned() {
    clear_vm_handler_module_cache_for_test();
...
"#,
    ...
    assert!(created.contains("set-cookie: terlan_session=s1; Path=/; HttpOnly; SameSite=Lax\r\n"), "{created}");
    ...
    assert!(rotated.contains("set-cookie: terlan_session=s2; Path=/; HttpOnly; SameSite=Lax\r\n"), "{rotated}");
    ...
    assert!(expired.contains("set-cookie: terlan_session=;"), "{expired}");
    ...
    assert!(replacement.contains("set-cookie: terlan_session=s3; Path=/; HttpOnly; SameSite=Lax\r\n"), "{replacement}");

    fs::remove_dir_all(dir).expect("cleanup");
    clear_vm_handler_module_cache_for_test();
}
include!("serve_test/package_validation_test.rs");

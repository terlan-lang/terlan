...
///   state.
fn validate_acme_provider_supported(plan: &AcmeRuntimePlan) -> Result<(), String> {
...
fn validate_acme_certificate_cache_age(
...
fn validate_acme_certificate_cache_domains(
    plan: &AcmeRuntimePlan,
...
///   verification.
fn validate_acme_certificate_cache_validity_window(
    plan: &AcmeRuntimePlan,
    certificates: &[CertificateDer<'static>],
    now: SystemTime,
) -> Result<(), String> {
...
fn webpki_unix_time(time: SystemTime) -> Result<UnixTime, String> {

...
/// Verifies source-level function-head pattern misses keep stable diagnostics.
///
/// Inputs:
...
///   the function-head mismatch metadata.
///
/// Transformation:
/// - Runs `terlc test` through the public VM test command so runtime clause
///   dispatch failures from source `.terl` tests are recorded as debugger-ready
///   diagnostics, not generic VM errors.
#[test]
fn run_test_reports_function_head_clause_pattern_miss_diagnostics() {
    let root = std::env::temp_dir().join(format!(
    ...
    assert!(
        message.contains("no function clause matched require_pair/1"),
        "{message}"
    );
    assert!(message.contains("event=pattern_head_failed"), "{message}");
    assert!(message.contains("function=require_pair/1"), "{message}");
    assert!(message.contains("clause_index=0"), "{message}");
    assert!(message.contains("guard_index=none"), "{message}");
    assert!(message.contains("pattern_index=0"), "{message}");
    assert!(message.contains("pattern_kind=Tuple"), "{message}");
}

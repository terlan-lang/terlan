...
/// Reserved template slot populated from an element's nested children.
///
/// This belongs to the shared template contract rather than the static-site
/// command because validation and every renderer must agree on the same name.
...
pub(crate) struct SyntaxTemplateFrontendInput {
    ...
    pub(crate) metadata: crate::terlan_html::TemplateMetadata,
    pub(crate) span: Span,
    pub(crate) parsed: crate::terlan_html::HtmlTemplate,
}
...
pub(crate) fn collect_syntax_template_frontend_inputs(
    ...
    for declaration in &module.declarations {
        ...
        let parsed = match crate::terlan_html::parse_template(&source, &resolved_path) {
            Ok(parsed) => parsed,
            Err(diagnostics) => {
        ...
        let metadata = match crate::terlan_html::extract_template_metadata(&source, &resolved_path)
        {
...
/// Resolves a template source path relative to the importing source file.
fn resolve_import_path(base_dir: &Path, source: &str) -> PathBuf {
...
fn validate_syntax_template_frontend_param_metadata(
    name: &str,
    props: &[SyntaxTemplatePropOutput],
    params: &[crate::terlan_html::TemplateParamMetadata],
) -> Option<String> {
...
pub(crate) fn collect_syntax_template_inputs(
    module: &SyntaxModuleOutput,
    source_path: &Path,
) -> Result<BTreeMap<String, crate::terlan_html::HtmlTemplate>, terlan_runtime_abi::BoundaryError> {
    let collected = collect_syntax_template_frontend_inputs(module, source_path);
    if !collected.errors.is_empty() {
        return Err(terlan_runtime_abi::BoundaryError::message(

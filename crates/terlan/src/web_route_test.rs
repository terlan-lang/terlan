...
fn route_param_types_extracts_defaults_and_typed_captures() {
    ...
    assert_eq!(
        route_param_types("/repo/:name.json").expect("filename capture route"),
        vec![("name".to_string(), "String".to_string())]
    );
    assert_eq!(
        route_ambiguity_key("/repo/:name.json").expect("filename capture route"),
        "/repo/:.json"
    );
}
...
fn validate_route_pattern_rejects_unsupported_route_param_type() {
    ...
    assert!(error
        .to_string()
        .contains("unsupported route parameter type `UserId`"));
}
...
fn validate_route_pattern_rejects_non_binding_capture_names() {
    ...
    assert!(uppercase
        .to_string()
        .contains("invalid route parameter `:Id`"));
    assert!(underscore
        .to_string()
        .contains("invalid typed route parameter `{_id:Int}`"));
}

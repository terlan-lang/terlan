use super::imported_type_refs::{
    collect_syntax_imported_type_refs, collect_syntax_selected_type_refs,
};
use super::interface_render::normalize_type_text;
use super::shapes::collect_syntax_shape_signatures;
use super::*;
use crate::terlan_syntax::{parse_expr_as_syntax_output, SyntaxExprOutput};

pub(super) fn syntax_module_output_to_prepared_interface(
    module: &SyntaxModuleOutput,
) -> ModuleInterface {
    let inferred_pure_callables = infer_body_available_pure_callables(module);
    let mut public_types = HashSet::new();
    let mut private_types = HashSet::new();
    let mut opaque_types = HashSet::new();
    let type_params = collect_syntax_type_params(module);
    let type_bodies = collect_syntax_type_bodies(module);
    let struct_fields = collect_syntax_struct_fields(module);
    let shapes = collect_syntax_shape_signatures(module);
    let mut functions = HashMap::new();
    let mut function_overloads = HashMap::new();
    let mut constants = HashMap::new();
    let mut const_functions = HashMap::new();
    let mut expression_macros = HashMap::new();
    let mut valued_unions = HashMap::new();
    let mut associated_constants = HashMap::new();
    let traits = collect_syntax_trait_signatures(module);
    let trait_conformances = collect_syntax_trait_conformances(module);
    let constructors = collect_syntax_constructor_signatures(module);
    let imported_type_refs = collect_syntax_selected_type_refs(module);

    for declaration in &module.declarations {
        match &declaration.payload {
            SyntaxDeclarationPayload::Constant {
                name,
                annotation,
                value,
                is_public: true,
            } => {
                let value_text = interface_render::render_constant_expr(value);
                constants.insert(
                    name.clone(),
                    ConstantSignature {
                        name: name.clone(),
                        annotation: normalize_type_text(&annotation.text),
                        value: value.clone(),
                        fingerprint: interface_fingerprint(&value_text),
                        value_text,
                        docs: declaration.docs.clone(),
                    },
                );
            }
            SyntaxDeclarationPayload::ConstFunction {
                name,
                params,
                return_type,
                body,
                is_public: true,
            } => {
                let body_text = interface_render::render_constant_expr(body);
                let key = (name.clone(), params.len());
                const_functions.insert(
                    key,
                    ConstFunctionSignature {
                        name: name.clone(),
                        params: syntax_param_signatures(params),
                        return_type: normalize_type_text(&return_type.text),
                        body: body.clone(),
                        fingerprint: interface_fingerprint(&body_text),
                        body_text,
                        docs: declaration.docs.clone(),
                    },
                );
            }
            SyntaxDeclarationPayload::Type {
                name,
                is_public,
                is_opaque,
                representation,
                valued_arms,
                ..
            } => {
                if *is_public {
                    public_types.insert(name.clone());
                    if *is_opaque {
                        opaque_types.insert(name.clone());
                    }
                    if let Some(representation) = representation {
                        valued_unions.insert(
                            name.clone(),
                            ValuedUnionSignature {
                                name: name.clone(),
                                representation: normalize_type_text(&representation.text),
                                arms: valued_arms
                                    .iter()
                                    .map(|arm| {
                                        let value_text =
                                            interface_render::render_constant_expr(&arm.value);
                                        ValuedUnionArmSignature {
                                            name: arm.name.clone(),
                                            value: arm.value.clone(),
                                            fingerprint: interface_fingerprint(&value_text),
                                            value_text,
                                        }
                                    })
                                    .collect(),
                            },
                        );
                    }
                } else {
                    private_types.insert(name.clone());
                }
            }
            SyntaxDeclarationPayload::Struct {
                name, is_public, ..
            } => {
                if *is_public {
                    public_types.insert(name.clone());
                } else {
                    private_types.insert(name.clone());
                }
            }
            SyntaxDeclarationPayload::Function {
                name,
                generic_params,
                params,
                return_type,
                generic_bounds,
                is_public,
                is_macro,
                clauses,
            } => {
                let key = (name.clone(), params.len());
                if *is_public && *is_macro {
                    if let Some(template) = clauses.first().and_then(|clause| {
                        (clause.guard.is_none()
                            && clause.body.kind == SyntaxExprKind::Quote
                            && clause.body.children.len() == 1)
                            .then(|| clause.body.children[0].clone())
                    }) {
                        let template_text = interface_render::render_constant_expr(&template);
                        expression_macros.insert(
                            key.clone(),
                            ExpressionMacroSignature {
                                name: name.clone(),
                                params: syntax_param_signatures(params),
                                return_type: normalize_type_text(&return_type.text),
                                template,
                                fingerprint: interface_fingerprint(&template_text),
                                docs: declaration.docs.clone(),
                            },
                        );
                    }
                }
                let signature = FunctionSignature {
                    name: name.clone(),
                    generic_params: generic_params.clone(),
                    params: qualify_param_signatures(
                        syntax_param_signatures(params),
                        &imported_type_refs,
                    ),
                    return_type: qualify_syntax_type_text(
                        &normalize_type_text(&return_type.text),
                        &imported_type_refs,
                    ),
                    generic_bounds: generic_bounds.clone(),
                    receiver_method: false,
                    receiver_mutable: false,
                    public: *is_public,
                    pure: declaration_has_marker_annotation(declaration, &["pure"])
                        || inferred_pure_callables.contains(&key),
                    docs: declaration.docs.clone(),
                };
                functions
                    .entry(key.clone())
                    .or_insert_with(|| signature.clone());
                function_overloads
                    .entry(key)
                    .or_insert_with(Vec::new)
                    .push(signature);
            }
            SyntaxDeclarationPayload::Method {
                receiver,
                name,
                generic_params,
                params,
                return_type,
                generic_bounds,
                is_public,
                ..
            } => {
                let signature_params = qualify_param_signatures(
                    syntax_method_param_signatures(receiver, params),
                    &imported_type_refs,
                );
                let key = (name.clone(), signature_params.len());
                let signature = FunctionSignature {
                    name: name.clone(),
                    generic_params: generic_params.clone(),
                    params: signature_params,
                    return_type: qualify_syntax_type_text(
                        &normalize_type_text(&return_type.text),
                        &imported_type_refs,
                    ),
                    generic_bounds: generic_bounds.clone(),
                    receiver_method: true,
                    receiver_mutable: receiver.is_mutable,
                    public: *is_public,
                    pure: declaration_has_marker_annotation(declaration, &["pure"])
                        || inferred_pure_callables.contains(&key),
                    docs: declaration.docs.clone(),
                };
                functions
                    .entry(key.clone())
                    .or_insert_with(|| signature.clone());
                function_overloads
                    .entry(key)
                    .or_insert_with(Vec::new)
                    .push(signature);
            }
            SyntaxDeclarationPayload::TraitImpl {
                trait_ref,
                for_type,
                constants: impl_constants,
                is_negative: false,
                is_public: true,
                ..
            } => {
                let trait_name = trait_ref.text.split('[').next().unwrap_or(&trait_ref.text);
                let owner = if trait_ref.text.contains('[') {
                    trait_ref.text.clone()
                } else {
                    format!("{}[{}]", trait_ref.text, for_type.text)
                };
                for constant in impl_constants {
                    let Some(contract) = traits
                        .get(trait_name)
                        .and_then(|signature| signature.constants.get(&constant.name))
                    else {
                        continue;
                    };
                    let value_text = interface_render::render_constant_expr(&constant.value);
                    let name = format!("{owner}.{}", constant.name);
                    associated_constants.insert(
                        name.clone(),
                        ConstantSignature {
                            name,
                            annotation: contract.annotation.clone(),
                            value: constant.value.clone(),
                            fingerprint: interface_fingerprint(&value_text),
                            value_text,
                            docs: declaration.docs.clone(),
                        },
                    );
                }
            }
            SyntaxDeclarationPayload::Config { name, text, .. } if name == "native" => {
                for native_sig in extract_native_function_signatures(text) {
                    let key = (native_sig.name.clone(), native_sig.arity);
                    let signature = FunctionSignature {
                        name: native_sig.name.clone(),
                        generic_params: Vec::new(),
                        params: native_sig
                            .params
                            .iter()
                            .map(|(param_name, annotation)| ParamSignature {
                                name: param_name.clone(),
                                annotation: annotation.clone(),
                                is_mutable: false,
                                default: None,
                                default_text: None,
                            })
                            .collect(),
                        return_type: native_sig.return_type.clone(),
                        generic_bounds: Vec::new(),
                        receiver_method: false,
                        receiver_mutable: false,
                        public: true,
                        pure: false,
                        docs: declaration.docs.clone(),
                    };
                    functions
                        .entry(key.clone())
                        .or_insert_with(|| signature.clone());
                    function_overloads
                        .entry(key)
                        .or_insert_with(Vec::new)
                        .push(signature);
                }
            }
            SyntaxDeclarationPayload::Import { .. }
            | SyntaxDeclarationPayload::Export { .. }
            | SyntaxDeclarationPayload::Constant { .. }
            | SyntaxDeclarationPayload::ConstFunction { .. }
            | SyntaxDeclarationPayload::Constructor { .. }
            | SyntaxDeclarationPayload::Trait { .. }
            | SyntaxDeclarationPayload::TraitImpl { .. }
            | SyntaxDeclarationPayload::AnnotationSchema { .. }
            | SyntaxDeclarationPayload::Template { .. }
            | SyntaxDeclarationPayload::Config { .. }
            | SyntaxDeclarationPayload::Raw { .. } => {}
        }
    }

    ModuleInterface {
        module: module.module_name.clone(),
        docs: module.docs.clone(),
        public_types,
        private_types,
        opaque_types,
        type_params,
        type_bodies,
        struct_fields,
        type_docs: collect_syntax_type_docs(module),
        shapes,
        traits,
        trait_conformances,
        constructors,
        functions,
        function_overloads,
        constants,
        const_functions,
        expression_macros,
        valued_unions,
        associated_constants,
    }
}

pub(super) fn interface_fingerprint(text: &str) -> String {
    let mut hash = 0xcbf29ce484222325u64;
    for byte in text.as_bytes() {
        hash ^= u64::from(*byte);
        hash = hash.wrapping_mul(0x100000001b3);
    }
    format!("fnv1a64:{hash:016x}")
}

/// Collects public and private trait conformance facts from syntax output.
///
/// Inputs:
/// - `module`: syntax-output module containing type, struct, or explicit impl
///   declarations.
///
/// Output:
/// - Sorted, deduplicated trait conformance summaries.
///
/// Transformation:
/// - Converts declaration-site `implements` and explicit `impl Trait[...] for
///   Type` declarations into stable interface metadata so imported modules can
///   expose conformance without exposing implementation bodies. Struct
///   `includes` is intentionally excluded because it expands struct shape, not
///   trait conformance.
pub(super) fn collect_syntax_trait_conformances(
    module: &SyntaxModuleOutput,
) -> Vec<TraitConformanceSignature> {
    let mut conformances = Vec::new();
    let imported_type_refs = collect_syntax_imported_type_refs(module);

    for declaration in &module.declarations {
        match &declaration.payload {
            SyntaxDeclarationPayload::Type {
                name,
                implements,
                is_public,
                ..
            }
            | SyntaxDeclarationPayload::Struct {
                name,
                implements,
                is_public,
                ..
            } => {
                conformances.extend(
                    implements
                        .iter()
                        .map(|trait_ref| TraitConformanceSignature {
                            trait_ref: qualify_syntax_type_text(
                                &normalize_type_text(&trait_ref.text),
                                &imported_type_refs,
                            ),
                            generic_params: Vec::new(),
                            for_type: qualify_syntax_type_text(
                                &normalize_type_text(name),
                                &imported_type_refs,
                            ),
                            is_negative: false,
                            source: TraitConformanceSource::Implements,
                            public: *is_public,
                        }),
                );
            }
            _ => {}
        }

        if let SyntaxDeclarationPayload::TraitImpl {
            trait_ref,
            generic_params,
            for_type,
            is_negative,
            is_public,
            ..
        } = &declaration.payload
        {
            conformances.push(TraitConformanceSignature {
                trait_ref: qualify_syntax_type_text(
                    &normalize_type_text(&trait_ref.text),
                    &imported_type_refs,
                ),
                generic_params: generic_params.clone(),
                for_type: qualify_syntax_type_text(
                    &normalize_type_text(&for_type.text),
                    &imported_type_refs,
                ),
                is_negative: *is_negative,
                source: TraitConformanceSource::ExplicitImpl,
                public: *is_public,
            });
        }
    }

    conformances.sort();
    conformances.dedup();
    conformances
}

/// Collects public trait signatures from syntax output.
///
/// Inputs: syntax-output module. Output: map from trait name to signature.
/// Transformation: ignores private traits and converts public trait methods to
/// span-free interface signatures.
pub(super) fn collect_syntax_trait_signatures(
    module: &SyntaxModuleOutput,
) -> HashMap<String, TraitSignature> {
    let mut traits = HashMap::new();

    for declaration in &module.declarations {
        let SyntaxDeclarationPayload::Trait {
            name,
            params,
            super_traits,
            is_public,
            methods,
            constants,
        } = &declaration.payload
        else {
            continue;
        };

        if !*is_public {
            continue;
        }

        let mut method_signatures = HashMap::new();
        for method in methods {
            method_signatures.insert(
                method.name.clone(),
                TraitMethodSignature {
                    generic_params: method.generic_params.clone(),
                    params: method
                        .params
                        .iter()
                        .map(|param| ParamSignature {
                            name: param.name.clone(),
                            annotation: normalize_type_text(&param.annotation.text),
                            is_mutable: param.is_mutable,
                            default: canonical_param_default(param),
                            default_text: param.default_text.clone(),
                        })
                        .collect(),
                    return_type: normalize_type_text(&method.return_type.text),
                    generic_bounds: method.generic_bounds.clone(),
                    has_default: method.default_body.is_some(),
                    pure: method.is_pure,
                    docs: method.docs.clone(),
                },
            );
        }

        let constant_signatures = constants
            .iter()
            .map(|constant| {
                (
                    constant.name.clone(),
                    TraitConstantSignature {
                        annotation: normalize_type_text(&constant.annotation.text),
                        default_text: constant
                            .default
                            .as_ref()
                            .map(interface_render::render_constant_expr),
                        default: constant.default.clone(),
                        docs: constant.docs.clone(),
                    },
                )
            })
            .collect();

        traits.insert(
            name.clone(),
            TraitSignature {
                name: name.clone(),
                type_params: params.clone(),
                super_traits: super_traits.clone(),
                methods: method_signatures,
                constants: constant_signatures,
                docs: declaration.docs.clone(),
            },
        );
    }

    traits
}

/// Collects constructor signatures from syntax output.
///
/// Inputs: syntax-output module. Output: constructor signatures keyed by type
/// name. Transformation: converts constructor clauses into fixed/vararg
/// signature metadata and computes minimum arity from defaults.
pub(super) fn collect_syntax_constructor_signatures(
    module: &SyntaxModuleOutput,
) -> HashMap<String, Vec<ConstructorSignature>> {
    let mut constructors = HashMap::new();

    for declaration in &module.declarations {
        let SyntaxDeclarationPayload::Constructor {
            name,
            params: type_params,
            clauses,
            is_public,
            ..
        } = &declaration.payload
        else {
            continue;
        };

        let signatures = clauses
            .iter()
            .map(|clause| {
                let params = clause
                    .params
                    .iter()
                    .filter(|param| !param.is_varargs)
                    .map(|param| ParamSignature {
                        name: param.name.clone(),
                        annotation: param.annotation.text.clone(),
                        is_mutable: false,
                        default: param
                            .default_text
                            .as_deref()
                            .and_then(|text| parse_expr_as_syntax_output(text).ok())
                            .or_else(|| param.default.clone()),
                        default_text: param.default_text.clone(),
                    })
                    .collect::<Vec<_>>();
                let vararg = clause
                    .params
                    .iter()
                    .find(|param| param.is_varargs)
                    .map(|param| ParamSignature {
                        name: param.name.clone(),
                        annotation: param.annotation.text.clone(),
                        is_mutable: false,
                        default: param
                            .default_text
                            .as_deref()
                            .and_then(|text| parse_expr_as_syntax_output(text).ok())
                            .or_else(|| param.default.clone()),
                        default_text: param.default_text.clone(),
                    });
                let min_arity = clause
                    .params
                    .iter()
                    .filter(|param| !param.is_varargs && !param.has_default)
                    .count();
                ConstructorSignature {
                    name: name.clone(),
                    type_params: type_params.clone(),
                    params,
                    vararg,
                    return_type: clause.return_type.text.clone(),
                    body: clause.body_text.clone(),
                    min_arity,
                    varargs: clause.params.iter().any(|param| param.is_varargs),
                    public: *is_public,
                    docs: declaration.docs.clone(),
                }
            })
            .collect::<Vec<_>>();

        constructors.insert(name.clone(), signatures);
    }

    constructors
}

/// Collects public type and struct parameters from syntax output.
///
/// Inputs: syntax-output module. Output: public type name to type parameters.
/// Transformation: ignores private declarations and preserves source parameter
/// order, including structural implication text on generic structs.
pub(super) fn collect_syntax_type_params(
    module: &SyntaxModuleOutput,
) -> HashMap<String, Vec<String>> {
    let mut params = HashMap::new();
    for declaration in &module.declarations {
        let entry = match &declaration.payload {
            SyntaxDeclarationPayload::Type {
                name,
                params: type_params,
                is_public: true,
                ..
            } => Some((name, type_params)),
            SyntaxDeclarationPayload::Struct {
                name,
                generic_params,
                is_public: true,
                ..
            } => Some((name, generic_params)),
            _ => None,
        };
        if let Some((name, type_params)) = entry {
            params.insert(name.clone(), type_params.clone());
        }
    }
    params
}

/// Collects public non-opaque type bodies.
///
/// Inputs: syntax-output module. Output: public type name to variant text.
/// Transformation: excludes opaque/private/empty bodies and preserves variant
/// order for imported constructor/type checks.
pub(super) fn collect_syntax_type_bodies(
    module: &SyntaxModuleOutput,
) -> HashMap<String, Vec<String>> {
    let mut bodies = HashMap::new();
    for declaration in &module.declarations {
        if let SyntaxDeclarationPayload::Type {
            name,
            variants,
            is_public,
            is_opaque,
            ..
        } = &declaration.payload
        {
            if *is_public && !*is_opaque && !variants.is_empty() {
                bodies.insert(
                    name.clone(),
                    variants
                        .iter()
                        .map(|variant| variant.text.clone())
                        .collect(),
                );
            }
        }
    }
    bodies
}

/// Collects public struct fields for interface summaries.
///
/// Inputs:
/// - `module`: syntax-output module containing source declarations.
///
/// Output:
/// - Map from public struct name to ordered field signatures.
///
/// Transformation:
/// - Converts public syntax-output struct fields into span-free interface
///   metadata so downstream modules can expand `struct Child includes Parent`
///   across module boundaries.
pub(super) fn collect_syntax_struct_fields(
    module: &SyntaxModuleOutput,
) -> HashMap<String, Vec<StructFieldSignature>> {
    let mut structs = HashMap::new();
    for declaration in &module.declarations {
        let SyntaxDeclarationPayload::Struct {
            name,
            fields,
            is_public,
            ..
        } = &declaration.payload
        else {
            continue;
        };

        if !*is_public {
            continue;
        }

        structs.insert(
            name.clone(),
            fields
                .iter()
                .map(|field| StructFieldSignature {
                    name: field.name.clone(),
                    annotation: normalize_type_text(&field.annotation.text),
                    is_private: field.is_private,
                })
                .collect(),
        );
    }

    structs
}

/// Collects public type and struct documentation.
///
/// Inputs: syntax-output module. Output: public type name to docs.
/// Transformation: copies declaration docs for public type-like declarations.
pub(super) fn collect_syntax_type_docs(
    module: &SyntaxModuleOutput,
) -> HashMap<String, Vec<String>> {
    let mut docs = HashMap::new();
    for declaration in &module.declarations {
        match &declaration.payload {
            SyntaxDeclarationPayload::Type {
                name, is_public, ..
            }
            | SyntaxDeclarationPayload::Struct {
                name, is_public, ..
            } if *is_public => {
                docs.insert(name.clone(), declaration.docs.clone());
            }
            _ => {}
        }
    }
    docs
}

/// Converts syntax parameters to HIR parameter signatures.
///
/// Inputs: syntax-output params. Output: span-free parameter signatures.
/// Transformation: normalizes type text and preserves parameter names and
/// mutability flags.
pub(super) fn syntax_param_signatures(params: &[SyntaxParamOutput]) -> Vec<ParamSignature> {
    params
        .iter()
        .map(|param| ParamSignature {
            name: param.name.clone(),
            annotation: normalize_type_text(&param.annotation.text),
            is_mutable: param.is_mutable,
            default: canonical_param_default(param),
            default_text: param.default_text.clone(),
        })
        .collect()
}

/// Re-parses a parameter's canonical default text into executable syntax.
///
/// Interface parsing may preserve quoted literal source in the structured
/// expression payload. Parsing the already validated source-like text gives
/// CoreIR lowering the same decoded literal representation as an ordinary call
/// argument while retaining the original payload as a defensive fallback.
pub(super) fn canonical_param_default(param: &SyntaxParamOutput) -> Option<SyntaxExprOutput> {
    param
        .default_text
        .as_deref()
        .and_then(|text| parse_expr_as_syntax_output(text).ok())
        .or_else(|| param.default.clone())
}

pub(super) fn qualify_param_signatures(
    mut params: Vec<ParamSignature>,
    imported_type_refs: &HashMap<String, String>,
) -> Vec<ParamSignature> {
    for param in &mut params {
        param.annotation = qualify_syntax_type_text(&param.annotation, imported_type_refs);
    }
    params
}

mod symbols;
pub(super) use symbols::{
    add_syntax_function_symbol, declaration_has_marker_annotation, syntax_method_param_signatures,
};

/// Resolves interface export summaries against local function definitions.
///
/// Inputs: export set, mutable function symbols, and diagnostics sink. Output:
/// function export flags or diagnostics are updated. Transformation: marks
/// matching symbols exported and reports missing definitions.
pub(super) fn resolve_exports_against_defs(
    exported_fns: &HashSet<(String, usize)>,
    function_symbols: &mut HashMap<(String, usize), FunctionSymbol>,
    diagnostics: &mut Vec<Diagnostic>,
) {
    for (name, arity) in exported_fns {
        if let Some(symbol) = function_symbols.get_mut(&(name.clone(), *arity)) {
            symbol.exported = true;
        } else {
            diagnostics.push(Diagnostic {
                span: Span::new(0, 0),
                message: format!("exported function {}/{} is not defined", name, arity),
            });
        }
    }
}

/// Collects interface-only export summaries into the resolver export set.
///
/// Inputs:
/// - `source_kind`: whether the syntax output came from a source module or an
///   interface summary.
/// - `items`: interface export-summary entries preserved by the syntax-output
///   frontend.
/// - `span`: source span for diagnostics.
/// - `exported_fns`: mutable export set keyed by function name and arity.
/// - `diagnostics`: mutable HIR diagnostic sink.
///
/// Output:
/// - No direct return value.
///
/// Transformation:
/// - Interface outputs may preserve export summaries, so their entries are
///   copied into `exported_fns`.
/// - Normal module outputs must use declaration-site `pub`; an export payload
///   in module mode is reported and ignored.
pub(super) fn collect_export_payloads(
    source_kind: SyntaxSourceKind,
    items: &[crate::terlan_syntax::SyntaxExportItem],
    span: Span,
    exported_fns: &mut HashSet<(String, usize)>,
    diagnostics: &mut Vec<Diagnostic>,
) {
    match source_kind {
        SyntaxSourceKind::Interface => {
            for entry in items {
                exported_fns.insert((entry.name.clone(), entry.arity));
            }
        }
        SyntaxSourceKind::Module => diagnostics.push(Diagnostic {
            span,
            message:
                "source export declarations are not part of canonical Terlan; use `pub` on declarations"
                    .to_string(),
        }),
    }
}

#[path = "interface_conversion/builtins.rs"]
mod builtins;
#[cfg(test)]
#[path = "lib_test.rs"]
#[cfg(test)]
mod lib_test;
pub(super) use builtins::builtin_interfaces;

...
impl ComposedCallProfile {
    ...
    pub(super) fn new(
        ...
        Self::build(function_body, continuations, terminal_profiles, None).ok()
    }

    /// Includes admitted closure-owned outward entries without copying their bodies.
    pub(super) fn with_dynamic_targets(
        function_body: &NativeExpr,
        continuations: &[NativeContinuation],
        terminal_profiles: &HashMap<usize, ComposedCallProfile>,
    ...
    fn build(
        ...
        continuations: &[NativeContinuation],
        terminal_profiles: &HashMap<usize, ComposedCallProfile>,
        dynamic: Option<&DynamicCallProfiles>,
        ...
        for id in outward_ids {
            ...
            let source = terminal_profiles
                ...
                .chain(dynamic.into_iter().flat_map(|profiles| {
                    profiles
                        .values()
                        .flatten()
                        .flat_map(|target| target.profile.continuations.iter())
                }))
...
pub(super) use profile_diagnostics::profile_gap_reason;

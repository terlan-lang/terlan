...
fn one_image_executes_closed_application_features_and_rejects_unbounded_peers() {
    ...
    *remote = CoreExpr::RemoteCall {
    ...
    // SAFETY: the freshly linked fixture exports this versioned symbol with
    // the `NativeDispatch` signature generated by the same compiler build.
    ...
    let generic_modules =
        NativeModule::lower_application(&[&generic_template]).expect("generic template image");
    assert!(generic_modules
        .iter()
        .flat_map(|module| &module.functions)
...
fn link_library(object: &[u8]) -> (Library, std::path::PathBuf) {
    ...
    // SAFETY: the path names the fixture shared object successfully produced
    // by the immediately preceding linker invocation.

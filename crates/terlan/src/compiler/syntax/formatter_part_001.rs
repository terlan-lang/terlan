...
const BINARY_LAYOUT_MAX_INLINE_LENGTH: usize = 120;
...
fn promote_repeated_direct_value_imports(module: &Module) -> Module {
    let mut counts = BTreeMap::new();
...
fn collapse_compatible_imports(module: &Module) -> Module {
...
fn collect_value_call_refs_from_module(
    module: &Module,
...
fn collect_value_call_refs_from_decl(decl: &Decl, counts: &mut BTreeMap<(String, String), usize>) {
    match decl {
...
fn collect_value_call_refs_from_function_clause(
...
fn collect_value_call_refs_from_case_clause(
    clause: &CaseClause,
    counts: &mut BTreeMap<(String, String), usize>,
) {
    if let Some(guard) = &clause.guard {

...
mod calls;
use calls::{format_call_expression, format_method_call_chain};
            ...
            clauses
                ...
                .map(|clause| {
                    let patterns = clause
                        ...
                        .enumerate()
                        .map(|(index, pattern)| {
                            let pattern = format_pattern(pattern);
                            match clause.parameter_types.get(index).and_then(Option::as_ref) {
                                Some(annotation) => {
                                    format!("{pattern}: {}", format_type_expr(annotation))
                                }
                                None => pattern,
                            }
                        })

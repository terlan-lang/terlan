...
/// Prevents user metadata from shadowing compiler-owned annotation paths.
#[test]
pub(super) fn annotation_isolation_parity_rejects_reserved_schema_namespace() {
    let error = parse_module_as_syntax_output(
        r#"
        module reserved_annotation_schema.

        annotation compiler.delete {
          applies_to: function;
          enabled: Bool;
        }.
        "#,
    )
    .expect_err("reserved annotation schema should fail");

    assert!(format!("{error:?}").contains(
        "annotation schema `@compiler.delete` uses reserved compiler metadata namespace `compiler`"
    ));
}

/// Prevents a later schema declaration from replacing an earlier contract.
#[test]
pub(super) fn annotation_isolation_parity_rejects_duplicate_schema_path() {
    let error = parse_module_as_syntax_output(
        r#"
        module duplicate_annotation_schema.

        annotation metadata.rewrite {
          applies_to: function;
          action: Name;
        }.
    ...
    .expect_err("duplicate annotation schema should fail");

    assert!(format!("{error:?}").contains("duplicate annotation schema `@metadata.rewrite`"));
}
...
/// - Exercises required-key enforcement before semantic lowering.
#[test]
pub(super) fn syntax_output_rejects_user_annotation_missing_required_key() {
    let error = parse_module_as_syntax_output(
        r#"
...
///   metadata.
#[test]
pub(super) fn syntax_output_rejects_user_annotation_unknown_key() {
    let error = parse_module_as_syntax_output(
        r#"
...
/// - Checks typed annotation metadata against user-declared key schemas.
#[test]
pub(super) fn syntax_output_rejects_user_annotation_wrong_value_type() {
    let error = parse_module_as_syntax_output(
        r#"
...
/// - Applies the schema repeatability default of false to user metadata.
#[test]
pub(super) fn syntax_output_rejects_user_annotation_duplicate_non_repeatable_key() {
    let error = parse_module_as_syntax_output(
        r#"
...
///   itself has matched the declaration.
#[test]
pub(super) fn syntax_output_rejects_user_annotation_key_target_mismatch() {
    let error = parse_module_as_syntax_output(
        r#"
...
///   receiver-method syntax is no longer downgraded to raw output.
#[test]
pub(super) fn syntax_output_preserves_receiver_methods_as_method_decls() {
...
///   mutability marker without lowering or resolving its semantics.
#[test]
pub(super) fn syntax_output_preserves_mutable_receiver_marker() {
    let output = parse_module_as_syntax_output(
        r#"
...
///   typechecking or backend lowering.
#[test]
pub(super) fn syntax_output_preserves_release_core_collection_contracts() {
    let contracts = [
        (
            "std.collections.Map",
    ...
    for (expected_module, source, expected_methods) in contracts {
            ...
            expected_methods
        {
            let method = output
                ...
                .unwrap_or_else(|| panic!("missing method `{expected_name}` in {expected_module}"));
...
///   structured declarations without typechecking or backend lowering.
#[test]
pub(super) fn syntax_output_preserves_release_traversal_contracts() {
    let iterator_output = parse_module_as_syntax_output(include_str!(
    ...
    let iterable_output = parse_module_as_syntax_output(include_str!(
...
///   syntax-output payload to match the EBNF `ConfigDecl` contract.
#[test]
pub(super) fn syntax_output_normalizes_config_declarations() {
    let output = parse_module_as_syntax_output(
        r#"
    ...
    match &output.declarations[0].payload {
        SyntaxDeclarationPayload::Config {
            ...
            assert_eq!(target, "vm");
            assert!(text.starts_with("target vm {"));
            assert_eq!(entries.len(), 4);
            assert_eq!(entries[0].key, "application");
            assert_eq!(
                entries[0].value,
...
#[test]
pub(super) fn interface_syntax_output_marks_source_kind() {
...
#[test]
pub(super) fn syntax_output_includes_struct_constructor_trait_and_template_signatures() {
    let output = parse_module_as_syntax_output(
        r#"
        module rich.

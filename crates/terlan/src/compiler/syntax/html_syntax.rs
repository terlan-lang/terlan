use super::{
    parse_tree::{Expr, HtmlAttr, HtmlAttrValue, HtmlElement, HtmlNamedSlot, HtmlNode},
    parser::{syntax_contract_parse_error, ParseError, ParseResult},
    span::Span,
    syntax_contract::ensure_canonical_syntax_contract_valid as ensure_syntax_contract_valid,
};
...
pub(crate) fn parse_terlan_expr(raw: &str) -> ParseResult<Expr> {
    ...
    if let Some(error) = crate::terlan_syntax::lalrpop_diagnostics::expression_preflight(&raw) {
        return Err(ParseError {
            message: error.message,
            span: error.span,
        });
    }
    let generated = crate::terlan_syntax::lalrpop_boundary::parse_lalrpop_expression(&raw)
        .map_err(|error| {
            crate::terlan_syntax::lalrpop_diagnostics::expression_diagnostic(&raw, error)
        })
        .map_err(|error| ParseError {
            message: error.message,
            span: error.span,
        })?;
    crate::terlan_syntax::lalrpop_lowering::lower_lalrpop_expression(&raw, &generated.root).map_err(
        |error| ParseError {
            message: error.message,
            span: error.span,
        },

...
fn collect_value_call_refs_from_expr(expr: &Expr, counts: &mut BTreeMap<(String, String), usize>) {
    match expr {
        Expr::Tuple(items)
        | Expr::List(items)
...
fn collect_value_call_refs_from_html_block(
    block: &HtmlBlockExpr,
...
fn collect_value_call_refs_from_html_node(
    node: &HtmlNode,
    counts: &mut BTreeMap<(String, String), usize>,
) {
    match node {
...
fn blocked_value_promotion_names(module: &Module) -> BTreeSet<String> {
    let mut blocked = BTreeSet::new();
    for decl in &module.declarations {
        match decl {
            Decl::Function(decl) => {
...
fn existing_value_imports(module: &Module) -> BTreeMap<String, String> {
    let mut imports = BTreeMap::new();
    for decl in &module.declarations {
        let Decl::Import(import) = decl else {
            continue;
...
fn selected_value_import_modules(module: &Module) -> BTreeSet<String> {
...
fn is_collapsible_import(import: &ImportDecl) -> bool {
    matches!(import.kind, ImportKind::Module)
        && import.source_path.is_none()
        && import.items.iter().all(|item| item.name != "*")
}

fn normalize_default_selected_import(import: &ImportDecl) -> ImportDecl {
...
fn import_items_equal(left: &ImportItem, right: &ImportItem) -> bool {
    left.name == right.name && left.as_alias == right.as_alias
}

fn import_item_sort_key(left: &ImportItem, right: &ImportItem) -> std::cmp::Ordering {
    (&left.name, &left.as_alias).cmp(&(&right.name, &right.as_alias))
}

fn is_value_import_candidate(module_name: &str, function_name: &str) -> bool {
...
fn value_call_callee_name(callee: &Expr) -> Option<&String> {
    match callee {
...
fn rewrite_value_call_refs_in_module(
    module: &mut Module,
...
fn rewrite_value_call_refs_in_decl(
    decl: &mut Decl,
    replacements: &BTreeMap<(String, String), String>,
) {
    match decl {
...
fn rewrite_value_call_refs_in_function_clause(
...
fn rewrite_value_call_refs_in_case_clause(
...
fn rewrite_value_call_refs_in_expr(
    expr: &mut Expr,
    replacements: &BTreeMap<(String, String), String>,
) {
    match expr {
        ...
        Expr::Call {
            ...
            if type_args.is_empty() {
                    ...
                    (remote.as_ref(), value_call_callee_name(callee))
                {
                    ...
                    {
                        *callee = Box::new(Expr::Atom(local_name.clone()));
                        *remote = None;
                    }
                }
            }

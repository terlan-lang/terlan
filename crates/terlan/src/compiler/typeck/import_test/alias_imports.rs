...
/// Verifies local structs qualify selected imported opaque field types.
///
/// Inputs:
/// - A provider exporting one opaque value type.
/// - A consumer importing that type under an alias, storing it in a local
///   struct, and returning the field through a function signature.
///
/// Output:
/// - No type diagnostics.
///
/// Transformation:
/// - Exercises the same import-resolution environment for struct fields that
///   function signatures already use, preserving opaque nominal identity.
#[test]
fn syntax_output_local_struct_fields_preserve_imported_opaque_identity() {
    let diagnostics = check_syntax_output_with_interface(
        "\
module imported_opaque_struct_field.\n\
import type provider.{Value as ImportedValue}.\n\
pub struct Envelope {\n\
    value: ImportedValue\n\
}.\n\
pub wrap(value: ImportedValue): Envelope ->\n\
    Envelope(value = value).\n\
pub unwrap(envelope: Envelope): ImportedValue ->\n\
    envelope.value.\n\
",
        "\
module provider.\n\
pub opaque type Value = String.\n\
",
    );

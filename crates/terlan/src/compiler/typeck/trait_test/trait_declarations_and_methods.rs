...
/// Verifies trait text qualification preserves compiler-owned primitive names.
#[test]
pub(super) fn imported_trait_type_refs_qualify_nominal_types_but_preserve_primitives() {
    let imported = HashMap::from([
        (
            "List".to_string(),
            QualifiedTypeName {
                module: "std.collections.List".to_string(),
                name: "List".to_string(),
            },
        ),
        (
            "Unit".to_string(),
            QualifiedTypeName {
                module: "std.core.Unit".to_string(),
                name: "Unit".to_string(),
            },
        ),
    ]);

    let refs = imported_type_text_refs(&imported);

    assert_eq!(
        refs.get("List"),
        Some(&"std.collections.List.List".to_string())
    );
    assert!(!refs.contains_key("Unit"));
}
...
#[test]
pub(super) fn syntax_output_accepts_unit_alias_in_explicit_trait_impl_methods() {
...
#[test]
pub(super) fn syntax_output_checks_trait_decls_on_formal_path() {
...
#[test]
pub(super) fn syntax_output_checks_declared_implements_receiver_methods_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_inherited_receiver_method_satisfies_declared_implements() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_rejects_declared_implements_missing_required_method_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_accepts_declared_implements_trait_default_methods_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_rejects_declared_implements_receiver_signature_mismatch_on_formal_path()
{
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_resolves_declared_implements_trait_method_calls_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_accepts_declared_implements_mutable_receiver_requirement() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_rejects_declared_implements_missing_mutable_receiver() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_uses_generic_bounds_for_trait_method_dispatch_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_checks_explicit_trait_impl_methods_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_rejects_explicit_trait_impl_missing_required_method_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_accepts_explicit_trait_impl_default_methods_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_accepts_imported_trait_impl_default_methods_on_formal_path() {
...
",
    ...
    assert!(
        diagnostics.is_empty(),
        "unexpected std.vm.GenServer diagnostics: {:?}",
        diagnostics
    );
}

#[test]
pub(super) fn syntax_output_rejects_explicit_trait_impl_signature_mismatch_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_rejects_explicit_trait_impl_body_return_mismatch_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_rejects_duplicate_declared_and_explicit_trait_impl_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_resolves_explicit_trait_impl_method_calls_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
/// Verifies explicit trait implementations are callable with receiver syntax.
///
/// Inputs:
/// - A trait whose method takes the implemented value as its first parameter.
/// - An explicit `impl` block for a concrete struct.
/// - A call using `value.method()` instead of `Trait.method(value)`.
///
/// Output:
/// - No diagnostics; receiver dispatch resolves through the trait
///   implementation rather than requiring a duplicate concrete receiver method.
///
/// Transformation:
/// - Prepends the receiver to the call arguments and reuses ordinary trait
///   method dispatch, preserving the trait as the single API contract.
#[test]
pub(super) fn syntax_output_resolves_explicit_trait_impl_receiver_calls_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
module explicit_trait_impl_receiver_dispatch.\n\
pub trait Show[T] {\n\
    show(value: T): String.\n\
}.\n\
\n\
...
#[test]
pub(super) fn syntax_output_resolves_explicit_hkt_trait_impl_method_calls_on_formal_path() {
    let diagnostics = check_syntax_output(
        "\
...
#[test]
pub(super) fn syntax_output_resolves_imported_hkt_trait_impl_method_calls_on_formal_path() {
    let interface_source = "\

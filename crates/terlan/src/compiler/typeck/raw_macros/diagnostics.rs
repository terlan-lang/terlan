...
use super::expansion::raw_macro_resolution_message;
...
pub(crate) fn raw_macro_resolution_message_for_expr(expr: &SyntaxExprOutput) -> String {
    ...
    if name == "sql" {
        let (
            ...
            result_type_message,
            query_kind_message,
            transaction_requirement_message,
            cardinality,
            ...
            Ok(Some(analysis)) => (
                ...
                format!(
                    "inferred SQL query kind: {}",
                    analysis.query_kind.as_diagnostic_label()
                ),
                format!(
                    "inferred SQL transaction requirement: {}",
                    analysis.transaction_requirement.as_diagnostic_label()
                ),
            ...
            Ok(None) => (
                ...
                "inferred SQL query kind: unknown".to_string(),
                "inferred SQL transaction requirement: unknown".to_string(),
...
fn sql_wrapper_plan_status_message(expr: &SyntaxExprOutput) -> String {
    match crate::terlan_typeck::sql_forms::build_sql_wrapper_plan(expr, expr.children.len()) {
        Ok(Some(plan)) => format!(
            "SQL wrapper plan: ready row_type={}, params={}, projection_fields={}, bound_sql_bytes={}, result_type={}, cardinality={}, transaction_requirement={}",
            plan.row_type,
            ...
            plan.transaction_requirement.as_diagnostic_label()
        ),
...
pub fn collect_syntax_raw_macro_diagnostics(module: &SyntaxModuleOutput) -> Vec<Diagnostic> {
    ...
    for declaration in &module.declarations {
        match &declaration.payload {
            ...
            SyntaxDeclarationPayload::ConstFunction { body, .. } => {
                collect_raw_macro_diagnostics_in_expr(
                    body,
                    declaration.span.into(),
                    &mut diagnostics,

...
/// Verifies a direct function reference must have the callback's arity.
///
/// Inputs:
/// - A function accepting a one-parameter callback.
/// - A local two-parameter function passed by name.
///
/// Output:
/// - Test passes when typechecking rejects the direct reference before code
///   generation.
#[test]
fn syntax_output_rejects_direct_callback_reference_with_wrong_arity() {
    let diagnostics = check_syntax_output(
        "\
module callback_arity_bad.\n\
\n\
pub accept(callback: (Int) -> Bool): Bool ->\n\
    callback(1).\n\
\n\
pub between(value: Int, lower: Int): Bool ->\n\
    value > lower.\n\
\n\
pub demo(): Bool ->\n\
    accept(between).\n\

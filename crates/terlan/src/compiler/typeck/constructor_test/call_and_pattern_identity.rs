...
///
/// Output:
/// - Test passes when typechecking succeeds and CoreIR canonicalizes both the
///   constructor head and identity to the provider declaration.
///
/// Transformation:
/// - Resolves the aliased import against an explicit interface map, lowers
///   to CoreIR, and verifies the local alias does not leak into the canonical
///   runtime constructor representation.
#[test]
...
///
/// Output:
/// - Test passes when CoreIR canonicalizes both the constructor head and
///   identity to `provider.Ok`.
///
/// Transformation:
/// - Resolves the aliased import against an explicit interface map, lowers
///   to CoreIR, and verifies the local alias does not leak into the canonical

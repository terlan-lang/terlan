...
///   to Terlan `Unit`.
pub(in crate::backends::erlang::emit) fn lower_runtime_console_println(
    args: Vec<ErlExpr>,
) -> Option<ErlExpr> {
...
pub(in crate::backends::erlang::emit) fn lower_runtime_file_exists(
    args: Vec<ErlExpr>,
) -> Option<ErlExpr> {
...
pub(in crate::backends::erlang::emit) fn lower_runtime_file_read_text(
    args: Vec<ErlExpr>,
) -> Option<ErlExpr> {
    let [path] = exact_args(args, 1)?.try_into().ok()?;
...
pub(in crate::backends::erlang::emit) fn lower_runtime_file_write_text(
    args: Vec<ErlExpr>,
) -> Option<ErlExpr> {
...
///   `std.io.File.FileError` records before returning `Result[Unit, FileError]`.
pub(in crate::backends::erlang::emit) fn lower_runtime_file_append_text(
    args: Vec<ErlExpr>,
) -> Option<ErlExpr> {
    let [path, text] = exact_args(args, 2)?.try_into().ok()?;
    let file_error = map_struct_name("FileError");
...
///   `std.io.File.FileError` records before returning `Result[Unit, FileError]`.
pub(in crate::backends::erlang::emit) fn lower_runtime_file_delete(
    args: Vec<ErlExpr>,
) -> Option<ErlExpr> {
    let [path] = exact_args(args, 1)?.try_into().ok()?;
    let file_error = map_struct_name("FileError");

...
/// Verifies custom type specs share canonical Terlan naming normalization.
///
/// Inputs:
/// - Acronym-heavy custom type names.
///
/// Output:
/// - Test passes when Erlang type names use the same word-boundary handling as
///   generated bindings.
///
/// Transformation:
/// - Lowers custom type names through the Erlang type-spec mapper and checks
///   that acronyms collapse into useful words instead of one-letter segments.
#[test]
fn maps_custom_type_names_with_shared_acronym_boundaries() {
    assert_eq!(super::map_type_name("HTMLElement"), "html_element");
    assert_eq!(super::map_type_name("URLValue"), "url_value");
}

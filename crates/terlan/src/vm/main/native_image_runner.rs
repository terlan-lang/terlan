...
use super::{evaluate_script_result, evaluate_test_result, RunResultMode};
...
pub(super) fn run_tvm_image(
    ...
    shutdown?;
    match result_mode {
        RunResultMode::Discard => {}
        RunResultMode::Test => evaluate_test_result(value)?,
        RunResultMode::Script => {
            if let Some(rendered) = evaluate_script_result(value) {
                output(&rendered);
            }
        }
    }

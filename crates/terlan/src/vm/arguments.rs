...
pub(super) fn parse_run_args(args: &[String]) -> VmCommand {
    let mut source = None;
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--" => {
                program_arguments.extend_from_slice(&args[index + 1..]);
                break;
            }
            "--help" | "-h" => return VmCommand::Help,
            "--test-eval" => {
                if result_mode == RunResultMode::Script {
                    return VmCommand::Error(
                        "--test-eval and --script-eval are mutually exclusive".to_string(),
                    );
                }
            ...
            "--script-eval" => {
                if result_mode == RunResultMode::Test {
                    return VmCommand::Error(
                        "--test-eval and --script-eval are mutually exclusive".to_string(),
                    );
                }
...
pub(super) fn render_persistent_actor_export_manifest(
    args: &VmPersistentActorExportCommandArgs,
) -> Result<String, String> {
    let export = build_persistent_actor_export(
        VmPersistentActorExportInput {
            actor_id: &args.actor_id,
            schema_id: &args.schema_id,
            schema_version: args.schema_version,
            source_adapter_kind: &args.source_adapter_kind,
            snapshot_generation: args.snapshot_generation,
            last_event_sequence: args.last_event_sequence,
            retained_event_count: args.retained_event_count,
            redacted_fields: &args.redacted_fields,
            resource_handles: &args.resource_handles,
        },
        false,
    )?;
...
pub(super) fn render_persistent_actor_restore_plan(
...
struct VmPersistentActorExportInput<'a> {
    actor_id: &'a str,
    schema_id: &'a str,
    schema_version: u64,
    source_adapter_kind: &'a str,
    snapshot_generation: u64,
    last_event_sequence: u64,
    retained_event_count: usize,
    redacted_fields: &'a [String],
    resource_handles: &'a [String],
}

fn build_persistent_actor_export(
    args: VmPersistentActorExportInput<'_>,
    compacted: bool,
) -> Result<VmPersistentActorExport, String> {
    let actor_id = VmPersistentActorId::new(args.actor_id.to_string())?;
    let schema = VmPersistentActorSchema::new(args.schema_id.to_string(), args.schema_version)?;
    let snapshot = VmPersistentActorSnapshot::new(
        ...
        VmPersistentActorDurability {
            resource_handles: args.resource_handles.to_vec(),
            last_event_sequence: args.last_event_sequence,
        },
    )?;
    let mut events = Vec::with_capacity(args.retained_event_count);
    for offset in 0..args.retained_event_count {
        let sequence = args
            .last_event_sequence
    ...
    Ok(
        VmPersistentActorExport::new(snapshot, events, args.redacted_fields.to_vec(), compacted)
            .map_err(|error| format!("error[vm_persistent_actor_export]: {error:?}"))?
            .with_source_adapter_kind(args.source_adapter_kind.to_string()),
    )
}

/// Parses `terlan-vm load` arguments.
pub(super) fn parse_load_args(args: &[String]) -> VmCommand {
    match args {
...
pub(super) fn parse_package_image_metadata_args(args: &[String]) -> VmCommand {
    let mut image = None;
...
pub(super) fn parse_validate_package_args(args: &[String]) -> VmCommand {
    match args {
        [flag] if matches!(flag.as_str(), "--help" | "-h") => VmCommand::Help,
...
pub(super) fn parse_support_bundle_args(args: &[String]) -> VmCommand {
    match args {
        [flag] if matches!(flag.as_str(), "--help" | "-h") => VmCommand::Help,
...
pub(super) fn parse_benchmark_http_handler_args(args: &[String]) -> VmCommand {
...
pub(super) fn parse_benchmark_http_stack_args(args: &[String]) -> VmCommand {
    let mut iterations = 10_000usize;
    let mut index = 0;
    while index < args.len() {
        match args[index].as_str() {
...
pub(super) fn parse_benchmark_in_memory_framing_args(args: &[String]) -> VmCommand {
    let mut iterations = 10_000usize;
...
pub(super) fn parse_benchmark_http_vm_stream_args(args: &[String]) -> VmCommand {
...
pub(super) fn parse_benchmark_http_socket_args(args: &[String]) -> VmCommand {
    let mut iterations = 100usize;
...
pub(super) fn parse_inspect_args(args: &[String]) -> VmCommand {
    match args {
        [flag] if matches!(flag.as_str(), "--help" | "-h") => VmCommand::Help,
...
pub(super) fn run_path(
    ...
    if is_tvm_image_path(source) {
        ...
        run_source_file(source, entry, result_mode, output)
    }
}

/// Returns the stable rejection for the deleted public serialized-artifact lane.
pub(super) fn tvm_json_runtime_removed_error(path: &Path) -> String {
    format!(
...
pub(super) fn is_vm_artifact_path(path: &Path) -> bool {
    path.file_name()

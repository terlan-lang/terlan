...
fn wait_for_vm_http_server(port: u16) -> Result<(), String> {
...
fn vm_http_round_trip(port: u16) -> Result<(), String> {
...
fn measure_vm_artifact_build_single_file(
...
fn measure_vm_artifact_load_single_file(
    vm_binary: &Path,
    compiler_binary: &Path,
    source: &Path,
) -> Result<(), String> {
...
fn build_single_file_vm_artifact(compiler_binary: &Path, source: &Path) -> Result<PathBuf, String> {
...
fn measure_vm_process_runtime_primitives() -> Result<(), String> {
...
fn measure_vm_process_inspection_startup() -> Result<(), String> {
    let mut processes = VmProcessTable::default();

...
pub(crate) struct WebRouteError(String);
...
impl From<WebRouteError> for String {
    fn from(error: WebRouteError) -> Self {
        error.0
    }
}
...
pub(crate) fn validate_route_pattern(route: &str) -> Result<(), WebRouteError> {
    if route == "*" {
        return Ok(());
    }
    if route == "/" {
    ...
    for component in Path::new(route.trim_start_matches('/')).components() {
        if matches!(
            ...
            return Err(WebRouteError(format!(
    ...
    for (index, segment) in segments.iter().enumerate() {
        ...
        if segment.starts_with(':') {
            let Some((name, _suffix)) = colon_route_param_segment(segment) else {
                return Err(WebRouteError(format!(
                    "error[serve_package]: invalid route parameter `{segment}` in `{route}`"
                )));
            };
...
pub(crate) fn route_ambiguity_key(route: &str) -> Result<String, WebRouteError> {
    ...
    let normalized = route_segments(route)
        .into_iter()
        .map(|segment| {
            if let Some((_name, suffix)) = colon_route_param_segment(segment) {
                if suffix.is_empty() {
                    ":".to_string()
                } else {
                    format!(":{suffix}")
...
pub(crate) fn route_param_names(route: &str) -> Result<Vec<String>, WebRouteError> {
...
///   same route contract.
pub(crate) fn route_param_types(route: &str) -> Result<Vec<(String, String)>, WebRouteError> {
    validate_route_pattern(route)?;
    if route == "*" {
        return Ok(Vec::new());
    }
    Ok(route_segments(route)
        ...
        .filter_map(|segment| {
            ...
            } else if let Some((name, _suffix)) = colon_route_param_segment(segment) {
...
/// Parses a colon capture with an optional literal filename suffix.
///
/// Examples are `:name` and `:name.json`. The suffix remains part of route
/// matching but is excluded from the value stored under the capture name.
pub(crate) fn colon_route_param_segment(segment: &str) -> Option<(&str, &str)> {
    let value = segment.strip_prefix(':')?;
    let name_bytes = value
        .bytes()
        .take_while(|byte| byte.is_ascii_alphanumeric() || *byte == b'_')
        .count();
    let (name, suffix) = value.split_at(name_bytes);
    if !is_route_param_name(name)
        || suffix.contains([':', '*', '{', '}', '/'])

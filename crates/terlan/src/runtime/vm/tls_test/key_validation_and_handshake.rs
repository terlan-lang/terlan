...
#[test]
pub(super) fn vm_tls_runtime_reports_missing_manual_private_key_file() {
...
#[test]
pub(super) fn vm_tls_runtime_reports_malformed_manual_private_key_file() {
...
#[test]
pub(super) fn vm_tls_runtime_reports_manual_private_key_without_supported_key() {
...
#[test]
pub(super) fn vm_tls_runtime_starts_manual_server_connection_with_readiness_state() {
...
#[test]
pub(super) fn vm_tls_runtime_starts_internal_server_connection_with_readiness_state() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_tls_runtime_rejects_plaintext_server_connection_start() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_tls_runtime_rejects_auto_server_connection_without_cache() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_tls_server_connection_roundtrips_handshake_and_plaintext() {
...
#[test]
pub(super) fn vm_tls_server_connection_reports_malformed_tls_packets() {
...
#[test]
pub(super) fn vm_tls_plaintext_reader_handles_eof_would_block_and_errors() {
    let mut eof_reader = plaintext_reader(vec![Ok(b"abc"), Ok(b"")]);
    assert_eq!(
        super::super::read_plaintext_from(&mut eof_reader).expect("read until eof"),
        b"abc".to_vec()
    );

    let mut would_block_reader = plaintext_reader(vec![Ok(b"abc"), Err(ErrorKind::WouldBlock)]);
    assert_eq!(
        super::super::read_plaintext_from(&mut would_block_reader).expect("read until would block"),
        b"abc".to_vec()
    );

    let mut error_reader = plaintext_reader(vec![Err(ErrorKind::ConnectionReset)]);
    assert_eq!(
        super::super::read_plaintext_from(&mut error_reader)
            .expect_err("hard read error should fail"),
        "VM TLS failed to read plaintext: scripted plaintext read failure"
    );
}

#[test]
pub(super) fn vm_tls_plaintext_writer_reports_written_bytes_and_errors() {
    let mut ok_writer = ScriptedPlaintextWriter { result: Ok(4) };
    assert_eq!(
        super::super::write_plaintext_to(&mut ok_writer, b"pong").expect("plaintext write"),
        4
    );
    ...
    assert_eq!(
        super::super::write_plaintext_to(&mut error_writer, b"pong")
...
#[test]
pub(super) fn vm_tls_tcp_server_stream_flushes_pending_tls_records_without_new_tcp_bytes() {
...
#[test]
pub(super) fn vm_tls_tcp_server_stream_roundtrips_over_vm_tcp_runtime() {
    let (dir, cert_path, key_path, cert_der) =
...
#[test]
pub(super) fn vm_tls_runtime_rejects_invalid_mode_specific_plans() {
    let mut runtime = VmTlsRuntime::new();
...
#[test]
pub(super) fn vm_tls_runtime_rejects_empty_listener_names() {
    let mut runtime = VmTlsRuntime::new();

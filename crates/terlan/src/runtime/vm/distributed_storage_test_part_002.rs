...
#[test]
fn vm_distributed_storage_schema_migration_rejects_stale_expected_version() {
...
#[test]
fn vm_distributed_storage_resource_handle_validation_rejects_missing_handles_without_mutation() {
    let mut adapter = opened_force_local_adapter();
    assert_eq!(
...
#[test]
fn vm_distributed_storage_reports_flush_timeout_with_retry_recovery() {
    let mut adapter = opened_force_local_adapter();
    assert!(matches!(
...
#[test]
fn vm_distributed_storage_exposes_capability_operation_and_mode_kinds() {
...
#[test]
fn vm_distributed_storage_recovered_snapshots_preserve_sequence_watermark() {
...
#[test]
fn vm_distributed_storage_durable_mode_writes_flushes_and_loads_snapshot() {
    let policy =
...
#[test]
fn vm_distributed_storage_cluster_capability_requires_cluster_mode_and_availability() {
...
#[test]
fn vm_distributed_storage_cluster_mode_writes_flushes_and_loads_snapshot() {
    let policy =
        VmDistributedStoragePolicy::new("cluster-prod", VmDistributedStorageMode::Cluster, true)
            .expect("cluster policy should be valid");
    let mut adapter = VmDistributedStorageAdapter::new(policy);
...
#[test]
fn vm_distributed_storage_cluster_mode_replicates_snapshots_through_adapter() {
    let policy =
        VmDistributedStoragePolicy::new("cluster-prod", VmDistributedStorageMode::Cluster, true)
            .expect("cluster policy should be valid");
    let mut adapter = VmDistributedStorageAdapter::new(policy);
...
#[test]
fn vm_distributed_storage_cluster_replication_reports_typed_failures() {
...
#[test]
fn vm_distributed_storage_reopen_preserves_snapshots_and_sequence_watermark() {
    let mut adapter = opened_force_local_adapter();
...
#[test]
fn vm_distributed_storage_closed_adapter_rejects_lifecycle_operations_until_reopen() {
    let mut adapter = opened_force_local_adapter();
    assert!(matches!(
        adapter.append(snapshot("checkpoint-a", 1, vec![entry("state", "cart", 1)])),
        VmDistributedStorageOutcome::Appended { sequence: 1, .. }
...
#[test]
fn vm_distributed_storage_returns_unavailable_for_missing_backend_without_panics() {
    let policy =
        VmDistributedStoragePolicy::new("durable-prod", VmDistributedStorageMode::Durable, false)
            .expect("policy should be valid");
    let mut adapter = VmDistributedStorageAdapter::new(policy);
...
#[test]
fn vm_distributed_storage_reports_unsupported_cluster_replication_for_local_mode() {
...
#[test]
fn vm_distributed_storage_compare_and_swap_append_rejects_stale_token() {
    let mut adapter = opened_force_local_adapter();
...
#[test]
fn vm_distributed_storage_rejects_invalid_policy_and_snapshot_descriptors() {
    assert_eq!(
...
fn opened_force_local_adapter() -> VmDistributedStorageAdapter {
    let mut adapter = VmDistributedStorageAdapter::new(VmDistributedStoragePolicy::force_local());
    assert!(matches!(
        adapter.open(),
        VmDistributedStorageOutcome::Opened { .. }
...
fn opened_durable_adapter() -> VmDistributedStorageAdapter {
    let policy =
        VmDistributedStoragePolicy::new("durable-prod", VmDistributedStorageMode::Durable, true)
            .expect("durable policy should be valid");
    let mut adapter = VmDistributedStorageAdapter::new(policy);
...
fn entry(namespace: &str, key: &str, value: i64) -> VmDistributedStateEntry {

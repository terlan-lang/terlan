use super::live_template_command::VmHttpSessionCommandPayload;
use super::{
    ...
    VmHttpSessionCommandOutcome, VmHttpSessionHotReloadMigrationReport,
...
fn apply_test_live_template_command(
...
fn live_template_source() -> VmHttpSessionLiveTemplateSourceSpan {
    VmHttpSessionLiveTemplateSourceSpan::new("app.Dashboard", 12, 5)
        .expect("live-template source span")
}

#[test]
fn http_session_lookup_creates_actor_and_sticky_metadata() {
    let mut sessions =
...
#[test]
fn http_session_adapter_functions_delegate_to_actor_runtime() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    ...
    assert_eq!(
        ...
        "stale HTTP session `s2`"
    );
}

#[test]
fn http_session_adapter_renders_non_string_values_for_string_get() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = current(&mut sessions, None).expect("current should create session");
...
#[test]
fn http_session_blank_cookie_creates_replacement_session() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
...
#[test]
fn http_session_affinity_accepts_single_typed_key() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
...
#[test]
fn http_session_affinity_merges_duplicate_matching_keys() {
    let affinity = [
...
#[test]
fn http_session_affinity_rejects_missing_and_conflicting_keys() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
...
#[test]
fn http_session_table_event_adapters_are_defensive() {
...
#[test]
fn http_session_delete_reports_stale_table_after_internal_cleanup() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
...
#[test]
fn http_session_private_lookup_paths_report_stale_sessions() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
...
#[test]
fn http_session_reuses_actor_and_table_state_for_cookie_lookup() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = sessions.lookup_or_create(None).expect("create session");
...
#[test]
fn http_session_actor_crash_during_request_cleans_state_and_replaces_cookie() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = sessions.lookup_or_create(None).expect("create session");
    sessions
...
#[test]
fn http_session_reconnect_after_actor_crash_replaces_cookie_without_reusing_state() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = sessions.lookup_or_create(None).expect("create session");
    sessions
...
#[test]
fn http_session_idempotent_command_replays_duplicate_result_without_rerun() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = sessions.lookup_or_create(None).expect("create session");
    let mut calls = 0;
...
#[test]
fn http_session_rejects_malformed_live_template_command_payload_before_dispatch() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = sessions.lookup_or_create(None).expect("create session");
    let command: fn(
...
#[test]
fn http_session_live_template_command_dispatches_actor_mailbox_postback_once() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = sessions.lookup_or_create(None).expect("create session");
...
#[test]
fn http_session_message_id_conversion_rejects_values_beyond_int() {
...
#[test]
fn http_session_live_template_subscribers_are_cleaned_after_actor_exit() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = sessions.lookup_or_create(None).expect("create session");
...
#[test]
fn http_session_live_template_subscription_requires_capability_before_registering() {
    let mut sessions =
        VmHttpSessionRuntime::new("node-a", 10).expect("session runtime should create");
    let created = sessions.lookup_or_create(None).expect("create session");

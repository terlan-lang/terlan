...
use super::{
    VmTlsMode, VmTlsPlan, VmTlsProvider, VmTlsRuntime, VmTlsTcpPoll, VmTlsTcpServerStream,
    VmTlsTransportMode,
};

fn plain_plan() -> VmTlsPlan {
    VmTlsPlan {
...
fn manual_plan_with_paths(cert_path: String, key_path: String) -> VmTlsPlan {
    VmTlsPlan {
        mode: VmTlsMode::Manual,
        domains: Vec::new(),
        email: None,
...
fn write_self_signed_cert_pair(name: &str) -> (std::path::PathBuf, String, String) {
...
fn write_self_signed_cert_pair_with_der(
    name: &str,
...
fn tls_client_for_cert(cert_der: Vec<u8>) -> ClientConnection {
...
fn pump_client_to_server(client: &mut ClientConnection, server: &mut super::VmTlsServerConnection) {
...
fn pump_server_to_client(server: &mut super::VmTlsServerConnection, client: &mut ClientConnection) {
...
fn complete_tls_handshake(
    ...
    server: &mut super::VmTlsServerConnection,
) {
    for _ in 0..10 {
...
fn expect_server_config_error(
    result: Result<super::VmTlsServerConfig, String>,
    message: &str,
) -> String {
    result.expect_err(message)
}

fn expect_server_connection_error(
    result: Result<super::VmTlsServerConnection, String>,
    message: &str,
) -> String {
    result.expect_err(message)
}

struct ScriptedPlaintextReader {
    steps: Vec<Result<Vec<u8>, ErrorKind>>,
}

struct ScriptedPlaintextWriter {
    result: Result<usize, ErrorKind>,
}
...
fn plaintext_reader(steps: Vec<Result<&[u8], ErrorKind>>) -> ScriptedPlaintextReader {
...
#[test]
fn vm_tls_runtime_installs_and_inspects_plain_plan() {
...
#[test]
fn vm_tls_runtime_accepts_manual_internal_and_auto_modes() {
    let mut runtime = VmTlsRuntime::new();
...
#[test]
fn vm_tls_runtime_binds_plan_to_tcp_listener_handle() {
...
#[test]
fn vm_tls_runtime_rejects_invalid_tcp_listener_plan() {
    let mut tcp = VmTcpRuntime::new();
    let listener = tcp.listen("https.local").expect("listener");
    let mut runtime = VmTlsRuntime::new();
...
#[test]
fn vm_tls_runtime_removes_tcp_listener_plan_on_shutdown() {
    let mut tcp = VmTcpRuntime::new();
    let listener = tcp.listen("https.local").expect("listener");
...
#[test]
fn vm_tls_runtime_enforces_rotation_overlap_window_before_retiring_old_config() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_rejects_zero_rotation_overlap() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_rejects_rotation_without_installed_listener_plan() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_hot_rotation_keeps_existing_connection_mode_for_old_accepts() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_reports_listener_transport_mode() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_reports_missing_listener_transport_plan() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_reports_missing_listener_server_config_and_connection() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_manual_and_internal_config_builders_report_missing_fields() {
    let mut manual = manual_plan();
    manual.cert_path = None;
    assert_eq!(
        expect_server_config_error(
            super::build_manual_server_config(&manual),
    ...
    assert_eq!(
        expect_server_config_error(
            super::build_internal_server_config(&internal),
...
#[test]
fn vm_tls_runtime_requires_plaintext_listener_for_raw_protocol_poll() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_requires_plaintext_listener_reports_missing_plan() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_builds_manual_rustls_server_config() {
...
#[test]
fn vm_tls_runtime_builds_internal_rustls_server_config() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_reports_internal_rustls_config_build_failure() {
    let generated =
        generate_simple_self_signed(vec!["localhost".to_string()]).expect("generate cert");
    let cert_der = generated.cert.der().as_ref().to_vec();

    let error = expect_server_config_error(
        super::build_internal_server_config_from_der(cert_der, vec![1, 2, 3]),
...
#[test]
fn vm_tls_runtime_rejects_plaintext_listener_server_config() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
fn vm_tls_runtime_rejects_invalid_manual_certificate_files() {
...
#[test]
fn vm_tls_runtime_rejects_manual_encrypted_private_key_plan() {
...
#[test]
fn vm_tls_runtime_reports_missing_manual_certificate_file() {
...
#[test]
fn vm_tls_runtime_reports_malformed_manual_certificate_file() {

...
use super::{
    ...
    VmSharedAllocationId, VmSharedAllocationKind, VmValueSizeError,
};
...
#[test]
fn memory_limits_reject_invalid_thresholds() {
    assert_eq!(
...
#[test]
fn memory_accounting_enforces_soft_and_hard_limits_without_partial_mutation() {
...
#[test]
fn memory_release_is_saturating_and_process_exit_clears_heap_state() {
    let mut processes = VmProcessTable::default();
    let pid = processes.spawn_root(source());
...
#[test]
fn memory_accounting_rejects_missing_and_exited_processes() {
    let mut processes = VmProcessTable::default();
    let pid = processes.spawn_root(source());
    processes
        .exit_process(pid, VmExitReason::Normal)
...
#[test]
fn memory_accounting_writes_deterministic_pressure_report() {
    let mut processes = VmProcessTable::default();
...
#[test]
fn memory_accounted_mailbox_send_receive_and_pressure_are_atomic() {
    let mut processes = VmProcessTable::default();
    ...
    assert_eq!(processes.get(recipient).expect("recipient").mailbox_len(), 1);
...
#[test]
fn memory_mailbox_routes_validate_before_heap_mutation_and_exit_releases_queue() {
    let mut processes = VmProcessTable::default();
    let sender = processes.spawn_root(source());
    let recipient = processes.spawn_root(source());
    let limits = VmMemoryLimits::new(64, 128).expect("limits");
...
#[test]
fn memory_selective_receive_releases_selected_charge_and_preserves_skipped_entries() {
    let mut processes = VmProcessTable::default();
    let sender = processes.spawn_root(source());
    let recipient = processes.spawn_root(source());
    let limits = VmMemoryLimits::new(100, 200).expect("limits");
...
#[test]
fn memory_resource_registration_rejects_hard_pressure_without_handle_mutation() {
    let mut processes = VmProcessTable::default();
...
#[test]
fn memory_resource_transfer_and_release_move_logical_ownership_atomically() {
    let mut processes = VmProcessTable::default();
    let owner = processes.spawn_root(source());
    let recipient = processes.spawn_root(source());
    let mut resources = VmResourceTable::default();
...
#[test]
fn memory_process_exit_releases_resources_mailbox_and_heap_without_cross_owner_cleanup() {
    let mut processes = VmProcessTable::default();
    let owner = processes.spawn_root(source());
...
#[test]
fn memory_process_exit_rejects_unaccounted_resource_without_mutation() {
    let mut processes = VmProcessTable::default();
    let owner = processes.spawn_root(source());
    let mut resources = VmResourceTable::default();
...
#[test]
fn memory_logical_value_size_accounts_nested_structural_values_exactly() {
...
#[test]
fn memory_logical_value_size_handles_deep_nesting_without_recursive_host_calls() {
...
#[test]
fn memory_logical_value_size_accounts_retained_achamp_patch_storage() {
    let entries = (0..128)
...
#[test]
fn memory_automatic_message_sizing_enforces_pressure_and_rejects_opaque_values() {
    let mut processes = VmProcessTable::default();
    let sender = processes.spawn_root(source());
    let recipient = processes.spawn_root(source());
...
#[test]
fn memory_shared_allocation_retain_release_and_pressure_are_atomic() {
    let mut processes = VmProcessTable::default();
    let owner = processes.spawn_root(source());
    let recipient = processes.spawn_root(source());

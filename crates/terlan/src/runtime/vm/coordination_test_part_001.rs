use super::{
    VmClusterMembership, VmClusterNodeState, VmCoordinationEnvelope, VmCoordinationProfile,
    VmDistributedDisconnectReason, VmDistributedInboundOutcome, VmDistributedReconnectOutcome,
    VmDistributedSessionState, VmDistributedTransportFrame, VmDistributedTransportSession,
    VmDistributionDelivery, VmMessageIdAllocator,
};
...
fn vm_coordination_accepts_compatible_peer_with_required_capability() {
    ...
    let envelope = VmCoordinationEnvelope::new(message_id, &from, &to, "vm.message.send")
        .expect("envelope");
...
#[test]
fn vm_distributed_transport_encodes_bounded_message_frame() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut session =
...
#[test]
fn vm_distributed_transport_tracks_needs_ack_lifecycle() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut session =
        VmDistributedTransportSession::open(from, to, 512).expect("session should open");
...
#[test]
fn vm_distributed_transport_rejects_incompatible_profiles_and_zero_limits() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
...
#[test]
fn vm_distributed_transport_rejects_oversized_encoded_messages_without_pending_ack() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut session =
...
#[test]
fn vm_distributed_transport_accepts_inbound_frames_and_rejects_duplicates() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut outbound =
...
#[test]
fn vm_distributed_transport_decodes_declared_atom_payload_before_acceptance() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut outbound =
        VmDistributedTransportSession::open(from.clone(), to.clone(), 512).expect("outbound");
...
#[test]
fn vm_distributed_transport_rejects_corrupt_or_mismatched_tetf_without_advancing() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut outbound =
        VmDistributedTransportSession::open(from.clone(), to.clone(), 512).expect("outbound");
...
#[test]
fn vm_distributed_transport_snapshot_preserves_message_and_delivery_continuity() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut outbound =
        VmDistributedTransportSession::open(from.clone(), to.clone(), 512).expect("outbound");
...
#[test]
fn vm_distributed_transport_restore_rejects_noncontiguous_inbound_history() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
...
#[test]
fn vm_distributed_transport_reports_out_of_order_then_accepts_gap_after_prior_frame() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut outbound =
        VmDistributedTransportSession::open(from.clone(), to.clone(), 512).expect("outbound");
...
#[test]
fn vm_distributed_transport_rejects_frame_for_wrong_session() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
...
#[test]
fn vm_distributed_transport_disconnect_blocks_encode_and_accept_until_reconnect() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut outbound =
        VmDistributedTransportSession::open(from.clone(), to.clone(), 512).expect("outbound");
...
#[test]
fn vm_distributed_transport_reconnect_preserves_pending_acks_and_rejects_wrong_remote() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let other = profile("other", "vm-c", "cluster-main", ["vm.message.send"]);
    let mut session =
...
#[test]
fn vm_distributed_transport_disconnect_blocks_ack_until_reconnect() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut session =
        VmDistributedTransportSession::open(from, to.clone(), 512).expect("session should open");
...
#[test]
fn vm_distributed_transport_rejects_incompatible_reconnect_profile() {
    let from = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let to = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
...
#[test]
fn vm_cluster_membership_joins_peer_with_deterministic_role_tags() {
...
#[test]
fn vm_cluster_membership_rejects_incompatible_peer_and_zero_timeout() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
...
#[test]
fn vm_cluster_membership_expires_and_recovers_unreachable_peer_by_heartbeat() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let peer = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
...
#[test]
fn vm_cluster_membership_keeps_fresh_peer_active_before_timeout() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let peer = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut membership = VmClusterMembership::new(local, 5).expect("membership should create");
    membership
...
#[test]
fn vm_cluster_membership_rejects_stale_left_and_fenced_heartbeats() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let peer = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut membership = VmClusterMembership::new(local, 5).expect("membership should create");
    membership
...
#[test]
fn vm_cluster_membership_rejects_fenced_leave() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let peer = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut membership = VmClusterMembership::new(local, 5).expect("membership should create");
    membership
...
#[test]
fn vm_cluster_membership_reports_unknown_node_operations() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let mut membership = VmClusterMembership::new(local, 5).expect("membership should create");
...
#[test]
fn vm_cluster_membership_prunes_only_expired_terminal_snapshots() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);

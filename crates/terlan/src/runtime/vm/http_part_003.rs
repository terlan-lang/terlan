...
fn poll_http1_tcp_exchange_with_connection(
    tcp: &mut VmTcpRuntime,
    stream: VmTcpStream,
    buffer: &mut VmHttpTcpRequestBuffer,
    close_connection: bool,
...
fn poll_http1_tls_tcp_exchange_with_connection(
    tcp: &mut VmTcpRuntime,
    tls_stream: &mut VmTlsTcpServerStream,
    buffer: &mut VmHttpTcpRequestBuffer,
    close_connection: bool,
...
fn poll_or_park_http1_tcp_exchange_with_connection(
    ...
    buffer: &mut VmHttpTcpRequestBuffer,
    close_connection: bool,
    response_memory: Option<&mut VmHttpResponseMemory>,
    handler: impl FnOnce(::http::Request<String>) -> Result<::http::Response<String>, String>,
) -> Result<VmHttpTcpActorPoll, String> {
...
fn poll_or_park_http1_tls_tcp_exchange_with_connection(
    processes: &mut VmProcessTable,
    tcp: &mut VmTcpRuntime,
    process: VmProcessId,
    tls_stream: &mut VmTlsTcpServerStream,
...
fn accept_http1_tls_tcp_handler(
    processes: &mut VmProcessTable,
    tcp: &mut VmTcpRuntime,
    tls: &VmTlsRuntime,
    listener: VmTcpListener,
...
fn request_wants_http1_close(request: &::http::Request<String>) -> bool {
    request
        .headers()
...
impl<'a> VmTcpReadStream<'a> {
    ...
    fn new(tcp: &'a mut VmTcpRuntime, stream: VmTcpStream) -> Self {
        Self {
            tcp,
            stream,
...
fn try_parse_http1_request_buffer(
    buffer: &[u8],
...
fn incomplete_http1_request_error(buffer: &[u8]) -> String {
...
fn parse_http1_request_headers(bytes: &[u8]) -> Result<ParsedHttpHeaders, String> {
...
fn parse_http1_response_content_length(
    bytes: &[u8],
    expected_status: u16,
) -> Result<usize, String> {
    let mut headers = [httparse::EMPTY_HEADER; 32];

...
use super::store::{
    FileBackedRecord, VmPersistentActorDurability, VmPersistentActorEvent, VmPersistentActorId,
    VmPersistentActorSchema, VmPersistentActorSnapshot,
};
use crate::runtime::vm::ReplValue;

#[cfg(test)]
pub(super) const DATABASE_BACKED_SQL_STATEMENTS: &[&str] = &[
...
pub(super) fn validate_database_table_name(table_name: &str) -> Result<(), String> {
...
#[cfg(test)]
pub(super) fn parse_database_backed_row_record(
    key: &str,
    value: &str,
) -> Result<FileBackedRecord, String> {
    parse_embedded_key_value_record(key, value)
}

pub(super) fn database_snapshot_row_key(actor_id: &VmPersistentActorId) -> String {
    embedded_snapshot_key(actor_id)
}

pub(super) fn database_event_row_key(actor_id: &VmPersistentActorId, sequence: u64) -> String {
    embedded_event_key(actor_id, sequence)
}

#[cfg(test)]
pub(super) fn parse_embedded_key_value_record(
    key: &str,
    value: &str,
) -> Result<FileBackedRecord, String> {
    ...
    match (
        ...
        (FileBackedRecord::Snapshot(snapshot), Some(encoded_actor), None) => {
            let expected = hex_encode(snapshot.actor_id.as_str().as_bytes());
        ...
        (FileBackedRecord::Event(event), None, Some(rest)) => {
            ...
            let expected_actor = hex_encode(event.actor_id.as_str().as_bytes());
...
pub(super) fn embedded_snapshot_key(actor_id: &VmPersistentActorId) -> String {
    format!("snapshot/{}", hex_encode(actor_id.as_str().as_bytes()))
}

pub(super) fn embedded_event_key(actor_id: &VmPersistentActorId, sequence: u64) -> String {
    format!(
        "event/{}/{sequence:020}",
        hex_encode(actor_id.as_str().as_bytes())
    )
}

pub(super) fn encode_snapshot_record(
    snapshot: &VmPersistentActorSnapshot,
) -> Result<String, String> {
    Ok(format!(
        "snapshot\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}",
        hex_encode(snapshot.actor_id.as_str().as_bytes()),
...
pub(super) fn encode_event_record(event: &VmPersistentActorEvent) -> Result<String, String> {
    Ok(format!(
        "event\t{}\t{}\t{}\t{}\t{}",
        hex_encode(event.actor_id.as_str().as_bytes()),
...
pub(super) fn parse_file_backed_record(line: &str) -> Result<FileBackedRecord, String> {
        ...
        ["snapshot", actor, schema_id, schema_version, generation, last_event_sequence, state, mailbox, timers, resources] => {
            Ok(FileBackedRecord::Snapshot(VmPersistentActorSnapshot::new(
                ...
                VmPersistentActorDurability {
                    resource_handles: parse_strings(resources)?,
                    last_event_sequence: parse_u64(last_event_sequence, "last event sequence")?,
                },
            )?))
        }
...
pub(super) fn encode_repl_values(values: &[ReplValue]) -> Result<String, String> {
    values
        .iter()
...
pub(super) fn encode_u64_values(values: &[u64]) -> String {
    values
        .iter()
        .map(u64::to_string)
        .collect::<Vec<_>>()
...
pub(super) fn encode_strings(values: &[String]) -> String {
    values
        .iter()
...
pub(super) fn encode_repl_value(value: &ReplValue) -> Result<String, String> {
    match value {
...
pub(super) fn parse_repl_values(value: &str) -> Result<Vec<ReplValue>, String> {
    if value.is_empty() {
        return Ok(Vec::new());
    }
...
pub(super) fn parse_u64_values(value: &str) -> Result<Vec<u64>, String> {
    if value.is_empty() {
        return Ok(Vec::new());
    }
    value
...
pub(super) fn parse_strings(value: &str) -> Result<Vec<String>, String> {
    if value.is_empty() {
        return Ok(Vec::new());
    }
...
pub(super) fn parse_repl_value(value: &str) -> Result<ReplValue, String> {
    if value == "u" {
...
pub(super) fn parse_i64(value: &str, label: &str) -> Result<i64, String> {
    value
        .parse::<i64>()
        .map_err(|err| format!("invalid {label} `{value}`: {err}"))
}
...
pub(super) fn hex_decode_string(value: &str) -> Result<String, String> {
...
pub(super) fn hex_value(byte: u8) -> Result<u8, String> {
    match byte {
        b'0'..=b'9' => Ok(byte - b'0'),
...
pub(super) fn temporary_file_backed_path(path: &Path) -> PathBuf {

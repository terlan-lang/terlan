...
/// Durable resources and event cursor retained with one snapshot.
pub(crate) struct VmPersistentActorDurability {
    pub(crate) resource_handles: Vec<String>,
    pub(crate) last_event_sequence: u64,
}
...
impl VmPersistentActorSnapshot {
    pub(crate) fn new(
        ...
        durability: VmPersistentActorDurability,
    ) -> Result<Self, String> {
        let VmPersistentActorDurability {
            resource_handles,
            last_event_sequence,
        } = durability;
        if generation == 0 {
...
pub(crate) enum VmPersistentActorStoreOutcome {
    SnapshotStored(Box<VmPersistentActorSnapshot>),
...
impl VmPersistentActorStoreAdapter for VmInMemoryPersistentActorStore {
    fn store_snapshot(
        ...
        VmPersistentActorStoreOutcome::SnapshotStored(Box::new(snapshot))
    }
...
#[cfg(any(test, feature = "benchmark-tools"))]
pub(super) enum FileBackedRecord {
    Snapshot(VmPersistentActorSnapshot),
    Event(VmPersistentActorEvent),
}

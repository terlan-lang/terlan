...
#[test]
pub(super) fn vm_cluster_membership_rejects_zero_stale_retention() {
...
#[test]
pub(super) fn vm_cluster_membership_prunes_expired_unreachable_snapshot() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
...
#[test]
pub(super) fn vm_cluster_membership_restart_requires_new_unfenced_incarnation() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let peer = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
...
#[test]
pub(super) fn vm_coordination_profile_rejects_epoch_overflow() {
...
#[test]
pub(super) fn vm_cluster_membership_partition_and_heal_enforce_lifecycle() {
    let local = profile("docs", "vm-a", "cluster-main", ["vm.message.send"]);
    let peer = profile("search", "vm-b", "cluster-main", ["vm.message.send"]);
    let mut membership = VmClusterMembership::new(local, 5).expect("membership should build");
    membership
...
pub(super) fn profile<const N: usize>(
    app_id: &str,
    vm_id: &str,
    cluster_id: &str,
    capabilities: [&str; N],

...
use super::{
    ...
    VmDistributedStorageSnapshot, VmDistributedStorageSnapshotIsolationProof,
    VmDistributedStorageTransactionalBatchProof, VmDistributedStorageTransactionalRollbackProof,
};

#[test]
fn vm_distributed_storage_force_local_writes_flushes_and_loads_snapshot() {
...
#[test]
fn vm_distributed_storage_rejects_stale_snapshot_replay() {
...
#[test]
fn vm_distributed_storage_compacts_old_snapshots_deterministically() {
    let mut adapter = opened_force_local_adapter();
    for sequence in 1..=4 {
...
#[test]
fn vm_distributed_storage_compaction_physically_removes_pruned_snapshots_and_retains_boundary() {
    let mut adapter = opened_force_local_adapter();
    for sequence in 1..=4 {
        let id = format!("checkpoint-{sequence}");
        assert!(matches!(
...
#[test]
fn vm_distributed_storage_compaction_preserves_monotonic_sequence_watermark() {
    let mut adapter = opened_force_local_adapter();
    assert!(matches!(
...
#[test]
fn vm_distributed_storage_detects_corrupt_snapshot_checksum() {
    let mut adapter = opened_force_local_adapter();
...
#[test]
fn vm_distributed_storage_reports_finalize_and_partial_write_failures() {
    let mut adapter = opened_force_local_adapter();
...
#[test]
fn vm_distributed_storage_atomic_append_proof_preserves_sequence_on_failed_append() {
    let mut adapter = opened_force_local_adapter();
    assert_eq!(
...
#[test]
fn vm_distributed_storage_snapshot_isolation_proof_survives_later_compaction() {
    let mut adapter = opened_force_local_adapter();
...
#[test]
fn vm_distributed_storage_durable_flush_proof_advances_only_after_successful_flush() {
    let mut adapter = opened_force_local_adapter();
    assert_eq!(
...
#[test]
fn vm_distributed_storage_transactional_batch_rejects_partial_commit_without_mutation() {
    let mut adapter = opened_force_local_adapter();
    assert_eq!(
...
#[test]
fn vm_distributed_storage_durable_transactional_batch_rollback_preserves_commit_boundary() {

...
#[test]
pub(super) fn vm_websocket_runtime_close_session_and_stream_sends_close_for_open_session() {
...
#[test]
pub(super) fn vm_websocket_runtime_timeout_termination_sends_close_and_reason() {
    let (mut tcp, client, server) = connected_tcp_pair();
    let mut runtime = VmWebSocketRuntime::new();
    let session = runtime.open_session(server);
...
///   timeout shutdown.
#[test]
pub(super) fn vm_websocket_runtime_cancelled_termination_cancels_stream_without_close_frame() {
    let (mut tcp, client, server) = connected_tcp_pair();
    let mut runtime = VmWebSocketRuntime::new();
    let session = runtime.open_session(server);
...
#[test]
pub(super) fn vm_websocket_runtime_close_session_and_stream_skips_close_for_closed_session() {
    let (mut tcp, client, server) = connected_tcp_pair();
    let mut runtime = VmWebSocketRuntime::new();
    let session = runtime.open_session(server);
...
#[test]
pub(super) fn vm_websocket_runtime_close_session_and_stream_reports_invalid_stream_release() {
...
#[test]
pub(super) fn vm_websocket_runtime_close_stream_session_and_stream_closes_bound_session() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_websocket_runtime_close_stream_session_and_stream_ignores_unbound_stream() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_websocket_runtime_close_sessions_and_streams_handles_selected_sessions() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_websocket_runtime_close_sessions_rejects_duplicate_without_partial_close() {
    let (mut tcp, client, server) = connected_tcp_pair();
    let mut runtime = VmWebSocketRuntime::new();
    let session = runtime.open_session(server);
...
#[test]
pub(super) fn vm_websocket_runtime_best_effort_close_sessions_reports_partial_results() {
    let (mut tcp, client, server) = connected_tcp_pair();
    let mut runtime = VmWebSocketRuntime::new();
    let session = runtime.open_session(server);
...
///   references.
#[test]
pub(super) fn vm_websocket_runtime_best_effort_close_sessions_reports_duplicates() {
    let (mut tcp, client, server) = connected_tcp_pair();
    let mut runtime = VmWebSocketRuntime::new();
    let session = runtime.open_session(server);
...
///   accepted.
#[test]
pub(super) fn vm_websocket_runtime_close_all_sessions_is_empty_for_empty_runtime() {
    let mut tcp = VmTcpRuntime::new();
    let mut runtime = VmWebSocketRuntime::new();
...
#[test]
pub(super) fn vm_websocket_runtime_close_all_sessions_closes_every_tracked_session() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_websocket_runtime_removes_sessions_and_rejects_unknown_handles() {
...
pub(super) struct TestWebSocketMemoryStream {
    read: Cursor<Vec<u8>>,
    written: Vec<u8>,
}
...
pub(super) fn encode_client_text_frame(text: &str) -> Vec<u8> {
...
pub(super) fn encode_client_binary_frame(bytes: &[u8]) -> Vec<u8> {
    let stream = TestWebSocketMemoryStream::new(Vec::new());
    let mut socket = WebSocket::from_raw_socket(stream, Role::Client, None);
    socket
...
pub(super) fn encode_client_control_frame(frame: VmWebSocketControlFrame) -> Vec<u8> {
    encode_test_control_frame(Role::Client, frame)
}

/// Decodes a server frame with tungstenite acting as the client peer.
pub(super) fn decode_server_text_frame(frame: &[u8]) -> String {
...
pub(super) fn decode_server_control_frame(frame: &[u8]) -> VmWebSocketControlFrame {
    let stream = TestWebSocketMemoryStream::new(frame.to_vec());
    let mut socket = WebSocket::from_raw_socket(stream, Role::Client, None);
    match socket.read().expect("client read") {
...
pub(super) fn encode_test_control_frame(role: Role, frame: VmWebSocketControlFrame) -> Vec<u8> {
    let stream = TestWebSocketMemoryStream::new(Vec::new());
...
pub(super) fn write_websocket_tls_cert_pair(
    name: &str,
...
pub(super) fn websocket_tls_manual_plan(cert_path: String, key_path: String) -> VmTlsPlan {
...
pub(super) fn websocket_tls_client_for_cert(cert_der: Vec<u8>) -> ClientConnection {
    crate::runtime::vm::tls_test_support::client_for_cert(cert_der)
}

/// Flushes pending rustls client bytes into VM TCP.
pub(super) fn flush_websocket_tls_client_to_tcp(
...
pub(super) fn pump_websocket_tls_tcp_to_client(
    tcp: &mut VmTcpRuntime,
    client_stream: crate::runtime::vm::tcp::VmTcpStream,
    client: &mut ClientConnection,
) {
...
pub(super) fn complete_websocket_tls_tcp_handshake(
    client: &mut ClientConnection,
    tcp: &mut VmTcpRuntime,
    client_stream: crate::runtime::vm::tcp::VmTcpStream,
...
pub(super) fn read_websocket_tls_client_plaintext(
    tcp: &mut VmTcpRuntime,
    client_stream: crate::runtime::vm::tcp::VmTcpStream,
    client: &mut ClientConnection,
) -> String {
...
pub(super) fn connected_tcp_pair() -> (
    VmTcpRuntime,
...
pub(super) fn connected_tcp_pair_at(
    tcp: &mut VmTcpRuntime,
    address: &str,
) -> (
    crate::runtime::vm::tcp::VmTcpStream,

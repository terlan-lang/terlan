...
use super::super::test_support::{ChunkedReader, FailingReader};
use super::super::{
    build_http_fairness_replay_seed, handle_http1_in_memory_exchange,
    incomplete_http1_request_error, parse_http1_request_headers, read_http1_request,
    read_http1_response, render_http_template_response, write_http1_response, VmHttpQueue,
    VmHttpQueueMetrics, VmHttpTcpServer, VmHttpTcpServerInfo, VmHttpTcpServerPoll,
...
pub(super) fn arithmetic_response_from_query(
    query: &str,
...
pub(super) fn query_int(query: &str, name: &str) -> Result<i64, String> {
    query
        .split('&')
...
pub(super) fn plain_tls_plan() -> VmTlsPlan {
    crate::runtime::vm::tls_test_support::plain_plan()
}

pub(super) fn manual_tls_plan() -> VmTlsPlan {
    VmTlsPlan {
...
pub(super) fn manual_tls_plan_with_paths(cert_path: String, key_path: String) -> VmTlsPlan {
    crate::runtime::vm::tls_test_support::manual_plan_with_paths(cert_path, key_path)
}

pub(super) fn write_http_tls_cert_pair(
    name: &str,
...
pub(super) fn http_tls_client_for_cert(cert_der: Vec<u8>) -> ClientConnection {
    crate::runtime::vm::tls_test_support::client_for_cert(cert_der)
}

pub(super) fn flush_http_client_tls_to_tcp(
...
pub(super) fn pump_http_tcp_to_tls_client(
    tcp: &mut VmTcpRuntime,
    client_stream: VmTcpStream,
    client: &mut ClientConnection,
) {
...
pub(super) fn complete_http_tls_handshake(
    client: &mut ClientConnection,
    tcp: &mut VmTcpRuntime,
    client_stream: VmTcpStream,
...
pub(super) fn complete_retained_http_tls_server_handshake(
...
pub(super) fn read_retained_http_tls_response(
    tcp: &mut VmTcpRuntime,
    client_stream: VmTcpStream,
    client: &mut ClientConnection,
) -> String {
...
#[test]
pub(super) fn vm_http_queue_rejects_zero_capacity() {
...
#[test]
pub(super) fn vm_http_queue_preserves_fifo_order_and_metrics() {
...
#[test]
pub(super) fn vm_http_queue_blocks_enqueue_until_consumer_frees_capacity() {
    ...
    let producer_park_deadline = Instant::now() + Duration::from_secs(1);
    loop {
        if queue
            .metrics()
            .expect("producer pressure metrics should read")
            .parked_producers
            == 1
        {
            break;
        ...
        assert!(
            Instant::now() < producer_park_deadline,
            "producer did not park on the full queue"
        );
        thread::yield_now();
    }
    assert_eq!(queue.dequeue().expect("first dequeue"), 1);
...
#[test]
pub(super) fn vm_http_queue_blocks_dequeue_until_producer_adds_item() {
    let queue = Arc::new(VmHttpQueue::new(1).expect("queue should be created"));

    let consumer_queue = Arc::clone(&queue);
    let consumer = thread::spawn(move || consumer_queue.dequeue().expect("dequeue should unblock"));

    let consumer_park_deadline = Instant::now() + Duration::from_secs(1);
    loop {
        if queue
            .metrics()
            .expect("consumer pressure metrics should read")
            .parked_consumers
            == 1
        {
            break;
        ...
        assert!(
            Instant::now() < consumer_park_deadline,
            "consumer did not park on the empty queue"
        );
        thread::yield_now();
    }
...
#[test]
pub(super) fn vm_http_fairness_replay_seed_captures_queue_and_server_counters() {
...
#[test]
pub(super) fn vm_http_fairness_replay_seed_rejects_empty_labels() {
...
#[test]
pub(super) fn vm_http_reads_http1_request_with_headers_query_and_body() {
...
#[test]
pub(super) fn vm_http_reads_fragmented_request_body() {
...
#[test]
pub(super) fn vm_http_rejects_partial_request_headers() {
...
#[test]
pub(super) fn vm_http_rejects_oversized_request_headers() {
...
#[test]
pub(super) fn vm_http_rejects_early_request_body_eof() {
...
#[test]
pub(super) fn vm_http_incomplete_request_classifier_falls_back_after_complete_body_bytes() {
    let error =
...
#[test]
pub(super) fn vm_http_incomplete_request_classifier_falls_back_after_malformed_headers() {
...
#[test]
pub(super) fn vm_http_rejects_non_utf8_request_body() {
...
#[test]
pub(super) fn vm_http_rejects_malformed_request_headers() {
...
#[test]
pub(super) fn vm_http_request_header_parser_reports_partial_headers() {
...
#[test]
pub(super) fn vm_http_reports_request_read_error() {
...
#[test]
pub(super) fn vm_http_rejects_invalid_request_content_length() {
    let mut wire = "POST / HTTP/1.1\r\nContent-Length: nope\r\n\r\n".as_bytes();
...
#[test]
pub(super) fn vm_http_rejects_oversized_request_body_declaration() {
...
#[test]
pub(super) fn vm_http_writes_http1_response_with_connection_policy() {
...
#[test]
pub(super) fn vm_http_writes_close_connection_policy() {
    let response = http::Response::builder()
        .status(200)
        .body("ok".to_string())
        .expect("response should build");
...
#[test]
pub(super) fn vm_http_writes_explicit_connection_upgrade_policy() {
    let response = http::Response::builder()
        .status(101)
...
#[test]
pub(super) fn vm_http_writes_explicit_content_length_policy() {
    let response = http::Response::builder()
        .status(200)
...
#[test]
pub(super) fn vm_http_in_memory_exchange_runs_typed_handler_and_writes_response() {
...
#[test]
pub(super) fn vm_http_in_memory_exchange_renders_template_response_through_handler_dispatch() {
...
#[test]
pub(super) fn vm_http_in_memory_exchange_runs_request_driven_arithmetic_handler() {
...
#[test]
pub(super) fn vm_http_in_memory_exchange_reports_malformed_request_before_handler() {
...
#[test]
pub(super) fn vm_http_in_memory_exchange_reports_request_driven_handler_error() {
...
#[test]
pub(super) fn vm_http_roundtrips_request_and_response_over_vm_tcp_streams() {
...
#[test]
pub(super) fn vm_http_tcp_server_reports_plaintext_transport_mode() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_http_tcp_server_reports_tls_transport_mode() {
    let mut tcp = VmTcpRuntime::new();
...
#[test]
pub(super) fn vm_http_tcp_server_reports_missing_transport_plan() {
    let mut tcp = VmTcpRuntime::new();

...
#[test]
pub(super) fn vm_http_tcp_exchange_runs_handler_and_sends_response_over_stream() {
...
#[test]
pub(super) fn vm_http_tcp_poll_exchange_waits_for_fragmented_request_then_completes() {
    let mut tcp = VmTcpRuntime::new();
    let listener = tcp.listen("http.local").expect("listen");
    let client = tcp.connect("http.local", "client").expect("connect");
    let server = tcp
...
#[test]
pub(super) fn vm_http_request_body_stream_dispatches_ordered_bounded_chunks() {
    let mut wire =
...
///   the serve adapter.
#[test]
pub(super) fn vm_http_tcp_poll_exchange_rejects_half_closed_partial_headers() {
    let mut tcp = VmTcpRuntime::new();
    let listener = tcp.listen("http.local").expect("listen");
    let client = tcp.connect("http.local", "client").expect("connect");
    let server = tcp
...
///   `terlc serve`.
#[test]
pub(super) fn vm_http_tcp_poll_exchange_rejects_half_closed_truncated_body() {
    let mut tcp = VmTcpRuntime::new();
    let listener = tcp.listen("http.local").expect("listen");
    let client = tcp.connect("http.local", "client").expect("connect");
    let server = tcp
...
#[test]
pub(super) fn vm_http_tcp_poll_exchange_rejects_buffered_truncated_body_after_late_eof() {
    let mut tcp = VmTcpRuntime::new();
    let listener = tcp.listen("http.local").expect("listen");
    let client = tcp.connect("http.local", "client").expect("connect");
    let server = tcp
...
#[test]
pub(super) fn vm_http_tls_tcp_exchange_parses_decrypted_request_and_encrypts_response() {
...
#[test]
pub(super) fn vm_http_tls_tcp_exchange_completes_from_buffered_plaintext_before_transport_poll() {
...
#[test]
pub(super) fn vm_http_tls_tcp_exchange_waits_for_fragmented_encrypted_request_then_completes() {
...
#[test]
pub(super) fn vm_http_tcp_keep_alive_exchange_preserves_pipelined_request_bytes() {
    let mut tcp = VmTcpRuntime::new();
    let listener = tcp.listen("http.local").expect("listen");
    let client = tcp.connect("http.local", "client").expect("connect");
    let server = tcp
...
#[test]
pub(super) fn vm_http_tcp_actor_poll_parks_then_wakes_through_tcp_scheduler_adapter() {
...
#[test]
pub(super) fn vm_http_tls_tcp_actor_poll_parks_then_wakes_through_tcp_scheduler_adapter() {
    ...
    let first = poll_or_park_http1_tls_tcp_exchange_with_connection(
        &mut tcp,
        &mut tls_stream,
        VmHttpActorExchange::without_response_memory(&mut processes, handler, &mut buffer, true),
...
#[test]
pub(super) fn vm_http_tls_tcp_actor_poll_rejects_missing_and_exited_handler_processes() {
    ...
    assert_eq!(
        poll_or_park_http1_tls_tcp_exchange_with_connection(
            &mut tcp,
            &mut tls_stream,
            VmHttpActorExchange::without_response_memory(
                &mut processes,
                missing,
                &mut missing_buffer,
                true,
...
#[test]
pub(super) fn vm_http_tcp_actor_poll_rejects_missing_and_exited_handler_processes() {
    let mut processes = VmProcessTable::default();

//! Canonical native boundary value identities.

/// Stable NativeBoundary value tags.
...
impl TvmBoundaryType {
    ...
    pub(crate) fn transition_words(&self) -> [i64; 3] {
        match self {
            Self::Unit => [0, 0, 0],
            Self::Bool => [1, 0, 0],
            Self::Int => [2, 0, 0],
    ...
    pub(crate) fn from_transition_words(words: &[i64]) -> Result<Self, String> {
        let [tag, low, high] = words else {
            return Err(format!(
                "error[tvm.transition.boundary_type]: expected 3 type words, received {}",
                words.len()
            ));
        };
        let scalar = |value| {
            if *low == 0 && *high == 0 {
                ...
                Err(
                    "error[tvm.transition.boundary_type]: scalar type has nonzero identity words"
                        .to_string(),
                )
            }
        };
        match *tag {
            ...
            7 => Err(
                "error[tvm.transition.boundary_type]: native resource has a nonzero high word"
                    .to_string(),
            ),
            8 => scalar(Self::Atom),
            9 => scalar(Self::Bytes),
            ...
            tag => Err(format!(
                "error[tvm.transition.boundary_type]: unknown type tag {tag}"
            )),
        }
    }
}

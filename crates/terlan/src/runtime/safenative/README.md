...
- Keep concrete native-backed resources, such as Vector, under
  `runtime/native` so SafeNative stays focused on bridge and safety contracts.
...
- `runtime/native`: concrete Rust-native adapters used through SafeNative
  bridge dispatch.
...
- Keep tests focused on value transformation, invalid input, and state
  transition behavior.
- Run `cargo test -p terlan terlan_safenative::` for this feature area.

...
///   unwrap/expect.
fn dispatch_ok(operation: &str, args: &[NativeBoundaryValue]) -> Option<NativeBoundaryValue> {
    let result = dispatch(operation, args);
    assert!(result.is_ok());
    result.ok()
}

fn expected_postgres_unreachable_code() -> &'static str {
...
#[test]
fn bridge_dispatch_json_keeps_atom_like_object_keys_as_text() {
    let mut store = ResourceStore::new();
    let Some(NativeBoundaryBridgeValue::Handle(root)) = bridge_dispatch_ok(
        &mut store,
        "std.data.json.parse",
...
#[test]
fn adversarial_native_boundary_dispatch_rejects_cross_resource_handle_confusion() {
    let mut store = ResourceStore::new();
    let Some(NativeBoundaryBridgeValue::Handle(vector)) = bridge_dispatch_ok(
        &mut store,
        "std.native.collections.vector.from_list",
...
#[test]
fn dispatch_http_stream_response_requires_vm_runtime() {
    let error = dispatch(

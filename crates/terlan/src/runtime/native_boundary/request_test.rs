...
///
/// Transformation:
/// - Wraps the raw integer in the NativeBoundary request id type.
...
/// Verifies a pending request can be cancelled only by matching id.
///
/// Inputs:
/// - Pending state with matching and mismatched request ids.
///
/// Output:
/// - Test passes when only the matching pending request becomes cancelled.
///
/// Transformation:
/// - Exercises the async cancellation rule used before a native reply arrives.
#[test]
fn cancel_request_requires_matching_pending_id() {
    let id = request_id(10);

    assert_eq!(
        cancel_request(RequestState::Pending(id), id),
        Some(RequestState::Cancelled(id))
    );
    assert_eq!(
        cancel_request(RequestState::Pending(id), request_id(11)),
        None
    );
    assert_eq!(cancel_request(RequestState::Idle, id), None);
    assert_eq!(cancel_request(RequestState::Completed(id), id), None);
}

/// Verifies a pending request can time out only by matching id.
///
/// Inputs:
/// - Pending state with matching and mismatched request ids.
///
/// Output:
/// - Test passes when only the matching pending request becomes timed out.
///
/// Transformation:
/// - Exercises the async timeout rule used before a native reply arrives.
#[test]
fn timeout_request_requires_matching_pending_id() {
    let id = request_id(10);

    assert_eq!(
        timeout_request(RequestState::Pending(id), id),
        Some(RequestState::TimedOut(id))
    );
    assert_eq!(
        timeout_request(RequestState::Pending(id), request_id(11)),
        None
    );
    assert_eq!(timeout_request(RequestState::Idle, id), None);
    assert_eq!(timeout_request(RequestState::Cancelled(id), id), None);
}

/// Verifies cancelled and timed-out requests cannot complete later.
///
/// Inputs:
/// - Terminal cancellation and timeout states.
///
/// Output:
/// - Test passes when both reject completion and no longer count as pending.
///
/// Transformation:
/// - Prevents late native replies from reviving actor-visible async work.
#[test]
fn terminal_async_request_states_reject_late_completion() {
    let id = request_id(10);

    assert_eq!(complete_request(RequestState::Cancelled(id), id), None);
    assert_eq!(complete_request(RequestState::TimedOut(id), id), None);
    assert!(!is_pending(RequestState::Cancelled(id), id));
    assert!(!is_pending(RequestState::TimedOut(id), id));
}

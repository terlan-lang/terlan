//! Request lifecycle accounting for NativeBoundary command/reply bridges.
//!
...
/// Identifier assigned to one NativeBoundary command.
...
pub enum RequestState {
    ...
    Completed(RequestId),
    /// A request was cancelled while pending.
    Cancelled(RequestId),
    /// A request timed out while pending.
    TimedOut(RequestId),
}
...
pub fn start_request(state: RequestState, request_id: RequestId) -> Option<RequestState> {
    match state {
        ...
        | RequestState::Cancelled(_)
        | RequestState::TimedOut(_) => None,
    }
}
...
/// Cancels a pending request with a matching request id.
///
/// Inputs:
/// - `state`: current lifecycle state.
...
///
/// Output:
/// - `Some(Cancelled(request_id))` when the state is pending for that id.
/// - `None` when the state is idle, completed, already cancelled, timed out,
///   or pending for another id.
///
/// Transformation:
/// - Converts only a matching `Pending(request_id)` into
///   `Cancelled(request_id)`.
pub fn cancel_request(state: RequestState, request_id: RequestId) -> Option<RequestState> {
    match state {
        RequestState::Pending(pending_id) if pending_id == request_id => {
            Some(RequestState::Cancelled(request_id))
        }
        RequestState::Idle
        | RequestState::Pending(_)
        | RequestState::Completed(_)
...
/// Times out a pending request with a matching request id.
///
/// Inputs:
/// - `state`: current lifecycle state.
/// - `request_id`: request id to time out.
///
/// Output:
/// - `Some(TimedOut(request_id))` when the state is pending for that id.
/// - `None` when the state is idle, completed, already cancelled, timed out,
///   or pending for another id.
///
/// Transformation:
/// - Converts only a matching `Pending(request_id)` into
///   `TimedOut(request_id)`.
pub fn timeout_request(state: RequestState, request_id: RequestId) -> Option<RequestState> {
    match state {
        RequestState::Pending(pending_id) if pending_id == request_id => {
            Some(RequestState::TimedOut(request_id))
        }
        RequestState::Idle
        | RequestState::Pending(_)
        | RequestState::Completed(_)

...
use crate::terlan_native_boundary::dispatch::DispatchError;
...
fn handle_fixture() -> NativeBoundaryHandle {
...
///
/// Inputs:
/// - Representative unit, text, bytes, integer, float, and bool values.
///
/// Output:
...
fn primitive_values_round_trip_through_terms() {
    let values = [
        ...
        NativeBoundaryBridgeValue::Text(String::from("hello")),
        NativeBoundaryBridgeValue::Bytes(vec![0, 127, 255]),
        NativeBoundaryBridgeValue::Int(7),
        NativeBoundaryBridgeValue::Float(1.5),
...
fn handle_value_round_trips_through_term_fields() {
    ...
    let value = NativeBoundaryBridgeValue::Handle(handle);
...
fn optional_values_round_trip_through_terms() {
    let values = [
        NativeBoundaryBridgeValue::OptionalText(Some(String::from("name"))),
        NativeBoundaryBridgeValue::OptionalText(None),
        NativeBoundaryBridgeValue::OptionalHandle(Some(handle_fixture())),
        NativeBoundaryBridgeValue::OptionalHandle(None),
    ];
...
///   call `std.db.postgres.connect` through `NativeBoundaryRuntime`.
#[test]
fn postgres_config_round_trips_through_terms() {
    ...
    let value = NativeBoundaryBridgeValue::PostgresConfig(config);
...
fn argument_lists_round_trip_in_order() {
    let args = vec![
        NativeBoundaryBridgeValue::Handle(handle_fixture()),
        NativeBoundaryBridgeValue::Text(String::from("key")),
...
fn successful_dispatch_reply_decodes_to_value() {
    let value = NativeBoundaryBridgeValue::Text(String::from("done"));

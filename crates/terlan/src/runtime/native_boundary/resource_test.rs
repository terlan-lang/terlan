...
/// Validates process-owned resources reject cross-process access.
#[test]
fn process_owned_resource_rejects_non_owner() {
    let mut store = ResourceStore::new();
    let Some(handle) = store.insert_for_owner(7, json_resource()).ok() else {
        return;
    };

    assert_eq!(store.validate_owner(handle, 7), Ok(()));
    let error = store
        .validate_owner(handle, 8)
        .err()
...
/// Validates unauthorized disposal leaves the owner resource live.
#[test]
fn process_owned_resource_rejects_non_owner_disposal() {
    let mut store = ResourceStore::new();
    let Some(handle) = store.insert_for_owner(7, json_resource()).ok() else {
        return;
    };

...
#[test]
fn definition_locations_resolve_imported_constructor_reference() -> std_io::Result<()> {
...
#[test]
fn definition_locations_resolve_imported_trait_reference() -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();
...
fn completion_doc_text(item: &CompletionItem) -> &str {

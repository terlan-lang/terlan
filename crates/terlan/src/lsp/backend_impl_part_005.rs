impl Backend {
    ...
    fn provider_definition_candidates(current_dir: &Path, module_name: &str) -> Vec<PathBuf> {
    ...
    fn identifier_bounds_at_byte_offset(
        text: &str,
        byte_offset: usize,
    ...
    ///   typechecker.
    fn is_receiver_member_reference(text: &str, identifier_start: usize) -> bool {
        text.as_bytes()
    ...
    fn find_child_symbol_selection_range(
    ...
    fn find_receiver_method_selection_range(
        symbols: &[DocumentSymbol],
        name: &str,
    ) -> Option<Range> {
    ...
    fn receiver_method_candidates(symbols: &[DocumentSymbol], name: &str) -> Vec<(u8, Range)> {
    ...
    fn receiver_method_priority(detail: Option<&str>) -> Option<u8> {
        match detail {
    ...
    fn declaration_symbol_children(
        text: &str,
        payload: &SyntaxDeclarationPayload,
    ...
    fn nested_constant_document_symbol(
        text: &str,
        name: &str,
        detail: &str,
        span: &crate::terlan_syntax::ebnf::EbnfSourceSpan,
    ...
    fn struct_field_document_symbol(text: &str, field: &SyntaxStructFieldOutput) -> DocumentSymbol {
    ...
    fn trait_method_document_symbol(
        text: &str,
    ...
    fn impl_method_document_symbol(text: &str, method: &SyntaxImplMethodOutput) -> DocumentSymbol {
        let source_span = Span::new(method.span.start, method.span.end);
        let range = OpenDocument::range_from_span(text, &source_span);
        let selection_range =
            Self::symbol_selection_range(text, &source_span, &method.name).unwrap_or(range);
    ...
    fn raw_shape_symbol_parts(raw_kind: &str, text: &str) -> Option<(String, String, SymbolKind)> {
        if raw_kind != "shape" {
            return None;
        }
    ...
    fn visibility_detail(is_public: bool, label: &str) -> String {
        if is_public {
            format!("pub {label}")
        } else {
            label.to_string()

...
#[test]
fn diagnostic_import_action_inserts_std_constructor_selected_fallbacks() {
    let cases = [
        (
            "Some",
...
#[test]
fn import_candidate_skips_already_selected_function_import() -> std_io::Result<()> {
...
#[test]
fn import_candidate_skips_function_import_when_wildcard_selected() -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();
...
#[test]
fn import_candidate_keeps_source_name_candidate_when_existing_import_uses_alias(
) -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
...
#[test]
fn import_candidate_keeps_ambiguous_function_choices() -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();

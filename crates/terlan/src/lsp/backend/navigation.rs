...
impl Backend {
    ...
    pub(in super::super) fn simple_call_argument_positions(
        line: &str,
        open_paren: usize,
        close_paren: usize,
    ) -> Vec<usize> {
        ...
        for (offset, byte) in bytes[open_paren + 1..close_paren.min(bytes.len())]
            .iter()
            .copied()
            .enumerate()
        {
    ...
    pub(in super::super) fn let_literal_inlay_hint_for_line(
        line: &str,
        line_number: u32,
    ) -> Option<InlayHint> {
    ...
    ///   chain.
    pub(in super::super) fn assignment_literal_inlay_hint_for_line(
        line: &str,
        line_number: u32,
    ) -> Option<InlayHint> {
        let leading_bytes = line.len() - line.trim_start().len();
    ...
    pub(in super::super) fn literal_binding_inlay_hint(
        line_number: u32,
        character: u32,
        type_name: &str,
    ) -> InlayHint {
    ...
    pub(in super::super) fn literal_type_name(value: &str) -> Option<&'static str> {
    ...
    pub(in super::super) fn reference_locations_for_position(
        ...
        if let Some((_, index)) = Self::binding_navigation(document) {
            if let Some(occurrence) = index.occurrence_at(byte_offset) {
                return index
                    .occurrences_for(occurrence.binding)
                    .into_iter()
                    .map(|occurrence| {
                        Location::new(
                            uri.clone(),
                            OpenDocument::range_from_span(&document.text, &occurrence.span),
                        )
                    })
                    .collect();
            }
    ...
    pub(in super::super) fn is_reference_declaration_range(
        document: &OpenDocument,
        range: Range,
    ) -> bool {
        ...
        if let Some((_, index)) = Self::binding_navigation(document) {
            if index.is_declaration(Span::new(start, end)) {
                return true;
            }
        }
    ...
    /// Builds compiler-owned binding navigation for the current source snapshot.
    pub(in super::super) fn binding_navigation(
        document: &OpenDocument,
    ) -> Option<(BindingAnalysis, BindingNavigationIndex)> {
        let module = document.parse_syntax().ok()?;
        let (module, macro_diagnostics) = expand_syntax_raw_macros(module);
        if macro_diagnostics
            .iter()
            .any(|diagnostic| matches!(diagnostic.severity, DiagSeverity::Error))
        {
            return None;
        }
        let analysis = analyze_syntax_bindings(&module);
        let index = BindingNavigationIndex::build(&document.text, &analysis);
        Some((analysis, index))
    }

    /// Checks that a rename replacement is exactly one non-keyword identifier.
    pub(in super::super) fn valid_rename_identifier(name: &str) -> bool {
        let Ok(tokens) = lex(name) else {
            return false;
        };
        matches!(
            tokens.as_slice(),
            [token, eof]
                if matches!(token.kind, TokenKind::Atom | TokenKind::Var)
    ...
    pub(in super::super) fn is_declaration_identifier_span(
        text: &str,
    ...
    pub(in super::super) fn definition_locations_for_position(
        ...
        if let Some((_, index)) = Self::binding_navigation(document) {
            if let Some(occurrence) = index.occurrence_at(byte_offset) {
                if let Some(declaration) = index.declaration_for(occurrence.binding) {
                    return vec![Location::new(
                        uri.clone(),
                        OpenDocument::range_from_span(&document.text, &declaration.span),
                    )];
                }
            }
        }
        ...
        let symbols = Self::document_symbols_for_document(document);
    ...
    pub(in super::super) fn receiver_identifier_before_member(
        text: &str,
        member_start: usize,
    ) -> Option<String> {
    ...
    pub(in super::super) fn parameter_type_for_name(
        module: &SyntaxModuleOutput,
        name: &str,
    ) -> Option<String> {
    ...
    pub(in super::super) fn selected_import_for_type(
        module: &SyntaxModuleOutput,
        type_name: &str,
    ...
    pub(in super::super) fn interface_exports_identifier(
    ...
    pub(in super::super) fn provider_exports_identifier(
        current_dir: &Path,
        module_name: &str,
        identifier: &str,
    ) -> bool {
    ...
    pub(in super::super) fn public_declaration_exports_identifier(
    ...
    ///   definitions.
    pub(in super::super) fn provider_definition_location(
        current_dir: &Path,
        module_name: &str,
        identifier: &str,
    ) -> Option<Location> {
    ...
    pub(in super::super) fn provider_struct_field_definition_location(
        current_dir: &Path,
        module_name: &str,
        struct_name: &str,
        field_name: &str,
    ...
    pub(in super::super) fn provider_definition_location_inner(
        current_dir: &Path,
        module_name: &str,
        identifier: &str,
    ...
    pub(in super::super) fn provider_reexport_source(
        text: &str,
        identifier: &str,
    ) -> Option<(String, String)> {
    ...
    pub(in super::super) fn provider_document_symbols_for_text(text: &str) -> Vec<DocumentSymbol> {
        parse_module_as_syntax_output(text)
            .or_else(|_| crate::terlan_syntax::parse_interface_module_as_syntax_output(text))

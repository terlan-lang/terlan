...
pub(super) fn candidate_edit(candidate: &ImportActionCandidate) -> TextEdit {
...
#[test]
pub(super) fn diagnostic_import_actions_recognize_unknown_constructor() {
...
#[test]
pub(super) fn diagnostic_import_actions_recognize_compact_unknown_constructor() {
    let actions = import_code_actions_for_diagnostic(
        &test_uri(),
        "module sample.\n\npub value(): Int ->\n  Vector(\"Alice\").\n",
        "unknown constructor Vector/1",
...
#[test]
pub(super) fn diagnostic_import_action_contains_workspace_edit() {
    let uri = test_uri();
    let actions = import_code_actions_for_diagnostic(
        &uri,
        "module sample.\n\npub value(): Int ->\n  Vector(\"Alice\").\n",
...
#[test]
pub(super) fn import_candidate_inserts_missing_vector_import() {
...
#[test]
pub(super) fn import_candidate_preserves_leading_module_docs() {
    let text = "\
//! Sample module docs.
module sample.
...
#[test]
pub(super) fn import_candidate_replaces_wrong_vector_import() {
    let text = "\
module sample.
...
#[test]
pub(super) fn import_candidate_skips_already_imported_vector() {
    let text = "\
module sample.
...
#[test]
pub(super) fn import_candidate_inserts_selected_function_from_provider_summary(
) -> std_io::Result<()> {
...
#[test]
pub(super) fn import_candidate_rejects_private_provider_function() -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();
...
#[test]
pub(super) fn import_candidate_rejects_stale_reexport_provider_summary() -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();
...
#[test]
pub(super) fn import_candidate_inserts_selected_function_from_generated_summary(
) -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
...
#[test]
pub(super) fn diagnostic_import_action_repairs_provider_function() -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();
...
#[test]
pub(super) fn import_candidate_groups_selected_function_with_existing_import() -> std_io::Result<()>
{
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
...
#[test]
pub(super) fn import_candidate_inserts_selected_constructor_from_provider_summary(
) -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
...
#[test]
pub(super) fn diagnostic_import_action_contains_provider_constructor_workspace_edit(
) -> std_io::Result<()> {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
...
#[test]
pub(super) fn diagnostic_import_action_repairs_provider_constructor_pattern() -> std_io::Result<()>
{
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")

...
///   documentation.
#[test]
pub(super) fn completion_items_include_local_and_imported_shapes_and_functions(
) -> std_io::Result<()> {
...
"
    ...
    crate::terlan_syntax::parse_module_as_syntax_output(&document.text)
        .expect("completion inventory fixture must parse");
...
",
    ...
    let uri = crate::lsp::uri::from_file_path(temp_dir.join("signatures.terl"))
        .map_err(|_| std_io::Error::new(ErrorKind::InvalidInput, "invalid temp URI"))?;
    let source = "\
module signatures.

use super::super::document::{DocumentKind, OpenDocument};
...
pub(super) async fn write_lsp_message(
...
pub(super) async fn read_lsp_message(reader: &mut DuplexStream) -> std_io::Result<String> {
...
///   ordering.
pub(super) fn assert_parse_diagnostic_message(message: &str, uri: &str, version: i32) {
...
pub(super) fn assert_type_diagnostic_message(message: &str, uri: &str, version: i32) {
    assert!(message.contains(r#""method":"textDocument/publishDiagnostics""#));
    assert!(message.contains(&format!(r#""uri":"{uri}""#)));
    assert!(message.contains(&format!(r#""version":{version}"#)));
    assert!(message.contains(r#""diagnostics":[{"#));
...
pub(super) fn assert_resolve_diagnostic_message(message: &str, uri: &str, version: i32) {
    assert!(message.contains(r#""method":"textDocument/publishDiagnostics""#));
    assert!(message.contains(&format!(r#""uri":"{uri}""#)));
    assert!(message.contains(&format!(r#""version":{version}"#)));
    assert!(message.contains(r#""diagnostics":[{"#));
...
///   diagnostic source.
pub(super) fn assert_type_warning_message(message: &str, uri: &str, version: i32) {
    assert!(message.contains(r#""method":"textDocument/publishDiagnostics""#));
    assert!(message.contains(&format!(r#""uri":"{uri}""#)));
    assert!(message.contains(&format!(r#""version":{version}"#)));
    assert!(message.contains(r#""diagnostics":[{"#));
...
/// - Performs a protocol-level smoke check on the serialized diagnostic
///   payload without depending on JSON field ordering beyond stable substrings.
pub(super) fn assert_template_diagnostic_message(message: &str, uri: &str, version: i32) {
    assert!(message.contains(r#""method":"textDocument/publishDiagnostics""#));
    assert!(message.contains(&format!(r#""uri":"{uri}""#)));
    assert!(message.contains(&format!(r#""version":{version}"#)));
    assert!(message.contains(r#""diagnostics":[{"#));
...
pub(super) fn assert_clear_diagnostic_message(message: &str, uri: &str, version: i32) {
    assert!(message.contains(r#""method":"textDocument/publishDiagnostics""#));
    assert!(message.contains(&format!(r#""uri":"{uri}""#)));
    assert!(message.contains(&format!(r#""version":{version}"#)));
    assert!(
...
async fn smoke_initialize_and_shutdown() -> std_io::Result<()> {
    ...
    assert!(initialize_response.contains(r#""documentFormattingProvider":true"#));
...
#[tokio::test]
async fn formatting_uses_the_canonical_compiler_formatter() -> std_io::Result<()> {
    ...
    write_lsp_message(
        &mut client_to_server,
        r#"{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{"textDocument":{"uri":"file:///tmp/format.terl","languageId":"terlan","version":1,"text":"module format.\npub keep(value: Bool): Bool -> value.\npub all(values: List[Bool]): Bool -> for_all(values, (value) -> keep(value)).\n"}}}"#,
    )
    .await?;
    let _ = timeout(
        Duration::from_millis(500),
    ...
    write_lsp_message(
        &mut client_to_server,
        r#"{"jsonrpc":"2.0","id":2,"method":"textDocument/formatting","params":{"textDocument":{"uri":"file:///tmp/format.terl"},"options":{"tabSize":4,"insertSpaces":true}}}"#,
    )
    .await?;
    ...
    .await
    .map_err(|_| std_io::Error::new(ErrorKind::TimedOut, "format response timeout"))??;
    assert!(response.contains(r#""id":2"#));
    assert!(response.contains("for_all(values, keep)"), "{response}");

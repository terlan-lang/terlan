...
fn open_script_document_uses_headerless_script_compilation_mode() {
    ...
    let parse_error = store.open(
        ...
        1,
        "terlan-script".to_string(),
    );

    assert!(parse_error.is_none());
    let document = store.snapshot(&uri).expect("open script");
    assert_eq!(document.kind, DocumentKind::Script);
    assert!(document.parse_ok);
    let syntax = document.parse_syntax().expect("reparse open script");
    assert_eq!(syntax.module_name, "script.Editor");
    assert!(syntax.declarations.iter().any(|declaration| {
        matches!(
            &declaration.payload,
...
#[test]
fn open_documents_format_source_interfaces_scripts_and_not_templates() {
    let source = OpenDocument {
        version: 1,
        language_id: "terlan".to_string(),
        kind: DocumentKind::Source,
        text: "module editor.Source.\npub value(): Int -> 42.\n".to_string(),
        parse_ok: true,
        resolve_diagnostics: Vec::new(),
        type_diagnostics: Vec::new(),
        template_diagnostics: Vec::new(),
    };
    assert!(source.formatted_text().expect("format source").is_some());

    let interface = OpenDocument {
        language_id: "terlan-interface".to_string(),
        text: "module editor.Interface.\npub value(): Int.\n".to_string(),
        ..source.clone()
    };
    assert!(interface
        .formatted_text()
        .expect("format interface")
        .is_some());

    let script = OpenDocument {
        language_id: "terlan-script".to_string(),
        kind: DocumentKind::Script,
        text: "answer = 40 + 2;\nanswer.\n".to_string(),
        ..source.clone()
    };
    assert!(script.formatted_text().expect("format script").is_some());

    let template = OpenDocument {
        language_id: "terlan-template-html".to_string(),
        kind: DocumentKind::Template,
        text: "<main>${answer}</main>\n".to_string(),
        ..source
    };
    assert_eq!(template.formatted_text().expect("skip template"), None);
}
...
#[test]
pub(super) fn open_document_loads_local_typi_interfaces_for_resolution() {
...
#[test]
pub(super) fn document_symbols_include_module_and_named_declarations() {
...
#[test]
pub(super) fn document_symbols_include_guarded_function_clauses() {
    let symbols = Backend::document_symbols_for_text(
        "\
...
#[test]
pub(super) fn document_symbols_include_raw_shape_declarations() {
    let symbols = Backend::document_symbols_for_text(
        "\

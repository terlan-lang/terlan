...
use crate::terlan_typeck::{
    CoreExpr, CoreExprSummary, CoreIntrinsicCall, CoreIntrinsicId, CorePrimitiveIntrinsic,
    CoreRuntimeCapability,
};

use super::super::{TargetProfile, TargetProfileViolation};
use super::module_support::is_std_runtime_module;

/// One target-gated standard-runtime operation family.
#[derive(Clone, Copy)]
pub(in crate::validation::target_profile) struct StdRuntimeOperationPolicy<'a> {
    pub(in crate::validation::target_profile) call_heads: &'a HashSet<String>,
    pub(in crate::validation::target_profile) diagnostic_label: &'static str,
    pub(in crate::validation::target_profile) canonical_module: &'static str,
    pub(in crate::validation::target_profile) supports_operation: fn(TargetProfile, &str) -> bool,
}
        ...
        && matches!(
            ...
            CoreIntrinsicId::Primitive(
                ...
                CoreRuntimeCapability::ConsolePrintln
                    ...
                    | CoreRuntimeCapability::FileIsExecutable
                    | CoreRuntimeCapability::FileSetExecutable
                    | CoreRuntimeCapability::FileCopy
                    | CoreRuntimeCapability::ArchiveCreate
                    | CoreRuntimeCapability::ArchiveExtract
                    | CoreRuntimeCapability::HashSha256File
                    | CoreRuntimeCapability::HashVerifySha256Manifest
                    | CoreRuntimeCapability::HashSha256Tree
                    | CoreRuntimeCapability::GitSourceTreeIdentity
            )
        )
}
...
pub(in crate::validation::target_profile) fn target_profile_supports_vm_native_bridge_operation(
    profile: TargetProfile,
    function: &str,
) -> bool {
...
pub(in crate::validation::target_profile) fn validate_std_runtime_operation_support(
    profile: TargetProfile,
    function_scope: &str,
    location: &str,
    module: &str,
    function: &str,
    policy: StdRuntimeOperationPolicy<'_>,
    violations: &mut Vec<TargetProfileViolation>,
) {
    if is_std_runtime_module(module, policy.canonical_module, policy.call_heads)
        && !(policy.supports_operation)(profile, function)
    {
        violations.push(TargetProfileViolation::unsupported(
            policy.diagnostic_label,
            profile,
            &format!("{function_scope} {location}"),
            &format!("{}.{function}", policy.canonical_module),
        ));
    }
}
...
pub(in crate::validation::target_profile) fn validate_std_runtime_operation_summary_support(
    profile: TargetProfile,
    function_scope: &str,
    location: &str,
    ...
    let function = summary
        ...
        .unwrap_or("<unknown>");
    if (policy.supports_operation)(profile, function) {
        return;
    }
    violations.push(TargetProfileViolation::unsupported(
        policy.diagnostic_label,

...
- `crates/terlan_cli/src/main.rs`: command handlers call validation modules.
- `crates/terlan_cli/src/formal_pipeline.rs`: `target_profile` validation is the
  backend-gate point before formal CoreIR artifacts are returned.
- Domain crates such as `terlan_html`: perform syntax- or artifact-specific
  checks.

...
///
/// Inputs:
/// - Cargo's compile-time `CARGO_MANIFEST_DIR` for `crates/terlan_cli`.
///
/// Output:
/// - Path to the repository root.
///
/// Transformation:
/// - Walks from `crates/terlan_cli` to `crates`, then to the repository root.
fn repo_root_for_test() -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(std::path::Path::parent)
        .expect("terlan_cli manifest should live under crates/")
        .to_path_buf()
}

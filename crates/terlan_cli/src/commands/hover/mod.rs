...
use terlan_typeck::type_check_syntax_module_output;
...
fn hover_component_function_names(tag_name: &str) -> Vec<String> {
    let snake_case = hover_pascal_to_snake_case(tag_name);
...
fn hover_component_prop_matches_param(prop_name: &str, param_name: &str) -> bool {
    prop_name == param_name
        ...
        || prop_name == hover_pascal_to_snake_case(param_name)
}

/// Converts a PascalCase-like name to snake_case.
///
/// Inputs:
/// - `name`: identifier text.
///
/// Output:
/// - Snake-case identifier text.
///
/// Transformation:
/// - Inserts underscores between lower/digit and uppercase transitions.
fn hover_pascal_to_snake_case(name: &str) -> String {

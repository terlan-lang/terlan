...
fn core_map_expr_to_js(fields: &[terlan_typeck::CoreMapExprField]) -> Option<String> {
    let fields = fields
        .iter()
        .map(|field| {
...
fn core_record_expr_to_js(fields: &[terlan_typeck::CoreRecordExprField]) -> Option<String> {
    let fields = fields
        .iter()
        .map(|field| {
            if !is_js_identifier(&field.key) {
...
fn core_record_update_expr_to_js(
    base: &CoreExpr,
    fields: &[terlan_typeck::CoreRecordExprField],
) -> Option<String> {
    let fields = fields
        .iter()
        .map(|field| {
...
fn core_template_expr_to_js(fields: &[terlan_typeck::CoreRecordExprField]) -> Option<String> {

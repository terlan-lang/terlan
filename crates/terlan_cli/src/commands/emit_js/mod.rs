...
";
    ...
    ///
    /// Inputs:
    /// - A checked Terlan module with a public binary function and a public
    ///   function using `1 |> add(2)`.
    ///
    /// Output:
    /// - Assertions over bootstrap JavaScript source before Oxc reprinting.
    ///
    ...
    /// - Compiles source through the formal pipeline, emits JS from
    ///   `CoreModule` through the bootstrap lowering helper, and checks that
    ///   `CoreExpr::BinaryOp` with `|>` becomes `add(1, 2)`.
    #[test]
...
";
        ...
        assert!(js.contains("export function add(x, y)"));
        assert!(js.contains("export function piped()"));
        assert!(js.contains("return add(1, 2);"), "{js}");
    }
...
";
    ...
    ///   with `|>` becomes a local JavaScript call with the piped expression

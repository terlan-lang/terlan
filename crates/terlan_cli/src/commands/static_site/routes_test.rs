...
fn markdown_input(
    alias: &str,
    source_path: &str,
    metadata: terlan_html::PageMetadata,
) -> SyntaxMarkdownInput {
    SyntaxMarkdownInput {
...
///
/// Transformation:
/// - Parses `body` through `terlan_html::parse_markdown` so route-discovery
...
fn markdown_input_with_body(
    ...
    input.document = terlan_html::parse_markdown(body, source_path).expect("parse Markdown body");
    input
}
...
fn markdown_static_routes_infer_nested_content_path() {
    let routes = discover_markdown_static_routes(&[markdown_input(
        ...
        terlan_html::PageMetadata::default(),
    )])

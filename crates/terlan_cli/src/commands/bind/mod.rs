...
In the current 0.0.3 roadmap slice:
...
const POLARS_FILES: &[GeneratedFile] = &[
    GeneratedFile {
        path: "terlan.toml",
...
pub(crate) fn run(cmd: CliCommand) -> ExitCode {
    ...
    match cmd.args[0].as_str() {
        ...
        other => {
            eprintln!("unsupported bind target `{other}`; supported targets: rust");
            print_usage();
            ExitCode::from(2)
        }
    }
...
mod tests {
    ...
    fn generate_package_writes_polars_skeleton() {
        ...
        let dataframe_source =
            fs::read_to_string(out_dir.join("src/std/native/polars/DataFrame.terl"))
        ...
        assert!(
            native_abi.contains("[result_conversions.\"std.native.polars.DataFrame.read_csv\"]")
        );

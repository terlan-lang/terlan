...
///
/// Transformation:
/// - Combines the process id and test name to keep concurrent test runs from
///   sharing package fixtures.
fn temp_dir(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("terlan_serve_tls_{name}_{}", std::process::id()));
    if dir.exists() {
        fs::remove_dir_all(&dir).expect("remove stale TLS fixture");
    }
    dir
}

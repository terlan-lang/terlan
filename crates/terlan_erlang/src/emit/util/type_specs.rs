...
pub(in crate::emit) fn lower_type_to_spec(input: &str) -> ErlType {
...
pub(in crate::emit) fn lower_tuple_type_elem_to_spec(input: &str) -> ErlType {
...
pub(in crate::emit) fn lower_bare_type_to_spec(src: &str) -> ErlType {
...
pub(in crate::emit) fn is_native_vector_type_head(head: &str) -> bool {
    matches!(
        head,
...
/// type tree.
pub(in crate::emit) fn compact_type_application(input: &str) -> String {
    input
        .replace("# {", "#{")
        .replace(" [", "[")
        .replace("[ ", "[")
...
pub(in crate::emit) fn split_fixed_array_args(mut args: Vec<String>) -> Option<(String, String)> {
    if args.len() != 2 {
        return None;
    }
...
pub(in crate::emit) fn erlang_type_param_name(param: &str) -> String {
...
pub(in crate::emit) fn strip_wrapping_parens(input: &str) -> Option<&str> {
...
pub(in crate::emit) fn compact_spaces(input: &str) -> String {
...
pub(in crate::emit) fn map_type_name(name: &str) -> String {
    match name {
...
pub(in crate::emit) fn map_module_name(name: &str) -> String {
...
pub(in crate::emit) fn is_generic_type_var(name: &str) -> bool {
...
pub(in crate::emit) fn is_upper_identifier(name: &str) -> bool {
...
pub(in crate::emit) fn is_builtin_type_name(name: &str) -> bool {
    matches!(
        name,
        "Int"
            | "Float"
...
/// generic variables.
pub(in crate::emit) fn is_custom_type_name(name: &str) -> bool {
    let mut chars = name.chars();
...
pub(in crate::emit) fn is_lower_identifier(name: &str) -> bool {
    let mut chars = name.chars();
...
pub(in crate::emit) fn is_raw_atom_name(name: &str) -> bool {
    let mut chars = name.chars();
    matches!(chars.next(), Some(ch) if ch.is_ascii_lowercase())
...
pub(in crate::emit) fn to_erlang_type_name(name: &str) -> String {
    let mut out = String::new();
...
pub(in crate::emit) fn map_struct_name(name: &str) -> String {
    name.to_lowercase()
}
...
pub(in crate::emit) fn parse_named_type_args(src: &str) -> Option<(&str, Vec<String>)> {
...
pub(in crate::emit) fn split_top_level_csv(input: &str) -> Vec<String> {
    let mut depth_p = 0;
    let mut depth_b = 0;
    let mut depth_br = 0;
...
pub(in crate::emit) fn split_top_level_union(input: &str) -> Vec<String> {
    let mut depth_p = 0;
    let mut depth_b = 0;
    let mut depth_br = 0;
    let mut start = 0usize;
...
/// branches.
pub(in crate::emit) fn is_union(input: &str) -> bool {
...
pub(in crate::emit) fn split_top_level_arrow(input: &str) -> Option<(String, String)> {
    let chars: Vec<char> = input.chars().collect();
    let mut depth_p = 0;
    let mut depth_b = 0;
    let mut depth_br = 0;
...
pub(in crate::emit) fn split_named_tuple_type_elem(input: &str) -> Option<(&str, &str)> {
...
pub(in crate::emit) fn parse_type_atom_literal(input: &str) -> Option<String> {
...
/// characters.
pub(in crate::emit) fn unquote_type_atom(text: &str) -> Option<String> {

...
- [ ] V9-1: Make release preparation resumable and publication retry-safe.
  ...
  - Reuse hosted downloads only after checking producer identity and cached
    ...
    The publication entry point now holds a repository-shared local publication
    lease and the preparation/restoration leases before preflight, tag handling,
    and upload. Worktrees share the publication lease; separate clones or hosts

...
- [ ] Add a PyTorch package through generated stable C ABI bindings.
...
- [ ] Slice 2: finalize torch package execution and baseline interop smoke.
...
- [ ] Add executable Terlan machine-learning experiments that combine Polars,
  ndarray, and PyTorch.
...
- [ ] Slice 5: close generated C++ package execution and manifest stability.
...
- [ ] Slice 4: make ML experiment results reproducible and portable through package CI.
...
- [ ] Implement automatic C++ package binding generation.
  ...
  - Acceptance remaining: prove this generator against the first real external
    OpenCV package without compiler-local bindings or fixture-only metadata.
...
- [ ] Add CUDA package support as an explicit optional native capability.
  ...
  - Current strict gate state: `make cuda-package-check` exists as an opt-in
    target and refuses to fake success; on CPU-only machines it fails with a
    stable message pointing users back to `cuda-package-availability-check`.
  ...
  - Acceptance remaining: implement a real external `terlan-cuda` package gate
    and the separate `terlan-pytorch` CUDA profile. Their execution reports and
    gate names must identify which backend was exercised.
...
- [ ] Slice 3: close the direct Rust CUDA package execution loop end-to-end.
...
- [ ] Slice 4: unify all external package gates under one machine-readable execution matrix.
...
- [ ] Slice 5: finish and close the C++ binding generator completion gap.
...
- [ ] Audit and port every relevant Erlang/OTP test suite still present under the external
  ...
  - Requirement: P0 areas (`scheduler`, `mailbox`, `timers`,
    ...
    become `ported` with a passing replacement gate and the obsolete external
    suite material deleted or explicitly removed after equivalent coverage.
  ...
  - Completed slice: `docs/runtime/TERLAN_VM_BEAM_TEST_SUITE_SUMMARY.tsv`
    ...
    external suite audit. The gate fails if the discovered 1,604-file active
    corpus, active file-status ledger, 316-row completed-deletion manifest,
    classification totals, or owner totals drift, so the migration inventory
    cannot silently change. Deleted rows no longer inflate the active ledger.
  - Current gate hardening: `docs/runtime/TERLAN_VM_BEAM_TEST_PORT_PLAN.tsv`
    is now paired with
    ...
    port/delete classifications, and the replacement-gate distribution so the
    migration plan cannot silently drift while the 1,604-file active corpus
    remains only partially ported.
  - Completed slice: `docs/runtime/TERLAN_VM_BEAM_TEST_PORT_STATUS.tsv`
    now records executable `ported`/`partial`/`not-ported` and
    `deleted`/`not-deleted` status for each prioritized port/delete area, plus
    a separately checked `rust-runtime`, `transitional-evaluator`, `native-aot`,
    or `not-proven` execution path and its owning execution gate.
    ...
    rejects unknown status values, rejects unrecognized native-AOT proof gates,
    and rejects `deleted` claims while matching external suite files still
    exist. Current audited state is 8 partial ports, 3 not-yet-ported areas,
    1 deleted and 10 not-yet-deleted legacy areas, 7 Rust-runtime execution
    paths, 4 unproven execution paths, and no native-AOT parity claim.
  - Current ledger shape: `TERLAN_VM_BEAM_TEST_FILE_STATUS.tsv` contains only
    the 1,604 active, not-yet-ported external files. The 316 completed removals
    live in `TERLAN_VM_BEAM_TEST_DELETION_MANIFEST.tsv` as compact path,
    classification, replacement-gate, and corpus-generation tombstones. The
    audit rejects a tombstoned path if it reappears in the external corpus.
  - Current progress: the OTP `lib/stdlib/test/pool_SUITE.erl` peer/RPC wrapper
    ...
    gate.
  - Current progress: the configurable termination-delay helper
  ...
  - Current progress: the OTP `lib/stdlib/test/naughty_child.erl` unlinking
  ...
  - Current progress: the OTP `lib/stdlib/test/dummy_via.erl` custom registry
  ...
  - Current progress: the 2,594-line OTP
  ...
  - Current progress: the 1,302-line OTP
  ...
  - Current progress: the 1,405-line OTP
  ...
  - Current progress: the 2,461-line OTP
  ...
  - Current progress: the 5,887-line OTP
  ...
  - Current progress: the OTP `erts/emulator/test/receive_SUITE.erl` harness is
  ...
  - Current progress: the OTP `erts/emulator/test/timer_bif_SUITE.erl` harness
  ...
  - Current progress: the OTP `erts/emulator/test/time_SUITE.erl` harness is
    ...
    remains open.
  - Current progress: the OTP `erts/emulator/test/statistics_SUITE.erl`
    ...
    item remains open.
  - Current progress: the OTP `erts/emulator/test/system_info_SUITE.erl`
    ...
    open.
  - Current progress: the OTP `erts/emulator/test/system_profile_SUITE.erl`
  ...
  - Current progress: the 2,284-line OTP
  ...
  - Current progress: the OTP
    ...
    parity item remains open.
  - Current progress: the OTP `erts/emulator/test/tracer_SUITE.erl` suite,
  ...
  - Current progress: the OTP `erts/emulator/test/tuple_SUITE.erl` suite is
  ...
  - Current progress: the OTP `erts/emulator/test/z_SUITE.erl` last-suite
  ...
  - Current progress: the five tracked
  ...
  - Current progress: the OTP `erts/emulator/test/a_SUITE.erl` first-suite
    ...
    remains open.
  - Current progress: the complete tracked
  ...
  - Current progress: the tracked
    ...
    parity item remains open.
  - Current progress: the tracked `erts/emulator/test/atomics_SUITE.erl` is
  ...
  - Current progress: the tracked `erts/emulator/test/beam_SUITE.erl` and its
  ...
  - Current progress: the tracked
    ...
    broader parity item remains open.
  - Current progress: the tracked `erts/emulator/test/bif_SUITE.erl` is retired
    ...
    parity item remains open.
  - Current progress: the tracked `erts/emulator/test/big_SUITE.erl` and its
  ...
  - Current progress: the tracked `erts/emulator/test/binary_SUITE.erl` and its
  ...
  - Current progress: the tracked `erts/emulator/test/bs_bincomp_SUITE.erl`,
  ...
  - Current progress: the tracked `erts/emulator/test/bs_construct_SUITE.erl`,
  ...
  - Current progress: the tracked `erts/emulator/test/bs_match_int_SUITE.erl`,
  ...
  - Current progress: the tracked `erts/emulator/test/bs_match_misc_SUITE.erl`,
  ...
  - Current progress: the tracked `erts/emulator/test/busy_port_SUITE.erl` and
  ...
  - Current progress: the tracked `erts/emulator/test/call_trace_SUITE.erl` and
  ...
  - Current progress: the tracked `erts/emulator/test/code_SUITE.erl`,
  ...
  - Current progress: the tracked
    ...
    the later corpus.
  - Current progress: the tracked `erts/emulator/test/counters_SUITE.erl` is
    ...
    and the later corpus.
  - Current progress: the tracked `erts/emulator/test/ddll_SUITE.erl` and
  ...
  - Current progress: the tracked `erts/emulator/test/dgawd_handler.erl` is
  ...
  - Current progress: the tracked `erts/emulator/test/dirty_bif_SUITE.erl` is
  ...
  - Current progress: the tracked `erts/emulator/test/dirty_nif_SUITE.erl` and
  ...
  - Current progress: the tracked `erts/emulator/test/distribution_SUITE.erl`,
    ...
    `dummy.erl`.
  - Current progress: the tracked `erts/emulator/test/driver_SUITE.erl` and
  ...
  - Current progress: the tracked `erts/emulator/test/dummy.erl` stub is
    ...
    API would create false parity. Its exact inventory row and deletion
    tombstone preserve that decision and prevent reintroduction. The canonical
    audit passes with 1,604 active not-yet-ported files and 316 completed
    removals; `dump_SUITE.erl` is the next ordered active entry.
  - Current progress: the tracked `erts/emulator/test/dump_SUITE.erl` crash-dump
  ...
  - Acceptance: the gate fails when a new Erlang test file appears under
    `terlan-vm` without an inventory row and classification.
  - Acceptance: the gate fails when an inventory row references a missing file,
    uses an unknown classification, or claims replacement coverage through a
    non-existent Rust/Terlan gate.
  - Acceptance: the gate fails while any `remove-non-portable` suite still
    exists in the `terlan-vm` checkout.
...
- [ ] Investigate and fix VM HTTP socket concurrency after the Erlang/OTP test
...
- [ ] Add a Lisp-style interactive Terlan debugger for command-line VM
  execution.
  - Current scope note: this slice intentionally does not implement stepping,
    frame inspection, process inspection, restart selection, or live VM pause.
    The command exits with `debugger_unimplemented` until the VM debugger
    runtime lands.
...
- [ ] Audit and complete the Lean proof track for the current Terlan language,
...
- [ ] Slice 4: run Lean proof slice-by-slice as a release-critical quality
  matrix.
...
- [ ] Slice 5: close explicit Lean proof gaps only through executable restoration
...
- [ ] Slice 9: add semantic-end-to-end Lean proof smoke through VM/runtime and
...
- [ ] Slice 10: generate executable feature-proof evidence matrix and reject any
...
- [ ] Slice 11: make Lean proof closure auditable by machine-readable release
...
- [ ] Slice 12: generate synthetic minimal counterexamples from failing proof
...
- [ ] Slice 14: close the 0.0.7 Lean proof gap for VM-native boundary

...
| `scripts/self_validation/NativeArtifactsTest.terl` | native artifact checker | Regenerates and compares NativeBoundary JSON/Rust artifacts through direct-AOT Terlan, then compiles every generated Rust skeleton without invoking `erlc` or accepting generated Erlang loader intermediates. | default-release-gate | Keep the checker independent of stock OTP and reject any return of generated Erlang loader artifacts. |
...
| `scripts/self_validation/RepositoryValidation.terls` | typed release boundary checker | Rejects scratch `.beam`, `.erl`, and `.hrl` artifacts in release contents. | default-release-gate | Keep as a no-OTP leak guard. |
...
| `scripts/self_validation/HttpRuntimeStackTest.terl` | HTTP runtime stack checker | Checks Hyper protocol ownership on VM task executors, the native MIME and cookie boundaries, and live-reload watcher integration. | default-release-gate | Keep rejecting manual HTTP parsing and any return of local cookie/header parsing outside the native boundary. |

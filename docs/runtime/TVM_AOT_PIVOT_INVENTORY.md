...
| --- | --- | --- | --- |
| `Makefile` | quality-gate, native-consumer-test | reusable-runtime-semantics | Keep conformance and release gates on canonical descriptor and native-image execution tests. |
...
| `tools/check_battleship_external_vm_contract.py` | native-consumer-test | reusable-runtime-semantics | Validate the external application bundle against its native image, paired VM runtime, isolated worker, metadata, and launcher. |
| `tools/package_release_artifact.py` | artifact-consumer | reusable-runtime-semantics | Package and smoke the installed native VM runtime against the emitted application image. |
